...
describe('ECDSA', function() {
  ...
  for (const ec of curves) {
    it(`should generate keypair and sign DER (${ec.id})`, () => {
      const msg = random.randomBytes(ec.size);
      const priv = ec.PrivateKey.generate();
      const pub = priv.toPublic();
      const pubu = ec.PublicKey.fromPoint(pub.toPoint(false));

      const sig = priv.sign(msg);
      assert(pub.verify(msg, sig, pub));
      assert(pubu.verify(msg, sig, pubu));
      msg[0] ^= 1;
      assert(!pub.verify(msg, sig, pub));
      assert(!pubu.verify(msg, sig, pubu));
      msg[0] ^= 1;
    ...
    it(`should generate keypair and sign RS (${ec.id})`, () => {
      ...
      const der = priv.sign(msg).toDER();
      const sig = ec.Signature.fromDER(der);
    ...
    it(`should tweak keys (${ec.id})`, () => {
      ...
      const tpriv = priv.tweakAdd(tweak);
      const tpub = pub.tweakAdd(tweak);

      const msg = random.randomBytes(ec.size);

      const sig = tpriv.sign(msg);
      assert(tpub.verify(msg, sig));

      const der = tpriv.sign(msg).toDER();
      const sig2 = ec.Signature.fromDER(der);
      assert(tpub.verify(msg, sig2));
    });

    it(`should do ECDH (${ec.id})`, () => {
      const alicePriv = ec.PrivateKey.generate();
      const alicePub = alicePriv.toPublic();
  ...
  describe('RFC6979 vector', function() {
    function test(opt) {
      const curve = opt.curve;
      const key = Buffer.from(opt.key, 'hex');
      const x = Buffer.from(opt.pub.x, 'hex');
      const y = Buffer.from(opt.pub.y, 'hex');
      ...
      for (const c of opt.cases) {
        ...
        it(desc, () => {
          const dgst = hash.digest(msg);
          const priv = new curve.PrivateKey(key);
          const pub = new curve.PublicKey(x, y);
          const sign = priv.sign(dgst);

          assert(pub.validate(), 'Invalid public key');
          assert(pub.verify(dgst, sign), 'Invalid signature (1)');
          assert(pub.verify(dgst, sig), 'Invalid signature (2)');
        });
      }
    }
  ...
  describe('Maxwell\'s trick', function() {
    ...
    for (const [i, vector] of vectors.entries()) {
      it(`should pass on vector #${i}`, () => {
        const curve = vector.curve;
        const point = Buffer.from(vector.pub, 'hex');
        const msg = Buffer.from(vector.message, 'hex');
        const der = Buffer.from(vector.sig, 'hex');
        const key = curve.PublicKey.fromPoint(point);
        const sig = curve.Signature.fromDER(der);

        const actual = key.verify(msg, sig);
        assert.strictEqual(actual, vector.result);
      });
    }
  });
  ...
  for (const curve of [secp256k1, secp256k1n]) {
    const msg = Buffer.from(lax2.msg, 'hex');
    const der = Buffer.from(lax2.sig, 'hex');
    const point = Buffer.from(lax2.pub, 'hex');

    it('should verify lax signature', () => {
      const key = curve.PublicKey.fromPoint(point);
      const sig = curve.Signature.fromDER(der);
      assert.strictEqual(key.verify(msg, sig), true);
    });
  }

  for (const curve of [secp256k1, secp256k1n]) {
    it('should recover the public key from a signature', () => {
      const priv = curve.PrivateKey.generate();
      const pub = priv.toPublic();
      const msg = Buffer.alloc(32, 0x01);
      const sig = priv.sign(msg);
      ...
      for (let i = 0; i < 4; i++) {
        ...
        if (r.toPoint().equals(pub.toPoint())) {
          found = true;
          break;
        }
      }

...
describe('HKDF', function() {
  for (const [i, vector] of vectors.entries()) {
    const {alg, ikm, salt, info, len, prkE, okmE} = vector;

    it(`should do hkdf (${i + 1})`, () => {
      const prk1 = HKDF.extract(alg, ikm, salt);
      const okm1 = HKDF.expand(alg, prk1, info, len);

      assert.bufferEqual(prk1, prkE);
    ...
    it(`should do object-oriented hkdf (${i + 1})`, () => {
      ...
      assert.bufferEqual(hkdf.prk, prkE);
    ...
    it(`should do incremental hkdf (${i + 1})`, () => {
      ...
      for (let i = 0; i < okmE.length; i++)
        parts.push(hkdf.generate(1));

      const okm = Buffer.concat(parts);

  ...
  fromString(str, passwd) {
    ...
    switch (block.type) {
      ...
      case 'EC PRIVATE KEY': { // SEC1
        ...
        this.type = type;
        this.key = leftPad(key.privateKey.value, size);
...
class RawPrivateKey extends bio.Struct {
  ...
  write(bw, passwd) {
    ...
    switch (this.type) {
      ...
      case keyTypes.ED25519: {
        writeBytes(bw, this.point);
        writeBytes(bw, bio.concat(this.key, this.point)); // wtf?
        break;
      }
      default: {
        throw new Error('Invalid key.');
    ...
    switch (this.type) {
      ...
      case keyTypes.P521: {
        ...
        if (this.type === keyTypes.P256)
          this.key = leftPad(this.key, 32);
        else if (this.type === keyTypes.P384)
          this.key = leftPad(this.key, 48);
        else
          this.key = leftPad(this.key, 66);

...
const ciphers_ = require('./internal/ciphers');
...
class SSHPublicKey extends bio.Struct {
  ...
  toKey() {
    switch (this.type) {
      ...
      case 'rsa': {
        return new rsa.RSAPublicKey(this.n, this.e);
      }
      ...
      case 'eddsa': {
        switch (this.curve) {
          case 'p256':
            return p256.PublicKey.fromPoint(this.point);
          case 'p384':
            return p384.PublicKey.fromPoint(this.point);
          case 'p521':
            return p521.PublicKey.fromPoint(this.point);
          case 'ed25519':
            return ed25519.PublicKey.fromPoint(this.point);
          default:
            throw new assert.AssertionError('Invalid key.');
        }
        break;
  ...
  toKey() {
    switch (this.type) {
      ...
      case 'eddsa': {
        switch (this.curve) {
          case 'p256':
            return new p256.PrivateKey(this.key);
          case 'p384':
            return new p384.PrivateKey(this.key);
          case 'p521':
            return new p521.PrivateKey(this.key);
          case 'ed25519':
            return new ed25519.PrivateKey(this.key);
          default:
            throw new assert.AssertionError('Invalid key.');
        }
        break;
...
function derive(cipher, passwd, salt, rounds) {
  ...
  const {blockSize, keySize} = ciphers_.get(name);
...
function encrypt(data, cipher, passwd, salt, rounds) {
  ...
  const ctx = new Cipher(name).init(key, iv);
...
function decrypt(data, cipher, passwd, salt, rounds) {
  ...
  const [name, key, iv] = derive(cipher, passwd, salt, rounds);
  const ctx = new Decipher(name).init(key, iv);

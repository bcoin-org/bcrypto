...
ed25519.privateKeyConvert = function privateKeyConvert(secret) {
  ...
  try {
    wasm.throws('bcrypto_ed25519_privkey_convert', out, secret);
...
ed25519.scalarTweakAdd = function scalarTweakAdd(scalar, tweak) {
  ...
  try {
    wasm.throws('bcrypto_ed25519_scalar_tweak_add', out, scalar, tweak);
    return wasm.read(out, 32);
  } finally {
    wasm.restore(save);
  }
...
ed25519.scalarTweakMul = function scalarTweakMul(scalar, tweak) {
  ...
  try {
    wasm.throws('bcrypto_ed25519_scalar_tweak_mul', out, scalar, tweak);
    return wasm.read(out, 32);
  } finally {
    wasm.restore(save);
  }
...
ed25519.scalarNegate = function scalarNegate(scalar) {
  ...
  try {
    wasm.throws('bcrypto_ed25519_scalar_negate', out, scalar);
    return wasm.read(out, 32);
  } finally {
    wasm.restore(save);
  }
...
ed25519.scalarInvert = function scalarInvert(scalar) {
  ...
  try {
    wasm.throws('bcrypto_ed25519_scalar_invert', out, scalar);
    return wasm.read(out, 32);
  } finally {
    wasm.restore(save);
  }
...
ed25519.publicKeyCreate = function publicKeyCreate(secret) {
  ...
  try {
    wasm.throws('bcrypto_ed25519_pubkey_create', out, secret);
    return wasm.read(out, 32);
  } finally {
    wasm.restore(save);
  }
...
ed25519.publicKeyFromScalar = function publicKeyFromScalar(scalar) {
  ...
  try {
    wasm.throws('bcrypto_ed25519_pubkey_from_scalar', out, scalar);
    return wasm.read(out, 32);
  } finally {
    wasm.restore(save);
  }
...
ed25519.publicKeyConvert = function publicKeyConvert(key) {
  ...
  try {
    wasm.throws('bcrypto_ed25519_pubkey_convert', out, key);
    return wasm.read(out, 32);
  } finally {
    wasm.restore(save);
  }
...
ed25519.publicKeyDeconvert = function publicKeyDeconvert(key, sign) {
  ...
  try {
    wasm.throws('bcrypto_ed25519_pubkey_deconvert', out, key, sign);
    return wasm.read(out, 32);
  } finally {
    wasm.restore(save);
  }
...
ed25519.publicKeyVerify = function publicKeyVerify(key) {
  ...
  return wasm.call('bcrypto_ed25519_pubkey_verify', key) === 1;
};
...
ed25519.publicKeyTweakAdd = function publicKeyTweakAdd(key, tweak) {
  ...
  try {
    wasm.throws('bcrypto_ed25519_pubkey_tweak_add', out, key, tweak);
    return wasm.read(out, 32);
  } finally {
    wasm.restore(save);
  }
...
ed25519.publicKeyTweakMul = function publicKeyTweakMul(key, tweak) {
  ...
  try {
    wasm.throws('bcrypto_ed25519_pubkey_tweak_mul', out, key, tweak);
    return wasm.read(out, 32);
  } finally {
    wasm.restore(save);
  }
...
ed25519.publicKeyAdd = function publicKeyAdd(key1, key2) {
  ...
  try {
    wasm.throws('bcrypto_ed25519_pubkey_add', out, key1, key2);
    return wasm.read(out, 32);
  } finally {
    wasm.restore(save);
  }
...
ed25519.publicKeyNegate = function publicKeyNegate(key) {
  ...
  try {
    wasm.throws('bcrypto_ed25519_pubkey_negate', out, key);
    return wasm.read(out, 32);
  } finally {
    wasm.restore(save);
  }
...
ed25519.sign = function sign(msg, secret, ph, ctx) {
  ...
  try {
    wasm.throws('bcrypto_ed25519_sign', out, msg, msg.length, secret, ph, ctx);
...
ed25519.signWithScalar = function signWithScalar(msg, scalar, prefix, ph, ctx) {
  ...
  const save = wasm.save();
  const out = wasm.alloc(64);
  const extsk = Buffer.concat([scalar, prefix]);

  try {
    wasm.throws('bcrypto_ed25519_sign_with_scalar', out, msg, msg.length, extsk, ph, ctx);
    return wasm.read(out, 64);
  } finally {
    wasm.restore(save);
  }
...
ed25519.verifyBatch = function verifyBatch(batch, ph, ctx) {
  backend.reseed();
  return binding.verifyBatch(batch, ph, ctx);
};

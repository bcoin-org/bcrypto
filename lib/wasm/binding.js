...
wasm.reseed = function reseed() {
  if (!crypto)
    crypto = require('crypto');

  const calls = wasm.call('bcrypto_random_calls');
  const diff = calls - lastReseed;

  // We try to reseed yarrow with
  // node's RNG every 100 calls.
  if (calls === 0 || diff < 0 || diff > 100) {
    wasm.call('bcrypto_random_seed', crypto.randomBytes(64), 64);
    lastReseed = calls;
  }
};

...
function derive(passwd, salt, keyLen, ivLen) {
  ...
  while (keyLen > 0 || ivLen > 0) {
    ...
    if (keyLen > 0) {
      ...
      used = Math.min(keyLen, last.length);
      last.copy(key, keyStart, 0, used);
      keyLen -= used;
    }

    if (used < last.length && ivLen > 0) {
      const ivStart = iv.length - ivLen;
      const length = Math.min(ivLen, last.length - used);
      last.copy(iv, ivStart, used, used + length);
      ivLen -= length;
    }
  }

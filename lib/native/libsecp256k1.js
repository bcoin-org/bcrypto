...
function handle() {
  if (!ctx) {
    ctx = binding.secp256k1_context_create();
...
function privateKeyGenerate() {
  return binding.secp256k1_privkey_generate(handle(), binding.entropy());
}
...
function privateKeyVerify(key) {
  ...
  return binding.secp256k1_privkey_verify(handle(), key);
}
...
function privateKeyImport(json) {
  ...
  return binding.secp256k1_privkey_import(handle(), json.d);
}
...
function privateKeyTweakAdd(key, tweak) {
  ...
  return binding.secp256k1_privkey_tweak_add(handle(), key, tweak);
}
...
function privateKeyTweakMul(key, tweak) {
  ...
  return binding.secp256k1_privkey_tweak_mul(handle(), key, tweak);
}
...
function privateKeyReduce(key) {
  assert(Buffer.isBuffer(key));
  return binding.secp256k1_privkey_reduce(handle(), key);
}
...
function privateKeyNegate(key) {
  assert(Buffer.isBuffer(key));
  return binding.secp256k1_privkey_negate(handle(), key);
}
...
function privateKeyInvert(key) {
  assert(Buffer.isBuffer(key));
  return binding.secp256k1_privkey_invert(handle(), key);
}
...
function isLowS(sig) {
  assert(Buffer.isBuffer(sig));
  return binding.secp256k1_signature_is_low_s(handle(), sig);
}
...
function isLowDER(sig) {
  assert(Buffer.isBuffer(sig));
  return binding.secp256k1_signature_is_low_der(handle(), sig);
}
 ...
 * @param {Buffer} msg
 * @param {Buffer} key - Private key.
 * @returns {Array} R/S-formatted signature and recovery ID.
 */
 ...
 * @param {Buffer} msg
 * @param {Buffer} key - Private key.
 * @returns {Array} DER-formatted signature and recovery ID.
 */
...
function schnorrSign(msg, key) {
  ...
  return binding.secp256k1_schnorr_sign(handle(), msg, key);
}

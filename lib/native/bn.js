...
class BN extends Native {
  ...
  condSwap(num, flag) {
    ...
    const items = [this.clone(), num.clone(), this.red, num.red];

    this.inject(items[0 ^ cond]);
    num.inject(items[1 ^ cond]);

    this.red = items[2 + (0 ^ cond)];
    num.red = items[2 + (1 ^ cond)];
  ...
  condInject(num, flag) {
    ...
    this.inject(items[cond]);
    this.red = items[2 + cond];
  ...
  condSet(num, flag) {
    const cond = ((flag >> 31) | (-flag >> 31)) & 1;
    const items = [this.clone(), BN.fromNumber(num)];

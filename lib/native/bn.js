...
function powm(x, e, m, size) {
  ...
  if (size === 0 && e <= 0x3ffffffn)
    return rtl(x, e, m);

  return ltr(x, e, m, size);
}
...
function ltr(x, e, m, size) {
  const bits = Math.max(bitLength(e), size);
  ...
  let pos = BigInt((steps - 1) * WND_WIDTH);
  let r = 1n;

  for (let i = 0; i < steps; i++) {
    const bits = Number((e >> pos) & WND_MASK_BIG);

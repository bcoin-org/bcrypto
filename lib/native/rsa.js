...
function privateKeyGenerate(bits, exponent) {
  ...
  const raw = binding.privateKeyGenerate(bits, exponent);
...
function privateKeyCompute(key) {
  const raw = binding.privateKeyRecover(_privateKeyExport(key));
...
function privateKeyVerify(key) {
  return binding.privateKeyVerify(_privateKeyExport(key));
}
...
function publicKeyVerify(key) {
  return binding.publicKeyVerify(_publicKeyExport(key));
}
...
function sign(hash, msg, key) {
  ...
  if (hash == null)
    ...
    hash = backend.hashes[hash];

  return binding.sign(hash, msg, _privateKeyExport(key));
}
...
function verify(hash, msg, sig, key) {
  ...
  return binding.verify(hash, msg, sig, _publicKeyExport(key));
}
...
function encrypt(msg, key) {
  return binding.encrypt(msg, _publicKeyExport(key));
}
...
function decrypt(msg, key) {
  return binding.decrypt(msg, _privateKeyExport(key));
}
...
function encryptOAEP(hash, msg, key, label) {
  return binding.encryptOAEP(backend.hash(hash),
                             msg,
...
function decryptOAEP(hash, msg, key, label) {
  return binding.decryptOAEP(backend.hash(hash),
                             msg,
...
function signPSS(hash, msg, key, saltLen) {
  return binding.signPSS(backend.hash(hash),
                         msg,
                         _privateKeyExport(key),
                         saltLen);
}
...
function verifyPSS(hash, msg, sig, key, saltLen) {
  return binding.verifyPSS(backend.hash(hash),
                           msg,
                           sig,
                           _publicKeyExport(key),
                           saltLen);
...
function encryptRaw(msg, key) {
  return binding.encryptRaw(msg, _publicKeyExport(key));
}
...
function decryptRaw(msg, key) {
  return binding.decryptRaw(msg, _privateKeyExport(key));
}
...
function veil(msg, bits, key) {
  return binding.veil(msg, bits, _publicKeyExport(key));
}
...
function unveil(msg, bits, key) {
  return binding.unveil(msg, bits, _publicKeyExport(key));
}

...
function paramsGenerate(bits) {
  ...
  const items = backend.paramsGenerate(bits);
...
async function paramsGenerateAsync(bits) {
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      backend.paramsGenerateAsync(bits, cb);
    } catch (e) {
      reject(e);
    }
  });
...
function paramsExport(params) {
  assert(params instanceof DSAParams);
  return backend.paramsExport(params.p, params.q, params.g);
}
...
function paramsImport(raw) {
  const items = backend.paramsImport(raw);
  const params = new DSAParams();
...
function privateKeyCreate(params) {
  ...
  const items = backend.privateKeyCreate(
    params.p,
    params.q,
    params.g
  );
...
function privateKeyCompute(key) {
  ...
  const y = backend.privateKeyCompute(
    key.p,
    key.q,
    key.g,
    key.y,
...
function privateKeyVerify(key) {
  ...
  return backend.privateKeyVerify(
    key.p,
    key.q,
    key.g,
    key.y,
...
function privateKeyExport(key) {
  ...
  return backend.privateKeyExport(
    key.p,
    key.q,
    key.g,
    key.y,
...
function privateKeyImport(raw) {
  const items = backend.privateKeyImport(raw);
  const key = new DSAPrivateKey();
...
function privateKeyExportPKCS8(key) {
  ...
  return backend.privateKeyExportPKCS8(
    key.p,
    key.q,
    key.g,
    key.y,
...
function privateKeyImportPKCS8(raw) {
  const items = backend.privateKeyImportPKCS8(raw);
  const key = new DSAPrivateKey();
...
function publicKeyVerify(key) {
  ...
  return backend.publicKeyVerify(
    key.p,
    key.q,
    key.g,
    key.y
...
function publicKeyExport(key) {
  ...
  return backend.publicKeyExport(
    key.p,
    key.q,
    key.g,
    key.y
...
function publicKeyImport(raw) {
  const items = backend.publicKeyImport(raw);
...
function publicKeyExportSPKI(key) {
  ...
  return backend.publicKeyExportSPKI(
    key.p,
    key.q,
    key.g,
    key.y
...
function publicKeyImportSPKI(raw) {
  const items = backend.publicKeyImportSPKI(raw);
  const key = new DSAPublicKey();

...
dsa.privateKeyExportPKCS8 = function privateKeyExportPKCS8(key) {
  ...
  return asn1.encodePKCS8({
    ...
    algorithm: {
      oid: asn1.DSA_OID,
      type: asn1.SEQ,
      params: asn1.encodeParams(key.p, key.q, key.g)
    },
    key: asn1.encodeInt(key.x)
  });
};
...
dsa.privateKeyImportPKCS8 = function privateKeyImportPKCS8(raw) {
  ...
  assert(pki.algorithm.oid.equals(asn1.DSA_OID));
  assert(pki.algorithm.type === asn1.SEQ);

  const [p, q, g] = asn1.decodeParams(pki.algorithm.params);
  const x = asn1.decodeInt(pki.key);

  const key = new DSAPrivateKey(p, q, g, null, x);
...
dsa.privateKeyTweakAdd = function privateKeyTweakAdd(key, tweak) {
  ...
  const [y, x] = binding.privateKeyTweakAdd(
    key.p,
    key.q,
    key.g,
    key.y,
...
dsa.publicKeyTweakAdd = function publicKeyTweakAdd(key, tweak) {
  ...
  const y = binding.publicKeyTweakAdd(
    key.p,
    key.q,
    key.g,
    key.y,

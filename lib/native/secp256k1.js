...
const backend = require('bindings')('secp256k1');
...
/**
 * Define a getter with a prefix.
 * @param {String} key
 * @param {Function} get
 */

secp256k1._define = function _define(key, get) {
  const prop = `${this.id.toUpperCase()}${key}`;
  get(secp256k1, prop, get);
};
...
get(secp256k1, '_keys', () => {
  const self = secp256k1;

  if (!self._key)
    self._key = require('../internal/eckey')(self);
...
get(secp256k1, 'PublicKey', () => {
  return this._keys.ECPublicKey;
});
...
get(secp256k1, 'PrivateKey', () => {
  return this._keys.ECPrivateKey;
});
...
get(secp256k1, 'Signature', () => {
  ...
  if (!self._sig)
    self._sig = require('../internal/ecsig')(self);

  return self._sig;
});

/*
 * Prefixed key & signature classes.
 */

secp256k1._define('Key', () => secp256k1.Key);
secp256k1._define('PublicKey', () => secp256k1.PublicKey);
secp256k1._define('PrivateKey', () => secp256k1.PrivateKey);
secp256k1._define('Signature', () => secp256k1.Signature);
...
secp256k1._sign = function _sign(msg, key) {
  assert(Buffer.isBuffer(msg));
  assert(Buffer.isBuffer(key));
  assert(key.length === 32);

  // Sign message.
  const {signature, recovery} = backend.sign(msg, key);

  // Ensure low S value.
  const sig = backend.signatureNormalize(signature);
...
secp256k1.toRS = function toRS(sig) {
  assert(Buffer.isBuffer(sig));
...
secp256k1.isLowDER = function isLowDER(raw) {
  ...
  try {
    sig = backend.signatureImportLax(raw);
  } catch (e) {
    return false;
  }

...
function publicKeyConvert(key, sign = false) {
...
function publicKeyToUniform(pub, sign) {
  if (sign == null)
    sign = (random.randomInt() & 1) !== 0;

  return binding.publicKeyToUniform(pub, sign);
}
...
function derive(pub, key) {
  return binding.derive(pub, key);
}

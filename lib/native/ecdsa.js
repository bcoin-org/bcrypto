...
class ECDSA {
  ...
  get _handle() {
    ...
    if (!this._ctx) {
      ...
      this._ctx = binding.ecdsa_context_create(type);
  ...
  isLowS(sig) {
    ...
    return binding.ecdsa_signature_is_low_s(this._handle, sig);
  }
  ...
  isLowDER(sig) {
    ...
    return binding.ecdsa_signature_is_low_der(this._handle, sig);
  }

...
class ECDSA {
  constructor(name) {
    ...
    this._order = Buffer.from(ec.order, 'hex');
    this._half = Buffer.from(ec.half, 'hex');
  }
  ...
  privateKeyVerify(key) {
    ...
    if (key.equals(this._zero))
      return false;

    return key.compare(this._order) < 0;
  }
  ...
  privateKeyMod(key) {
    return binding.privateKeyMod(this.id, key);
  }
  ...
  signatureNormalize(raw) {
    ...
    if (sig.r.equals(z) || sig.r.compare(n) >= 0)
      throw new Error('Invalid R value.');

    if (sig.s.equals(z) || sig.s.compare(n) >= 0)
      throw new Error('Invalid S value.');

    if (sig.s.compare(nh) > 0) {
      const s = Buffer.from(sig.s);

      let carry = 0;

      for (let i = this.size - 1; i >= 0; i--) {
        const r = n[i] - s[i] + carry;
        carry = r >> 8;
        s[i] = r & 0xff;
      }
  ...
  isLowS(sig) {
    return Signature.isLowS(sig, this.size, this._half);
  }

  isLowDER(sig) {
    return Signature.isLowDER(sig, this.size, this._half);
  }

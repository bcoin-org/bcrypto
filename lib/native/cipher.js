...
class CipherWrap {
  constructor(name, encrypt) {
    ...
    const desc = ciphers.get(name);
    const type = backend.ciphers[desc.algorithm];
    const mode = backend.modes[desc.mode];

    if (hasCipher(type, mode))
      this.ctx = new CipherBase(type, mode, encrypt);
    else

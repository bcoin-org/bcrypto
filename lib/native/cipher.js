...
class CipherBase {
  ...
  update(data) {
    ...
    const [out, len] = binding.cipher_update(this._handle, data);
...
function encrypt(name, key, iv, data) {
  ...
  const [type, mode] = parseName(name);
  const [out, len] = binding.cipher_encrypt(type, mode, key, iv, data);
...
function decrypt(name, key, iv, data) {
  ...
  const [type, mode] = parseName(name);
  const [out, len] = binding.cipher_decrypt(type, mode, key, iv, data);

...
class ShortCurve extends Curve {
  ...
  _isomorphism(curve) {
    ...
    const x = curve.g.x.forceRed(this.red);
  ...
  pointFromShort(point) {
    ...
    return this.point(x.redMul(u2), y.redMul(u3));
  }
...
class MontCurve extends Curve {
  ...
  _mont(b0) {
    ...
    //
    // E(A,B)->E(A,B') transformation
    // is valid when (B / B') is square.
    assert(b0 instanceof BN);
  ...
  _isomorphism(curve) {
    ...
    if (this.g.isInfinity() || curve.g.isInfinity()) {
      const b = curve.b.forceRed(this.red);
      return b.redDivSqrt(this.b);
    }

    const y = curve.g.y.forceRed(this.red);

    return this.g.y.redDiv(y);
  }
  ...
  pointFromMont(point) {
    ...
    return this.point(x, c.redMul(y));
  }
...
class EdwardsCurve extends Curve {
  ...
  _isomorphism(curve) {
    ...
    if (this.g.isInfinity() || curve.g.isInfinity()) {
      const a = curve.a.forceRed(this.red);
      return a.redDivSqrt(this.a);
    }

    const x = curve.g.x.forceRed(this.red);

    return this.g.x.redDiv(x);
  }
  ...
  isIsomorphic(curve, invert = false) {
    ...
    if (curve.type === 'edwards') {
      // a' * d = a * d'
      const a = curve.a.forceRed(this.red);
      const d = curve.d.forceRed(this.red);
  ...
  pointFromEdwards(point) {
    ...
    const y = point.y.forceRed(this.red);
    const z = point.z.forceRed(this.red);
    const t = point.t.forceRed(this.red);

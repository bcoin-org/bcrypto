...
class ShortPoint extends Point {
  ...
  recover(x1, z1, x2, z2) {
    ...
    // X0Z = X0 * Z
    const x0z = this.x.redMul(z);

    // Y0Z = Y0 * Z
    const y0z = this.y.redMul(z);

    // X1Z = X1 * Z2
    const x1z = x1.redMul(z2);

    // X2Z = X2 * Z1
    const x2z = x2.redMul(z1);

    // A = 2 * b * ZZZ
    const a = this.curve.b.redMul(zzz).redIMuln(2);

    // B = a * ZZ + X0Z * X1Z
    const b = x0z.redMul(x1z).redIAdd(this.curve.a.redMul(zz));

    // C = X0Z + X1Z
    const c = x0z.redAdd(x1z);

    // D = A + B * C
    const d = a.redIAdd(b.redMul(c));

    // E = X2Z * (X0Z - X1Z)^2
    const e = x2z.redMul(x0z.redSub(x1z).redSqr());

    // F = D - E
    const f = d.redISub(e);

    // G = 2 * Y0Z * ZZ
    const g = y0z.redMul(zz).redIMuln(2);

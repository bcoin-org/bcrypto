...
class ShortCurve extends Curve {
  ...
  pointFromEdwards(point) {
    ...
    const x = point.z.redAdd(point.y);
    const z = point.z.redSub(point.y);
    const nx = x.redMul(z.redInvert()).redIAdd(a3);
    const ny = c.redMul(x).redMul(z.redMul(point.x).redInvert());

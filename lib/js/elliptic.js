...
class Curve {
  ...
  _init(Point, type, conf) {
    ...
    assert(conf.magic == null || Array.isArray(conf.magic));
    ...
    if (conf.magic) {
      for (const c of conf.magic)
        this.magic.push(BN.fromJSON(c).toRed(this.red));
    }
...
class EdwardsCurve extends Curve {
  ...
  isIsogenous(curve) {
    ...
    if (curve.type === 'edwards') {
      ...
      if (self.mOneA || !curve.mOneA)
        return false;

      return curve.d.cmp(self.d.redSub(self.one)) === 0;
    }
  ...
  pointFromEdwards(point) {
    ...
    // Ed448 <-> Twisted Ed448.
    if (this.isIsogenous(point.curve)) {
      // 4-isogeny maps for E(1,d)<->E(-1,d-1):
      //
      //   x = (2 * x * y) / (2 - y^2 - a * x^2)
      ...
      const nt = this.extended ? xx.redMul(yy) : null;
      const p = this.point(nx, ny, nz, nt);

      return this.twisted ? p : p.divH();
    }
...
class SECP256K1 extends ShortCurve {
  ...
  pointFromShort(point) {
    assert(point instanceof ShortPoint);
    assert(point.curve.id === 'ISO256K1');
    ...
    // 3-isogeny maps:
    //
    //   x' = (c1 * x^3 + c2 * x^2 + c3 * x + c4) / (x^2 + c5 * x + c6)
    //   y' = (c7 * x^3 * y + c8 * x^2 * y + c9 * x * y + c10 * y) /
    //        (x^3 + c11 * x^2 + c12 * x + c13)
    //
    // No exceptional cases that I'm aware of.
    const [c1, c2, c3, c4, c5, c6, c7, c8,
           c9, c10, c11, c12, c13, c14] = this.magic;
    const x = point.x.forceRed(this.red);
    const y = point.y.forceRed(this.red);
    const x2 = x.redSqr();
    const x3 = x2.redMul(x);
    const t0 = c1.redMul(x3);
    const t1 = c2.redMul(x2);
    const t2 = c3.redMul(x);
    const t3 = c5.redMul(x);
    const t4 = c7.redMul(x3).redMul(y);
    const t5 = c8.redMul(x2).redMul(y);
    const t6 = c9.redMul(x).redMul(y);
    const t7 = c10.redMul(y);
    const t8 = c11.redMul(x2);
    const t9 = c12.redMul(x);
    const n0 = t0.redIAdd(t1).redIAdd(t2).redIAdd(c4);
    const d0 = x2.redAdd(t3).redIAdd(c6);
    const n1 = t4.redIAdd(t5).redIAdd(t6).redIAdd(t7);
    const d1 = x3.redAdd(t8).redIAdd(t9).redIAdd(c13);
    const d = d0.redMul(d1).redInvert();
    const nx = n0.redMul(d.redMul(d1));
    const ny = n1.redMul(d.redMul(d0));
    const p = this.point(nx, ny);

    return p.mul(c14.fromRed());
  }
}

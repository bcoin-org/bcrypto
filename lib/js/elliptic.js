...
class ShortCurve extends Curve {
  ...
  _svdwi(p, hint) {
    ...
    //   U = [un is a root and svdw(un) = x, ...]
    //   r = random integer in [0..4]
    //   u = none, if U[r] = 0
    //     = sign(y) * abs(ur), otherwise
    const {z, i2} = this;
    const e = this.p.subn(2);
    const z2 = z.redSqr();
    ...
    // SVDW(u) with uniformally random field
    // elements picks (x1@50%, x2@25%, x3@25%).
    // To mirror this, the inverse should simply
    // pick (u1@50%, u2@25%, u3@12.5%, u4@12.5%).
    const U = [
      s1 & x1.ceq(x),
      s2 & x2.ceq(x),
      s0 & s3 & x3.ceq(x),
  ...
  pointToHash(p, rng) {
    ...
    for (;;) {
      ...
      const p1 = this.pointFromUniform(u1);
      const p2 = p.add(p1.neg());

      let u2;
      try {
        u2 = this.pointToUniform(p2, rng.randomInt());
      } catch (e) {

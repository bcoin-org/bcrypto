 ...
 *
 *   [MONT4] Elliptic Curve Arithmetic for Cryptography
...
class Curve {
  ...
  _init(Point, type, conf) {
    ...
    // Memoize.
    this._isomorphism = memoize(this._isomorphism, this);
...
class ShortCurve extends Curve {
  ...
  _short(a0, odd) {
    ...
    assert(a0 instanceof BN);
    assert(odd == null || typeof odd === 'boolean');
    assert(!this.a.isZero() && !this.b.isZero());

    const u4 = a0.redDiv(this.a);
    const u2 = u4.redSqrt();
    ...
    const a = this.a.redMul(u4);
    const b = this.b.redMul(u6);
  ...
  _isomorphism(curve) {
    ...
    if (this.g.isInfinity() || curve.g.isInfinity()) {
      ...
      const u4 = this.a.redDiv(a);
      const u6 = this.b.redDiv(b);
      const u2 = u4.redSqrt();
    ...
    const x = this.field(curve.g.x);
    const y = this.field(curve.g.y);
    const u2 = this.g.x.redDiv(x);
    const u3 = this.g.y.redDiv(y);
  ...
  _edwards(hint) {
    if (hint instanceof BN)
      return this._edwards1(hint);

    return this._edwards2(hint);
  }

  _edwards1(a0) {
    // Twisted Edwards Equivalence.
    //
    // Transformation:
    //
    //   a' = -(7 * d') +- 4 * sqrt(3 * (d'^2 - a))
    //   d' = -(7 * a') +- 4 * sqrt(3 * (a'^2 - a))
    //
    // To pick the correct square root, we
    // can try to compute the 2-torsion point:
    //
    ...
    const a = a0.clone();
    const a2 = a.redSqr();
    const a7 = a.redMuln(-7);
    const c = a2.redSub(this.a).redIMuln(3).redSqrt().redIMuln(4);
    const d0 = a7.redAdd(c);
    const d1 = a7.redSub(c);
    const x0 = a.redAdd(d0).redMul(this.i6);
    const x1 = a.redAdd(d1).redMul(this.i6);
    const y0 = this.solveY2(x0);
    const y1 = this.solveY2(x1);
    const d = y0.isZero() ? d0 : d1;
  ...
  _icart(u0) {
    ...
    const v = a.redMuln(3).redISub(u4).redMul(iu6);
    const v2 = v.redSqr();
    const u6d27 = u6.redDivn(27);
  ...
  _sswu(u) {
    ...
    const {a, b, z, one} = this;
    const e = this.p.subn(2);
    const z2 = z.redSqr();
    const ba = b.redNeg().redDiv(a);
    const bza = b.redDiv(z.redMul(a));
    const u2 = u.redSqr();
    const u4 = u2.redSqr();
  ...
  _svdwf(u) {
    ...
    const x3 = z.redSub(t4.redMul(t2).redMul(i3));
    const y1 = this.solveY2(x1);
    const y2 = this.solveY2(x2);
  ...
  _svdwi(p, hint) {
    ...
    //   u3 = (3 * (-x * z^2 + z^3) - 2 * g(z) + t4) / 2
    //   u4 = (3 * (-x * z^2 + z^3) - 2 * g(z) - t4) / 2
    //   r = random integer in [1..4]
    //   u = sign(y) * abs(sqrt(ur))
    const {z, two} = this;
    const z2 = z.redSqr();
    ...
    const t5 = xz2.redIAdd(z3).redIMuln(3).redISub(gz2);
  ...
  isIsomorphic(curve) {
    ...
    if (curve.type === 'short') {
      // a' = a * u^4
      // b' = b * u^6
      const jinv = this.jinv();

      if (jinv.isZero())
        return false;

      return jinv.eq(curve.jinv());
    }

    if (curve.type === 'mont') {
      // (A / (3 * B))^3 + a * (A / (3 * B)) + b = 0
      const {a3, bi} = curve;
      const x = this.field(a3.redMul(bi));
      const y2 = this.solveY2(x);
  ...
  pointFromShort(point) {
    ...
    if (this.isIsomorphic(point.curve)) {
      ...
      const [u2, u3] = this._isomorphism(point.curve);
  ...
  pointFromMont(point) {
    ...
    if (this.isIsomorphic(point.curve)) {
      ...
      return this.point(this.field(nx),
                        this.field(ny));
    }
  ...
  pointFromEdwards(point) {
    ...
    if (this.isIsomorphic(point.curve)) {
      ...
      if (point.x.isZero())
        return this.point(this.field(ad6), this.zero);
      ...
      return this.cpoint(this.field(xx),
                         this.field(xz),
                         this.field(yy),
                         this.field(yz));
    }
  ...
  toShort(a0, odd, sign = null) {
    ...
    if (sign != null) {
      const [, u3] = curve._isomorphism(this);
...
class JPoint extends Point {
  ...
  _uadd(p) {
    ...
    const inf3 = nz.czero() & (inf1 ^ 1) & (inf2 ^ 1);
    ...
    // If either point is infinity, the
    // new point should also be infinity.
    assert(((inf1 & inf2) ^ 1) | nz.czero());
  ...
  ztrplu() {
    ...
    // [COZ] Algorithm 22, Page 16, Appendix C.
    // 6M + 7S + 15A + 4*2 + 1*8
...
class MontCurve extends Curve {
  ...
  _edwards(a0, invert) {
    ...
    if (a0) {
      const a = a0.clone();
      const b = ap2.redDiv(a);
      const d = am2.redDiv(b);
  ...
  _scale(curve, invert) {
    ...
    return this.field(curve._scale(this, invert));
  }
  ...
  _elligator2(u) {
    ...
    //   - Let A and B be elements of F(p)
    //     such that A * B * (A^2 - 4 * B) != 0.
    //   - Let z be a non-square in F(p).
    //   - y^2 = x^3 + A * x^2 + B * x.
    //   - z * u^2 != -1.
    //
  ...
  pointFromMont(point) {
    ...
    if (this.isIsomorphic(point.curve)) {
      ...
      const c = this._isomorphism(point.curve);
  ...
  pointFromEdwards(point) {
    ...
    if (this.isIsomorphic(point.curve, true)) {
      // Rational maps for E(a,d)->E(d,a)->M(-A,-B):
      //
      //   u = (y + 1) / (y - 1)
      //   v = (2 / +-sqrt(B * (d - a))) * u / x
      //
  ...
  toMont(b0, sign = null) {
    ...
    if (sign != null) {
      const c = curve._isomorphism(this);
  ...
  toEdwards(a0, invert = false, sign = null) {
...
class EdwardsCurve extends Curve {
  ...
  _mont(b0, invert = false) {
    ...
    // equivalence between E(d,a) and M(-A,-B).
    //
    ...
    // for saner mapping to Curve448.
    //
    // Another way to put it is that IsoEd448
    // is _not_ equivalent to Curve448, it is
    // isomorphic to an Edwards curve E(d,a).
    // The _latter_ is equivalent to Curve448.
  ...
  _edwards(a0) {
    ...
    //   d' = a' * d / a
    //
    // Where jacobi(4 / (a - d)) == jacobi(4 / (a' - d')).
    assert(a0 instanceof BN);

    const a = a0.clone();
    const d = a0.redMul(this.d).redDiv(this.a);
    const b0 = this.four.redDiv(this.a.redSub(this.d));
    const b1 = this.four.redDiv(a.redSub(d));

    if (b0.redJacobi() !== b1.redJacobi())
      throw new Error('Invalid `a` coefficient.');
  ...
  _scale(curve, invert) {
    ...
    if (this.g.isInfinity() || curve.g.isInfinity()) {
      ...
      return b.redDivSqrt(b0).redInvert();
    }

    const {x, y, z} = this.g;
    const v = this.field(curve.g.y);
    ...
    const u0 = uu.redDiv(uz);
    const v0 = u0.redDiv(x);
  ...
  _elligator1(t) {
    ...
    const v0 = r2.redSub(two).redMul(u3);
    const v = u5.redAdd(v0).redIAdd(u);
    ...
    const rX = r.redMul(X);
    const y0 = rX.redSub(Xp2);
    const y1 = rX.redAdd(Xp2);
    const y = y0.redMul(y1.redPow(e0));
  ...
  _invert1(p, hint) {
    ...
    const e2 = this.p.addn(1).iushrn(2);
    const [x, y] = p.getXY();
    const n0 = y.redSub(one);
    const n1 = y.redAdd(one).redIMuln(2);
    const n = n0.redMul(n1.redPow(e0));
    const nr = n.redMul(r);
    const nr1 = one.redAdd(nr);
    const Xr2 = nr1.redSqr().redISub(one);
    const Xl = nr1.redNeg();
    const Xr = Xr2.redPow(e2);
    const sqr = Xr.redSqr().ceq(Xr2);
    const X = Xl.redAdd(Xr);
    const z0 = c.redSub(one);
    const z1 = one.redAdd(X);
    const z2 = X.redSqr().redIAdd(c2.redInvert());
    const z3 = z0.redMul(s).redMul(X).redMul(z1);
    const z4 = z3.redMul(x).redMul(z2);
    const z = z4.redPow(e1);
    const u = z.redMul(X);
    const t0 = one.redSub(u);
    const t1 = one.redAdd(u);
    const t = t0.redMul(t1.redPow(e0));

    t.cinject(t.redNeg(), t.redIsOdd() ^ (hint & 1));
  ...
  pointFromShort(point) {
    ...
    if (this.isIsomorphic(point.curve)) {
      ...
      //
      // Undefined for y = 0 or x = (5 * d - a) / 12.
      //
      // Exceptional Cases:
      //   - O -> (0, 1)
      //   - ((a + d) / 6, 0) -> (0, -1)
  ...
  pointFromMont(point) {
    ...
    if (this.isIsomorphic(point.curve, true)) {
      // Rational maps for M(-A,-B)->E(d,a)->E(a,d):
      //
      //   x = (2 / +-sqrt(B * (d - a))) * u / v
      //   y = (u + 1) / (u - 1)
      //
  ...
  toMont(b0, invert = false, sign = null) {

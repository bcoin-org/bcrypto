...
class MontCurve extends Curve {
  ...
  _invert2(p) {
    ...
    const n = p.x.clone();
    const d = p.x.redAdd(this.a);
    const lt = clt(p.y, fq2, this.fieldSize);
    ...
    u.cinject(u.redNeg(), u.redIsOdd() ^ p.y.redIsOdd());
...
class EdwardsCurve extends Curve {
  ...
  pointFromUniform(curve, u) {
    ...
    const m = curve.pointFromUniform(f);

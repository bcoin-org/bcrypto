...
class MontCurve extends Curve {
  ...
  pointFromEdwards(point) {
    ...
    if (this.hasIsogeny(point.curve)) {
      // 4-isogeny maps:
      //
      //   u = y^2 / x^2
      //   v = (2 - x^2 - y^2) * y / x^3
...
class EdwardsCurve extends Curve {
  constructor(conf) {
    ...
    this.context = Boolean(conf.context);
    this.invert = Boolean(conf.invert);
  ...
  pointFromMont(point) {
    ...
    // Check for a possible 4-isogeny.
    const iso4 = this.hasIsogeny(point.curve);
  ...
  toJSON(pre) {
    ...
    json.prefix = this.prefix.length > 0

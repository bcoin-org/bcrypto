...
class Curve {
  ...
  _init(type, conf) {
    ...
    this.hi = this.n.isZero() ? new BN(1) : this.h.invert(this.n);
...
class MontCurve extends Curve {
  ...
  pointFromEdwards(point) {
    ...
    if (point.curve.twisted) {
      ...
      // Edwards curve with d = 39082 / 39081.
      //
      // Birational maps:
      //
      //   u = (y - 1) / (y + 1)
      ...
      //
      // Undefined for x = 0 or y = -1.
      nx = point.y.redSub(point.z);
      nz = point.y.redAdd(point.z);
    }
  ...
  elligator2(r) {
    ...
    const f = v3.redAdd(this.a.redMul(v2)).redIAdd(this.b.redMul(v));
    ...
    const y = e.redNeg().redMul(this.solveY(x));

    return [this.point(x), y.redIsOdd()];
  }
  ...
  invert2(p, sign) {
    ...
    return lhs.redMul(rhs.redInvert()).redSqrt();
  }
...
class EdwardsCurve extends Curve {
  ...
  _solveMod43(y) {
    ...
    //
    // Note that `p` is congruent to mod 4 == 3.
    //
  ...
  _solveMod85(y) {
    ...
    //
    // Note that `p` is congruent to mod 8 == 5.
    //
  ...
  solveX(y) {
    ...
    if (this.p.andln(3) === 3)
      return this._solveMod43(y);

    // p mod 8 == 5 (p25519)
    if (this.p.andln(7) === 5)
      return this._solveMod85(y);
  ...
  pointFromMont(point, sign) {
    ...
    if (this.twisted) {
      ...
      //
      // Undefined for u = 1 or v = 0.
      const y = point.z.redAdd(point.x);
  ...
  elligator2(curve, r) {
    ...
    const [m, sign] = curve.elligator2(u);

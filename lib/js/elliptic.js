...
class MontCurve extends Curve {
  ...
  _invert2(p, hint) {
    ...
    u.cinject(u.redNeg(), u.redIsNeg() ^ y0.redIsNeg());
...
class EdwardsCurve extends Curve {
  ...
  pointFromEdwards(point) {
    // https://git.zx2c4.com/goldilocks/tree/src/per_curve/decaf.tmpl.c#n1056
    assert(point instanceof EdwardsPoint);
    ...
    // Twisted Ed448 -> Ed448.
    if (this.iso4 && point.curve.twisted) {
      // 4-isogeny maps:
      //
      //   x = (2 * x * y) / (y^2 + x^2)
      //   y = (y^2 - x^2) / (2 * z^2 - y^2 + x^2)
      //
      // Rewritten as (denominators swapped):
      //
      //   x = ((y + x)^2 - (y^2 + x^2)) / (2 - (y^2 - x^2))
      //   y = (y^2 - x^2) / (y^2 + x^2)
      //
      // Undefined for y^2 - x^2 = 2 and y^2 + x^2 = 0.
      const x2 = x.redSqr();
      const y2 = y.redSqr();
      const yy = y2.redSub(x2);
      const yz = y2.redAdd(x2);
      const xx = y.redAdd(x).redSqr().redISub(yz);
      const xz = z.redSqr().redIMuln(2).redISub(yy);
      const nx = xx.redMul(xz);
      const ny = yy.redMul(yz);
      const nz = xz.redMul(yz);

      return this.point(nx, ny, nz).divH();
    }

    // Ed448 -> Twisted Ed448.
    if (this.twisted && point.curve.iso4) {
      // 4-isogeny maps:
      //
      //   x = (2 * x * y) / (y^2 - a * x^2)
      ...
      const x2 = x.redSqr();
      const y2 = y.redSqr();
      const yy = y2.redAdd(x2);
      const yz = y2.redSub(x2);
      const xx = y.redAdd(x).redSqr().redISub(yy);
      const xz = z.redSqr().redIMuln(2).redISub(yy);
      const nx = xx.redMul(xz);
      const ny = yy.redMul(yz);
      const nz = xz.redMul(yz);
...
class EdwardsPoint extends Point {
  ...
  _addExt(p, flag) {
    // a = -1
    if (this.mOneA)
      return this._addExtM1(p, flag);

    return this._addExtA(p, flag);
  }
  ...
  _addProj(p, flag) {
    ...
    // 10M + 1S + 7A + 1*c + 1*d
    //
    // https://hyperelliptic.org/EFD/g1p/auto-edwards-projective.html#addition-madd-2007-bl
    // 9M + 1S + 7A + 1*c + 1*d
    const zOne = p.zOne & (flag ^ 1);
    ...
    // Y3 = A * G * (D - C)
    const ny = a.redMul(g).redMul(d.redISub(c));

    // Z3 = c * F * G
    const nz = this.curve._mulC(f).redMul(g);
  ...
  _dblProj(flag) {
    ...
    // X3 = c * (B - E) * J
    const nx = this.curve._mulC(b.redISub(e)).redMul(j);

    // Y3 = c * E * (C - D)
    const ny = this.curve._mulC(e).redMul(c.redISub(d));
  ...
  _trplProj(flag) {
    ...
    // X3 = P * U * X1
    const nx = p.redMul(u).redMul(this.x);

    // Y3 = Q * T * Y1
    const ny = q.redMul(t).redMul(this.y);

    // Z3 = T * U * Z1
    const nz = zOne ? t.redMul(u) : t.redMul(u).redMul(this.z);

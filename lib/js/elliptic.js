...
class ShortCurve extends Curve {
  ...
  pointFromMont(point) {
    ...
    const {a, bi, i3} = point.curve;
    const nx = point.x.redMuln(3).redIAdd(a).redMul(i3).redMul(bi);
  ...
  pointFromEdwards(point) {
    ...
    const a3 = a.redMul(point.curve.i3);
...
class MontCurve extends Curve {
  ...
  pointFromShort(point) {
    ...
    const u = b.redMul(x).redIMuln(3).redISub(a).redMul(i3);
    const v = b.redMul(y);
...
class EdwardsPoint extends Point {
  ...
  _check() {
    ...
    // addition formulas, the curve
    // must satisfy these parameters:
    //
    //   Regular: d = QNR
    //   Twisted: a = QR, d = QNR
    //

...
class ShortCurve extends Curve {
  ...
  _sswu(u) {
    ...
    const beta = this.solveY2(x2).redLegendre();

    x1.cinject(x2, -beta >>> 31);

    return this.pointFromX(x1, u.redIsOdd());
  }
  ...
  _svdw(u) {
    ...
    const alpha = this.solveY2(x1).redLegendre();
    const beta = this.solveY2(x2).redLegendre();
    const i = mod((alpha - 1) * beta, 3);
    const x = [x1, x2, x3][i];

    return this.pointFromX(x, u.redIsOdd());
  }

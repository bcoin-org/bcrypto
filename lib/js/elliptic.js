...
class EdwardsCurve extends Curve {
  ...
  _montA() {
    ...
    //   B = 1 (assumed)
    //
    // Assumes jacobi(B) = jacobi(4 / (a - d)).
    const apd = this.a.redAdd(this.d);
    const amd = this.a.redSub(this.d);

    return apd.redMul(amd.redInvert()).redIMuln(2);
  }
...
class EdwardsPoint extends Point {
  ...
  static decode(curve, bytes) {
    ...
    // implementation.
    //
    // Hamburg rejects infinity (0, 1),
    // however, the torsion point (0, -1)
    // is also treated as it were infinity
    // (i.e. -1^2 = 1).
    //
    // We artificially reject (0, -1) here
    // for this reason.
    if (curve.iso4) {
      if (p.y.eq(p.z) || p.y.eq(p.z.redNeg()))
        throw new Error('Invalid point.');
    }

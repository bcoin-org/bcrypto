...
class ShortCurve extends Curve {
  ...
  pointFromMont(point, sign) {
    ...
    const ny = point.getY(sign);

    return this.point(nx.fromRed(), ny);
  }
  ...
  pointFromEdwards(point) {
    ...
    const i3 = point.curve.three.redInvert();
    const [a] = point.curve._untwist();
    const a3 = a.redMul(i3);
    const c = a.redAdd(point.curve.two).redINeg().redSqrt();
...
class MontCurve extends Curve {
  ...
  _elligator2(u) {
    ...
    const x2 = x1.redNeg().redISub(this.a);
    const y1 = this.solveY2(x1);
    const i = y1.redLegendre() >>> 31;
    const x = [x1, x2][i];

    return [this.point(x), u.redIsOdd()];
  }
  ...
  _invert2(p, sign) {
    ...
    const fq2 = this.p.ushrn(1);
    const [x, y] = p.getXY(sign);
    const n = x;
    const d = x.redAdd(this.a);
    const lt = clt(y, fq2, this.fieldSize);
    ...
    u.cinject(u.redNeg(), u.redIsOdd() ^ sign);
  ...
  pointFromShort(point) {
    ...
    //   https://tools.ietf.org/id/draft-ietf-lwig-curve-representations-02.html#rfc.appendix.E.2
    assert(point instanceof Point);
    assert(point.curve instanceof ShortCurve);
    assert(point.curve.p.eq(this.p));
    ...
    const x = point.x.fromRed().toRed(this.red);
    const nx = x.redISub(a3);

    return this.point(nx);
  }
  ...
  pointFromEdwards(point) {
    ...
    if (point.curve.twisted) {
      ...
      vv = point.curve._scale(this).redMul(uu);
      vz = point.x.redMul(uz);
      ...
      uu = y2;
      uz = x2;
      vv = two.redSub(x2).redISub(y2).redMul(y);
      vz = x3;
    } else {
      ...
      //
      // Undefined for x = 0 or y = 1.
      uu = point.y.redNeg().redISub(point.z);
      uz = point.y.redNeg().redIAdd(point.z);
      vv = point.curve._scale(this).redMul(uu);
      vz = point.x.redMul(uz);
    }
    ...
    p.x = uu.redMul(vz).forceRed(this.red);
    p.y = vv.redMul(uz).forceRed(this.red);
    p.z = uz.redMul(vz).forceRed(this.red);
  ...
  pointFromUniform(u) {
    ...
    // b != 1
    if (this.b.cmp(this.one) !== 0)
      throw new Error('Not implemented.');

    return this._elligator2(u);
  }

  pointToUniform(p, sign) {
    ...
    return this._invert2(p, sign);
  }

  pointFromHash(curve, bytes) {
    assert(curve == null || (curve instanceof EdwardsCurve));
    assert(Buffer.isBuffer(bytes));

    if (curve) {
      const p = curve.pointFromHash(this, bytes);
      return this.pointFromEdwards(p);
    }
    ...
    const u1 = this.decodeUniform(s1);
    const u2 = this.decodeUniform(s2);
    const [p1, sign1] = this.pointFromUniform(u1);
    const [p2, sign2] = this.pointFromUniform(u2);

    // Add in the twisted edwards space.
    const [A, D] = this._twist();
    const [x1, y1, z1] = p1._twist(sign1);
    const [x2, y2, z2] = p2._twist(sign2);
    ...
    const f = b.redSub(e);
    const g = b.redIAdd(e);
    const y3 = a.redMul(g).redMul(d.redISub(A.redMul(c)));
    const z3 = f.redMul(g);
    const u = z3.redAdd(y3);
    const z = z3.redSub(y3);

    return this.point(u, z).mulH();
  }
...
class EdwardsCurve extends Curve {
  ...
  _scale(curve) {
    assert(!this.g.isInfinity());
    assert(curve instanceof MontCurve);
    assert(curve.g.y != null);
    ...
    if (!this._factor) {
      ...
      this._factor = y.redMul(v.redInvert());
    }
  ...
  _elligator1(t) {
    ...
    const f1 = u2.redAdd(c2.redPow(e0)).redPow(e1);
    const X = f0.redMul(u);
  ...
  _invert1(p) {
    ...
    const x0 = xl.redMul(xr).redMul(r.redPow(e0));
    const nr = n.redMul(r);
    ...
    const z2 = X.redSqr().redIAdd(c2.redPow(e0));
  ...
  pointFromShort(point) {
    ...
    const x = point.x.fromRed().toRed(this.red);
    const y = point.y.fromRed().toRed(this.red);
    const [a] = this._untwist();
    const c = a.redAdd(this.two).redINeg().redSqrt();
  ...
  pointFromMont(point, sign) {
    ...
    if (this.twisted) {
      ...
      const c = this._scale(curve).forceRed(red);
      const [u, v] = point.getXY(sign);
      ...
      const u5 = u3.redMul(u2);
      const v2 = v.redSqr();
      const a = v.redIMuln(4);
      const b = u2.redSub(one);
      ...
      xx = a.redMul(b);
      xz = u4.redISub(c).redIAdd(d).redIAdd(one);
      ...
      xx = c.redMul(u);
      xz = v;
      yy = one.redAdd(u).redINeg();
      yz = one.redSub(u);
    }

    // Edwards point.
    const x = xx.redMul(yz).forceRed(this.red);
    const y = yy.redMul(xz).forceRed(this.red);
    const z = xz.redMul(yz).forceRed(this.red);
  ...
  pointFromUniform(curve, u) {
    ...
    const f = u.fromRed().toRed(curve.red);
    const [m, sign] = curve.pointFromUniform(f);
  ...
  pointToUniform(curve, p) {
    ...
    const sign = m.normalize().y.redIsOdd();
    const f = curve.pointToUniform(m, sign);
    const u = f.fromRed().toRed(this.red);
  ...
  pointFromHash(curve, bytes) {
    ...
    const u1 = this.decodeUniform(s1);
    const u2 = this.decodeUniform(s2);
    const p1 = this.pointFromUniform(curve, u1);
    const p2 = this.pointFromUniform(curve, u2);
...
class EdwardsPoint extends Point {
  ...
  _untwist() {
    ...
    assert(this.curve.twisted);

    const x = this.x.redMul(this.z.redInvert());
    const zpy = this.z.redAdd(this.y);
    const zmy = this.z.redSub(this.y);
    const u = zpy.redMul(x);
    const v = zpy;
    const z = zmy.redMul(x);
...
function clt(u, v, size) {
  assert(u instanceof BN);
  assert(v instanceof BN);
  assert(!v.red);

  const a = u.fromRed().encode('le', size);
  const b = v.encode('le', size);

  let eq = ~0;
  let lt = 0;

  // Check `a < b` in constant time.

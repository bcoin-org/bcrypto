...
class Curve {
  ...
  _init(type, conf) {
    ...
    // Sanity checks.
    assert(!this.p.isZero() && this.p.isOdd());
    assert(!this.h.isZero() && this.h.cmpn(255) <= 0);
...
class ShortCurve extends Curve {
  ...
  _icart(u0) {
    ...
    //
    // Other Resources:
    //   https://github.com/cfrg/draft-irtf-cfrg-hash-to-curve/blob/7e7ccaf/draft-irtf-cfrg-hash-to-curve.md
    //   #icart-method-icart
    //
    // Assumptions:
    //
    ...
    const v2 = v.redSqr();
    const u6d27 = u6.redMul(this.three.redMuln(9).redInvert());
    const u2d3 = u2.redMul(this.three.redInvert());
  ...
  _sswu(u) {
    // Simpler variant of the Shallue-Woestijne-Ulas (SWU) algorithm.
    //
    ...
    // Assumptions:
    //
    //   - Let z be a non-square in F(q).
    //   - g(b / (z * a)) is square in F(q).
    //   - a, b != 0.
    //
    // Map:
    //
    //   g(x) = x^3 + a * x + b
    //   x2(t) = (-b / a) * (1 + (1 / (t^4 - t^2)))
    //   x3(t) = -t^2 * x2(t)
    //   u(t) = t^3 * g(x2(t))
    //   u(t)^2 = -g(x2(t)) * g(x3(t))
    //
    ...
    y.cinject(y.redNeg(), y.redIsNeg() ^ u.redIsNeg());
  ...
  _svdw(u) {
    ...
    // Assumptions:
    //
    //   - Let r1, r2, r3 be random elements in F(q).
    //   - Let z be a unique element in F(q).
    //   - g((sqrt(-3 * z^2) - z) / 2) is square in F(q).
    //   - t != 0.
    //   - t^2 != -1 - b.
    //   - q = 1 mod 3.
    //   - b != 0.
    ...
    // Map:
    //
    //   x1 = v = (-1 + sqrt(-3)) / 2 - (sqrt(-3) * t^2) / (1 + b + t^2)
    //   x2 = -1 - v = (-1 - sqrt(-3)) / 2 + (sqrt(-3) * t^2) / (1 + b + t^2)
    //   x3 = 1 + y^2 = 1 - (1 + b + t^2)^2 / (3 * t^2)
    //
    // Procedure:
    //
    //   f(a) = a^((q - 1) / 2)
    ...
    const e = this.p.subn(2);
    const z2 = this.z.redSqr();
    const m3z2s = this.three.redNeg().redMul(z2).redSqrt();
    const i3z2 = z2.redMuln(3).redInvert();
    const u2 = u.redSqr();
    const u4 = u2.redSqr();
    const gz = this.solveY2(this.z);
    const t1 = u2.redAdd(gz);
    const t2 = u2.redMul(t1).redPow(e);
    const t3 = u4.redMul(t2).redMul(m3z2s);
    const t13 = t1.redSqr().redMul(t1);
    const x1 = m3z2s.redSub(this.z).redMul(this.i2).redISub(t3);
    const x2 = t3.redSub(m3z2s.redAdd(this.z).redMul(this.i2));
    const x3 = this.z.redSub(t13.redMul(t2).redMul(i3z2));
    const y1 = this.solveY2(x1);
    const y2 = this.solveY2(x2);
  ...
  _svdwf(s0) {
    // Shallue-van de Woestijne (covert ECDH version).
    //
    // See: Covert ECDH over secp256k1.
    //   Pieter Wuille.
    ...
    //   - p = 3 (mod 4).
    //   - a = 0, b != 0.
    //   - g((sqrt(-3) - 1) / 2) is square in F(q).
    //   - If s is a square, the first of [f1(s), f2(s), f3(s)] which
    //     is a valid x coordinate on the curve, and the corresponding
    //     y coordinate that is square.
    //   - If s is not a square, the first of [f1(-s), f2(-s), f3(-s)]
    //     which is a valid x coordinate on the curve, and the
    //     corresponding y coordinate that is not square.
    //
    // Map:
    //
    //   c = sqrt(-3)
    ...
    //     = -y', otherwise
    const s = s0.clone();
    const quad = s.redIsSquare();

    s.cinject(s.redNeg(), quad ^ 1);

    const e = this.p.subn(2);
    const c = this.three.redNeg().redSqrt();
    const t0 = this.one.redAdd(this.b).redIAdd(s);
    const t1 = c.redMul(s).redMul(t0.redPow(e));
    const x1 = c.redSub(this.one).redMul(this.i2).redISub(t1);
    const x2 = this.one.redNeg().redISub(x1);
    const x3 = this.one.redSub(t0.redSqr().redMul(s.redMuln(3).redPow(e)));
    const y1 = this.solveY2(x1);
    const y2 = this.solveY2(x2);
    const y3 = this.solveY2(x3);
    const alpha = y1.redLegendre() | 1;
  ...
  _svdw2(s) {
    const [x, yy, quad] = this._svdwf(s);
    const y = yy.redSqrt();

    // Note that `y` is always a square
    // at this point (for p = 3 mod 4).
    y.cinject(y.redNeg(), quad ^ 1);
  ...
  _invert2(p, rng) {
    ...
    //
    //   c = sqrt(-3)
    //   t(x) = (6(2b - 1)x + 9(x^2) - 12b - 3)^(1/2) / 2
    //   f1^-1(x) = (b + 1)(c - 2x - 1)/(c + 2x + 1)
    //   f2^-1(x) = (b + 1)(c + 2x + 1)/(c - 2x - 1)
    //   f3^-1(x) = -b - (3x/2) + 1/2 +- t(x)
    //
    // Note that we changed a few things from the gist:
    //
    //   1. We modified f3^-1(x) to use a solution directly from sage.
    //   2. We also swapped f1^-1(x) and f2^-1(x) (they're mixed up).
    //
    // Loop:
    //
    //   1. Generate a random point (x, y) on the curve.
    //   2. Compute the set S as the values s that come out of f1^-1(x),
    //      f2^-1(x), and f3^-1(x) that are squares and for which f(s) = x.
    //   3. If S is empty: continue.
    //   4. Generate a random integer r in [0..251] (252 possibilities).
    //   5. If S has size 1 and r >= 9: continue.
    //   6. If S has size 2 and r >= 14: continue.
    //   7. If S has size 3 and r >= 189: continue.
    //   8. Pick s = S[r mod n].
    //   9. If y is a square, return s; otherwise, return -s.
    //
    // Method:
    //
    //   c = sqrt(-3)
    //   t = sqrt(6 * (2 * b - 1) * x + 9 * x^2 - 12 * b - 3) / 2
    //   s1 = (b + 1) * (c - 2 * x - 1) / (c + 2 * x + 1)
    //   s2 = (b + 1) * (c + 2 * x + 1) / (c - 2 * x - 1)
    //   s3 = -b - ((3 * x) / 2) + (1 / 2) + t
    //   s4 = -b - ((3 * x) / 2) + (1 / 2) - t
    //   S = [sn if sn is square and f(sn) = x, ...]
    //   r = random integer in [0..251]
    //   s = none, if r >= [0, 9, 14, 189, 252][n]
    //     = S[r mod n], if y is square
    //     = -S[r mod n], otherwise
    const {one, three, i2} = this;
    const e = this.p.subn(2);
    const w = this.p.addn(1).iushrn(2);
    const c = three.redNeg().redSqrt();
    const bp1 = this.b.redAdd(one);
    const b2 = this.b.redMuln(2);
    const b12 = this.b.redMuln(12);
    const [x, y] = p.getXY();
    const xt2p1 = x.redMuln(2).redIAdd(one);
    const c0 = c.redSub(xt2p1);
    const c1 = c.redAdd(xt2p1);
    const c2 = c0.redMul(c1).redPow(e);
    const t0 = b2.redSub(one).redIMuln(6).redMul(x);
    const t1 = x.redSqr().redIMuln(9);
    const t2 = t0.redIAdd(t1).redISub(b12).redISub(three).redPow(w).redMul(i2);
    const t3 = this.b.redNeg().redISub(x.redMuln(3).redMul(i2)).redIAdd(i2);
    const s1 = bp1.redMul(c0).redMul(c2.redMul(c0));
    const s2 = bp1.redMul(c1).redMul(c2.redMul(c1));
    const s3 = t3.redAdd(t2);
    const s4 = t3.redSub(t2);
    const [x1,, quad1] = this._svdwf(s1);
    const [x2,, quad2] = this._svdwf(s2);
    const [x3,, quad3] = this._svdwf(s3);
    const [x4,, quad4] = this._svdwf(s4);
    const r = BN.random(rng, 0, 252).word(0);
    const S = [];
    ...
    const max = [0, 9, 14, 189, 252][S.length];

    if (r >= max)
      throw new Error('Invalid point.');

    const s = S[r % S.length];

    s.cinject(s.redNeg(), y.redIsSquare() ^ 1);
  ...
  pointFromMont(point) {
    ...
    const i3 = point.curve.three.redInvert();
    const a3 = point.curve.a.redMul(i3);
    const nx = point.x.redAdd(a3);
    const ny = point.y;

    return this.point(nx.fromRed(), ny.fromRed());
  }
  ...
  pointFromEdwards(point) {
    ...
    //
    // Unexceptional Cases:
    //   - (+-1, 0) -> (a / 3 + 1, +-sqrt(-(a + 2) / b))
    const i3 = point.curve.three.redInvert();
    const [a] = point.curve._untwisted();
    const a3 = a.redMul(i3);
    const c = a.redAdd(point.curve.two).redINeg().redSqrt().redINeg();
    const zpy = point.z.redAdd(point.y);
    const zmy = point.z.redSub(point.y);

    // (0, -1) -> (a / 3, 0)
    if (point.x.isZero())
      return this.point(a3.fromRed(), this.zero);

    const nx = zpy.redMul(zmy.redInvert()).redIAdd(a3);
    const ny = c.redMul(zpy).redMul(zmy.redMul(point.x).redInvert());
...
class MontCurve extends Curve {
  ...
  _twisted(a0) {
    ...
    //   https://eprint.iacr.org/2017/293.pdf
    //
    // Montgomery to Twisted Edwards conversion:
    //
    //   a = (A + 2) / B
    //   d = (A - 2) / B
    //
    ...
    if (a0) {
      if (a0.cmp(this.one) === 0 || a0.redJacobi() !== 1)
        throw new Error('Invalid `a` coefficient.');

      const a = a0.clone();
      const b = this.a.redAdd(this.two).redMul(a.redInvert());
      const d = this.a.redSub(this.two).redMul(b.redInvert());

      return [a, d];
    }

    const a = this.a.redAdd(this.two).redMul(this.bi);
    const d = this.a.redSub(this.two).redMul(this.bi);
  ...
  _twist(p) {
    ...
    if (p.x.isZero() && p.y.isZero())
      return [zero, one.redNeg(), one];

    const xx = p.x;
    const xz = p.y;
    const yy = p.x.redSub(one);
    const yz = p.x.redAdd(one);
  ...
  _untwist(x, y, z) {
    ...
    // (0, -1) -> (0, 0)
    if (x.isZero() && y.eq(z.redNeg()))
      return this.point(this.zero, this.zero);

    const uu = z.redAdd(y);
    const uz = z.redSub(y);
    const vv = z.redMul(uu);
  ...
  _scale(curve) {
    ...
    return curve._scale(this).forceRed(this.red);
  }
  ...
  _elligator2(u, spec) {
    ...
    //   - Let u be a non-square in F(q) (we call this `z`).
    //   - Furthermore vexy != 0 and y^2 = x^3 + Ax^2 + Bx.
    //   - Let sqrt() pick the smallest root (i.e. in F(q / 2)).
    //
    // Note that elligator2 is defined over the form:
    //
    //   y'^2 = x'^3 + a' * x'^2 + b' * x'
    //
    // Instead of:
    ...
    //   y = b * y'
    //
    // If we want to support b != 1, we need to do the conversion.
    //
    // Map:
    //
    //   f(a) = a^((q - 1) / 2)
    ...
    //   x = x1, if g(x1) is square
    //     = x2, otherwise
    //   y = sign(u) * sqrt(g(x)), if spec
    //     = -sqrt(g(x)), if g(x1) is square
    //     = sqrt(g(x)), otherwise
    ...
    if (spec) {
      // Follow the spec and use the field element's sign.
      y0.cinject(y0.redNeg(), y0.redIsNeg() ^ u.redIsNeg());
    } else {
      // Negate if we were a quadratic residue.
      y0.cinject(y0.redNeg(), y0.redIsNeg() ^ (i ^ 1));
    }
  ...
  _invert2(p) {
    ...
    const n = x0;
    const d = x0.redAdd(this.a0);

    n.cswap(d, y0.redIsNeg());
  ...
  pointFromEdwards(point) {
    ...
    if (point.curve.twisted) {
      ...
      uu = y.redNeg().redISub(z);
      uz = y.redNeg().redIAdd(z);
      vv = c.redMul(z).redMul(uu);
      vz = x.redMul(uz);
    }
  ...
  pointFromUniform(u, spec = false) {
    assert(u instanceof BN);
    assert(typeof spec === 'boolean');

    // z = 0 or a = 0
    if (this.z.isZero() || this.a.isZero())
      throw new Error('Not implemented.');

    return this._elligator2(u, spec);
  }
  ...
  pointFromHash(bytes) {
    ...
    const u1 = this.decodeUniform(s1);
    const u2 = this.decodeUniform(s2);
    const p1 = this.pointFromUniform(u1, true);
    const p2 = this.pointFromUniform(u2, true);

    return p1.add(p2).mulH();
  }
...
class EdwardsCurve extends Curve {
  ...
  _untwisted(b0) {
    ...
    //   https://eprint.iacr.org/2008/013.pdf
    //
    // Twisted Edwards to Montgomery conversion:
    //
    //   A = 2 * (a + d) / (a - d)
    //   B = 4 / (a - d)
    //
    ...
    const apd = this.a.redAdd(this.d);
    const amd = this.a.redSub(this.d);
    const i = amd.redInvert();
    const a = apd.redMuln(2).redMul(i);
    const b = this.four.redMul(i);
  ...
  _scale(curve) {
    ...
    // Missing base points. Use native equivalence.
    if (this.g.isInfinity() || curve.g.isInfinity())
      return this.one;

    // Calculate scaling factor.
    if (!this.factor) {
      const y = curve.g.y.forceRed(this.red);
      const g = this.g;
      ...
      if (this.twisted) {
        ...
        uu = g.y.redNeg().redISub(g.z);
        uz = g.y.redNeg().redIAdd(g.z);
      }

      const u = uu.redMul(uz.redInvert());
      const v = u.redMul(g.x.redInvert());

      this.factor = y.redMul(v.redInvert());
    }
  ...
  solveY2(x) {
    // sage: solve(a * x^2 + y^2 == c^2 * (1 + d * x^2 * y^2), y^2)
    // [y^2 == (a*x^2 - c^2)/(c^2*d*x^2 - 1)]
    assert(x instanceof BN);
  ...
  validate(point) {
    ...
    // T = (X * Y) / Z
    if (this.extended) {
      const tz = point.zOne ? point.t : point.t.redMul(point.z);
  ...
  pointFromShort(point) {
    ...
    // Isomorphism:
    //
    //   x = -sqrt(-(a + 2) / b) * (3 * x - a) / (3 * y)
    //   y = (3 * x - a - 3) / (3 * x - a + 3)
    //
    // Undefined for x = (a / 3) - 1 or y = 0.
    //
    // Exceptional Cases:
    //   - O -> (0, 1)
    //   - (a / 3 - 1, y) -> invalid (wei25519)
    //   - (a / 3, 0) -> (0, -1)
    //
    // Unexceptional Cases:
    //   - (a / 3 + 1, +-sqrt(-(a + 2) / b)) -> (+-1, 0)
    const x = point.x.forceRed(this.red);
    const y = point.y.forceRed(this.red);
    const [a] = this._untwisted();
    const c = a.redAdd(this.two).redINeg().redSqrt().redINeg();
    const t = x.redMuln(3).redISub(a);
    const tm3 = t.redSub(this.three);
    const tp3 = t.redAdd(this.three);

    // (a / 3, 0) -> (0, -1)
    if (t.isZero())
      return this.point(this.zero, this.one.redNeg());

    const nx = c.redMul(t).redMul(y.redMuln(3).redInvert());
    const ny = tm3.redMul(tp3.redInvert());
  ...
  pointFromMont(point) {
    ...
    if (this.twisted) {
      ...
      xx = c.redMul(u);
      xz = v;
      yy = this.one.redAdd(u).redINeg();
      yz = this.one.redSub(u);
    }
    ...
    const y = yy.redMul(xz);
    const z = xz.redMul(yz);
    const t = this.extended ? xx.redMul(yy) : null;
  ...
  pointFromUniform(curve, u, spec) {
    ...
    if (!curve) {
      ...
      if (this.s.isZero() || this.p.andln(3) !== 3)
        throw new Error('Elligator 1 is unsupported.');
    ...
    const p0 = curve.pointFromUniform(u0, spec);
    const p = this.pointFromMont(p0);

    return p;
  }

  pointToUniform(curve, p) {
    assert(curve == null || (curve instanceof MontCurve));
    ...
    if (!curve) {
      ...
      return this._invert1(p);
    }

    // Elligator 2.
    const p0 = curve.pointFromEdwards(p);
    const u0 = curve.pointToUniform(p0);
    const u = u0.forceRed(this.red);

    return u;
  }

  pointFromHash(curve, bytes) {
    assert(curve == null || (curve instanceof MontCurve));
    assert(Buffer.isBuffer(bytes));
    ...
    const u1 = this.decodeUniform(s1);
    const u2 = this.decodeUniform(s2);
    const p1 = this.pointFromUniform(curve, u1, true);
    const p2 = this.pointFromUniform(curve, u2, true);
...
class EdwardsPoint extends Point {
  ...
  dbl() {
    ...
    // a = -1
    if (this.curve.extended)
      return this._dblExt(0);

    return this._dblProj(0);
  }
  ...
  trpl() {
    ...
    // a = -1
    if (this.curve.extended)
      return this._trplExt(0);

    return this._trplProj(0);
  }
  ...
  _trplProj(flag) {
    ...
    if (this.curve.twisted) {
      ...
      // Z3 = Z1 * F * G
      nz = zOne ? f.redMul(g) : this.z.redMul(f).redMul(g);
    } else {
      let zz;
      ...
      // Z3 = T * U * Z1
      nz = zOne ? t.redMul(u) : t.redMul(u).redMul(this.z);
    }
  ...
  udbl() {
    // a = -1
    if (this.curve.extended)
      return this._dblExt(1);

    return this._dblProj(1);
  }

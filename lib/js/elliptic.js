...
class ShortCurve extends Curve {
  ...
  _svdw2(s0) {
    ...
    //   y = -y, if f(y) != sign
    const s = s0.clone();
    const sign = s.redLegendre() >>> 31;

    s.cinject(s.redNeg(), sign);
    ...
    y.cinject(y.redNeg(), (y.redLegendre() >>> 31) ^ sign);
  ...
  _invert2(p, rng) {
    ...
    const e = this.p.subn(2);
    const c = this.three.redNeg().redSqrt();
    const w = this.p.addn(1).iushrn(1);
    ...
    const s4 = t4.redNeg().redISub(xt3).redIAdd(t3).redMul(c).redMul(this.i2);
    const r1 = -s1.redLegendre() >>> 31;
    const r2 = -s2.redLegendre() >>> 31;
    const r3 = -s3.redLegendre() >>> 31;
    const r4 = -s4.redLegendre() >>> 31;
    const x1 = this._svdw2(s1).x;
    const x2 = this._svdw2(s2).x;
    const x3 = this._svdw2(s3).x;
    const x4 = this._svdw2(s4).x;
    ...
    s.cinject(s.redNeg(), y.redLegendre() >>> 31);
  ...
  pointFromUniform(u) {
    ...
    // p = 1 mod 3, b != 0
    if (this.p.modrn(3) === 1 && !this.b.isZero())
      return this._svdw(u);
...
class MontCurve extends Curve {
  ...
  _elligator2(u, spec) {
    ...
    if (spec) {
      ...
      // Pick lowest square root (required for 5 mod 8).
...
class EdwardsCurve extends Curve {
  ...
  _elligator1(t) {
    ...
    assert(this.p.andln(3) === 3);
    assert(!this.s.isZero());
  ...
  pointFromUniform(curve, u, spec) {
    ...
    if (!curve) {
      if (this.s.isZero())
        throw new Error('Elligator is unsupported.');

      return this._elligator1(u);
    }

    const f = u.forceRed(curve.red);
    const m = curve.pointFromUniform(f, spec);
    const p = this.pointFromMont(m);
  ...
  pointToUniform(curve, p) {
    ...
    const f = curve.pointToUniform(m);
    const u = f.forceRed(this.red);

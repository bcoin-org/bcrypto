...
class MontCurve extends Curve {
  ...
  pointFromEdwards(point) {
    ...
    if (point.curve.twisted) {
      ...
      // Edwards curve with a `d` coefficient of:
      //
...
class EdwardsCurve extends Curve {
  ...
  _solve25519(y) {
    ...
    // V = d * Y2 + 1
    const v = this.d.redMul(y2).redIAdd(this.one);

    // U = Y2 - 1
    const u = y2.redISub(this.one);
  ...
  _solve448(y) {
    ...
    // U = Y2 - 1
    const u = y2.redSub(this.one);

    // V = d * Y2 - 1
    const v = y2.redIMuln(-39081).redISub(this.one);

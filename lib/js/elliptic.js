...
class EdwardsCurve extends Curve {
  ...
  isIsogenous(curve) {
    ...
    // E(a,d) <-> M(2-4d,1)
    if (curve.type === 'mont') {
      const a = this.two.redSub(this.d.redMuln(4));
      return curve.a.eq(a) && curve.b.eq(this.one);
    }

    // E(a,d) <-> E(-a,d-a)
    if (curve.type === 'edwards') {
      return curve.a.eq(this.a.redNeg())
          && curve.d.eq(this.d.redSub(this.a));
    }

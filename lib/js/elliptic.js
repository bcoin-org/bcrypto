...
class ShortCurve extends Curve {
  ...
  _sswui(p, hint) {
    ...
    //   u3 = sqrt((t4 + t3) / t1)
    //   u4 = sqrt((t4 - t3) / t1)
    //   U = [un if un is a root, ...]
    //   r = random integer in [0..2^32-1]
    //   u = none, if n = 0 or t2 is not a root
    //     = sign(y) * abs(U[r mod n]), otherwise
    const {a, b, z, i2} = this;
    const e = this.p.subn(2);
    const [x, y] = p.getXY();
    ...
    const t0 = a.redMul(x).redMul(z).redIAdd(b.redMul(z)).redPow(e);
    const t1 = b.redMul(z).redInvert();
    ...
    const [s1, u1] = sqrt(t5.redMul(t0));
    const [s2, u2] = sqrt(t6.redMul(t0));
    const [s3, u3] = sqrt(t5.redMul(t1));
    const [s4, u4] = sqrt(t6.redMul(t1));
    const r = hint >>> 0;
    const U = [];
    ...
    // distributed (more or less). SSWU covers
    // about 37.5% of the curve points. Assuming
    // the point is within that set, each point
    // has a ~25% chance of inverting to any of
    // the preimages. This means we can simply
    // randomly select a preimage if one exists.
    if (U.length === 0)
      throw new Error('Invalid point.');

    const u = U[r % U.length];

    u.cinject(u.redNeg(), u.redIsNeg() ^ y.redIsNeg());
  ...
  _svdwf(u) {
    ...
    const c = this.three.redNeg().redMul(z2).redSqrt();
  ...
  _svdwi(p, hint) {
    ...
    //
    // Idea modified from: Covert ECDH over secp256k1.
    //   Pieter Wuille.
    ...
    //   U = [un if un is a root and svdw(un) = x, ...]
    //   r = random integer in [0..251]
    //   u = none, if r >= [0, 9, 14, 189, 252][n]
    //     = sign(y) * abs(U[r mod n]), otherwise
    const {z, i2} = this;
    const e = this.p.subn(2);
    const z2 = z.redSqr();
    ...
    const t2 = gz.redMul(x).redISub(gz.redMul(z)).redIMuln(12);
    ...
    const [s1, u1] = sqrt(gz.redMul(c0).redMul(c2.redMul(c0)));
    const [s2, u2] = sqrt(gz.redMul(c1).redMul(c2.redMul(c1)));
    const [s3, u3] = sqrt(t5.redAdd(t4));
    const [s4, u4] = sqrt(t5.redSub(t4));
    const [x1] = this._svdwf(u1);
    const [x2] = this._svdwf(u2);
    const [x3] = this._svdwf(u3);
    const [x4] = this._svdwf(u4);
    const r = hint % 252;
    const U = [];
    ...
    // The preimages here are not evenly
    // distributed. In order to maintain
    // distribution, we must randomly reject
    // certain preimages based on a weight.
    const max = [0, 9, 14, 189, 252][U.length];
...
class MontCurve extends Curve {
  constructor(conf) {
    ...
    this.smi = this.a24.word(0);
    this.smallA24 = this.prime != null && this.a24.length === 1;
...
class XPoint extends Point {
  ...
  dbl() {
    ...
    // Z3 = C * (BB + a24 * C)
    const nz = c.redMul(bb.redIAdd(this.curve._mulA24(c)));
  ...
  diffAddDbl(p, q) {
    ...
    // Z4 = E * (BB + a24 * E)
    const dz = e.redMul(bb.redIAdd(this.curve._mulA24(e)));

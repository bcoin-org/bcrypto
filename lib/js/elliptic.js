...
class ShortCurve extends Curve {
  constructor(conf) {
    ...
    this.ia = this.a.isZero() ? this.zero : this.a.redInvert();
    this.iz = this.z.isZero() ? this.zero : this.z.redInvert();
  ...
  _sswu(u) {
    ...
    const {b, z, ia, iz, one} = this;
    const e = this.p.subn(2);
    const z2 = z.redSqr();
    const ba = b.redNeg().redMul(ia);
    const bza = b.redMul(iz).redMul(ia);
    const u2 = u.redSqr();
    const u4 = u2.redSqr();
  ...
  _svdwf(u) {
    ...
    //     = x3, otherwise
    //   y = sign(u) * abs(sqrt(g(x)))
    const {c, z, iz, i2, i3} = this;
    const e = this.p.subn(2);
    const gz = this.solveY2(z);
    const z3 = i3.redMul(iz.redSqr());
    const u2 = u.redSqr();
    const u4 = u2.redSqr();
...
class EdwardsCurve extends Curve {
  constructor(conf) {
    ...
    this.is = this.s.isZero() ? this.zero : this.s.redInvert();
  ...
  _elligator1(t) {
    ...
    const {s, is, i2, one, two} = this;
    const c = is.redSqr().redIMuln(2);
    const ic = s.redSqr().redMul(i2);
    const ic2 = ic.redSqr();
    const r = c.redAdd(ic);
    const r2 = r.redSqr();
    ...
    const f1 = u2.redAdd(ic2).redPow(e1);
    const f2 = f0.redMul(f1);
    const X = f0.redMul(u);
    const Y = f0.redMul(v).redPow(e2).redMul(f2);
    const Xp = one.redAdd(X);
    const sXXp = s.redMul(X).redMul(Xp);
    const rX = r.redMul(X);
    const Xp2 = Xp.redSqr();
    const xx = c.redSub(one).redMul(sXXp);
    const xz = Y.cinject(this.one, u.czero());
    const yy = rX.redSub(Xp2);
    const yz = rX.redAdd(Xp2);
  ...
  _invert1(p, hint) {
    ...
    const e2 = this.p.addn(1).iushrn(2);
    const n0 = y.redSub(z);
    const n1 = y.redAdd(z).redIMuln(2);
    ...
    const z2 = X.redSqr().redIAdd(ic2);
    const z3 = z0.redMul(s).redMul(X).redMul(z1);
    const z4 = z3.redMul(x.redMul(z)).redMul(z2);

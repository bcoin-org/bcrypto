...
class ShortCurve extends Curve {
  ...
  isIsomorphic(curve) {
    ...
    if (curve.type === 'short') {
      ...
      if (jinv.isZero()) {
        return this.a.cmp(curve.a) === 0
            && this.b.cmp(curve.b) === 0;
      }

      return jinv.cmp(curve.jinv()) === 0;
    }
...
class MontCurve extends Curve {
  ...
  _mont(b0) {
    ...
    assert(c.redInvert().redJacobi() === 1);

    return [this.a.clone(), b0.clone()];
  }
...
class EdwardsCurve extends Curve {
  ...
  isIsomorphic(curve, invert = false) {
    ...
    if (curve.type === 'edwards') {
      ...
      return this.a.redMul(d).cmp(a.redMul(this.d)) === 0;
    }

...
class Curve {
  ...
  sanity() {
    ...
    assert(this._simpleMul(g, k).eq(g));

    // Test Shamir's trick (simple).
    assert(this._simpleMulAdd([g, g], [k, k]).eq(g.dbl()));

    // Test unified ladder.
    assert(this._ladderMul(g, k).eq(g));

    // Test Co-Z ladder.
    if (this.type === 'short')
      assert(this._coZLadderMul(g, k.muln(2)).eq(g.dbl()));

    // Test fixed NAF multiplication.
    if (this.g.pre && b === this.scalarBits)
      assert(this._fixedNafMul(this.g, k).eq(g));

    // Test wNAF multiplication.
    assert(this._wnafMul(4, g, k).eq(g));

    // Test wNAF multiplication (precomp).
    assert(this._wnafMul(4, this.g, k).eq(g));

    // Test Shamir's trick.
    assert(this._wnafMulAdd(4, [g, g], [k, k]).eq(g.dbl()));

    // Test Shamir's trick (precomp).
    assert(this._wnafMulAdd(4, [this.g, a], [k, k]).eq(g.dbl()));

    // Test Shamir's trick (precomp + JSF).
    assert(this._wnafMulAdd(4, [this.g, a, a], [k, k, k]).eq(g.trpl()));
  }

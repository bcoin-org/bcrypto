...
class Curve {
  ...
  _scalarBlinding(rng) {
    // We blind scalar multiplications.
    //
    // This is an extra precaution given
    // that our bigint implementation is
    // only constant time for 235-285 bit
    // ints (though, in reality it may not
    // be constant time at all due to the
    // deoptimizer kicking in after a SMI
    ...
    const b = this.randomScalar(rng);
    const bi = b.invert(this.n);

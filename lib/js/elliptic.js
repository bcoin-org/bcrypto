...
class MontCurve extends Curve {
  ...
  _twist(p) {
    ...
    if (p.inf) {
      const x = this.zero.clone();
      const y = this.one.clone();
      const z = this.one.clone();
    ...
    const xx = p.x;
    const xz = p.y;
    const yy = p.x.redSub(this.one);
    const yz = p.x.redAdd(this.one);
  ...
  pointFromEdwards(point) {
    ...
    if (point.curve.twisted) {
      ...
      uu = point.y.redSqr();
      uz = point.x.redSqr();
      vv = point.curve.two.redSub(uz).redISub(uu).redMul(point.y);
      vz = uz.redMul(point.x);
    } else {
    ...
    // Montgomery point.
    const z = uz.redMul(vz).redInvert();
    const u = uu.redMul(vz).redMul(z);
    const v = vv.redMul(uz).redMul(z);

    return this.point(u.fromRed(), v.fromRed());
  }

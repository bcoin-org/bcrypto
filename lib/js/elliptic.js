...
class EdwardsPoint extends Point {
  ...
  static decode(curve, bytes) {
    ...
    // implementation.
    //
    // Hamburg rejects (0, c) and (0, -c).
    // The latter is not 4-isogenous with
    // the Montgomery form of Curve448.
    if (curve.id === 'ED448') {
      if (p.x.isZero() && p.y.eq(curve.c.redNeg()))
        throw new Error('Invalid point.');
    }
...
class X448 extends MontCurve {
  ...
  _fromEdwards(point) {
    ...
    //   u = y^2 / x^2
    //   v = (2 - x^2 - y^2) * y / x^3
    const nx = point.y.redSqr();
    const nz = point.x.redSqr();

...
class ShortCurve extends Curve {
  ...
  _short(a0, sign) {
    ...
    const u4 = a0.redMul(this.a.redInvert());
    const u2 = u4.redSqrt();
  ...
  _isomorphism(curve) {
    ...
    if (this.g.isInfinity() || curve.g.isInfinity()) {
      ...
      const u4 = this.a.redMul(a.redInvert());
      const u6 = this.b.redMul(b.redInvert());
      const u2 = u4.redSqrt();
    ...
    const u2 = this.g.x.redMul(x.redInvert());
    const u3 = this.g.y.redMul(y.redInvert());
...
class MontCurve extends Curve {
  ...
  _mont(b0) {
    ...
    const c = this.b.redMul(b0.redInvert());
  ...
  _isomorphism(curve) {
    ...
    if (this.g.isInfinity() || curve.g.isInfinity()) {
      const b = curve.b.forceRed(this.red);
      return this._isqrt(b, this.b);
    }

    const y = curve.g.y.forceRed(this.red);

    return this.g.y.redMul(y.redInvert());
  }
...
class EdwardsCurve extends Curve {
  ...
  _edwards(a0) {
    ...
    const dai = this.d.redMul(this.a.redInvert());
    const a = a0.clone();
    const d = a0.redMul(dai);
    const b0 = this.a.redSub(this.d).redInvert().redIMuln(4);
    const b1 = a.redSub(d).redInvert().redIMuln(4);
  ...
  _isomorphism(curve) {
    ...
    if (this.g.isInfinity() || curve.g.isInfinity()) {
      const a = curve.a.forceRed(this.red);
      return this._isqrt(a, this.a);
    }

    const x = curve.g.x.forceRed(this.red);

    return this.g.x.redMul(x.redInvert());
  }

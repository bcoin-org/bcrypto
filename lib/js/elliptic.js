...
class Curve {
  ...
  _ladderMul(p, k) {
    ...
    //          + 512*a + 1024*4 + 512*3
    //          + 1024*2
    const [sign, bits, exp] = getLadderBits(k, this.n);
  ...
  _coZLadderMul(p, k) {
    ...
    const [sign, bits, exp] = getCOZBits(k, this.n);
...
class ShortPoint extends Point {
  ...
  recover(x1, x2) {
    ...
    if (!x1.red)
      x1 = x1.toRed(this.curve.red);

    if (!x2.red)
      x2 = x2.toRed(this.curve.red);
...
class MontPoint extends Point {
  ...
  static fromJSON(curve, json) {
    assert(curve instanceof MontCurve);
    assert(Array.isArray(json));
    assert(json.length === 0 || json.length === 1);
...
class EdwardsCurve extends Curve {
  ...
  solveX2(y) {
    ...
    const rhs = this.ccd.redMul(y2).redISub(this.a);
  ...
  solveY2(x) {
    ...
    const rhs = this.one.redSub(this.ccd.redMul(x2));
  ...
  validate(point) {
    ...
    const dxy = this.d.redMul(x2).redMul(y2);
...
class EdwardsPoint extends Point {
  ...
  eq(p) {
    ...
    //     return this.x.redMul(p.y).eq(p.x.redMul(this.y));
    //
    // [1] https://www.shiftleft.org/papers/decaf/decaf.pdf
    return this.getX().cmp(p.getX()) === 0
        && this.getY().cmp(p.getY()) === 0;
  }
...
class X448 extends MontCurve {
  ...
  _fromEdwards(point) {
    // Note: x448 is 4-isogenous to ed448. It
    // is birationally equivalent to an unnamed
    // curve with a `d` coefficient of:
    //
    ...
    return lhs.redMul(rhs.redInvert());
  }
}
...
function getLadderBits(k, n) {
  ...
  const bits = Math.max(k.bitLength(), n.bitLength());
  const bytes = (bits + 7) >>> 3;

...
class ShortCurve extends Curve {
  ...
  _svdw(u) {
    ...
    const alpha = y1.redLegendre();
    const beta = y2.redLegendre();
  ...
  _svdwf(s0) {
    ...
    //   y = -y, if f(y) != sign
    const s = s0.clone();
    const q = ~s.redLegendre() >>> 31;

    s.cinject(s.redNeg(), q ^ 1);
    ...
    const x2 = c.redNeg().redISub(this.one).redMul(this.i2).redIAdd(t1);
  ...
  _svdw2(s0) {
    const [x, yy, q] = this._svdwf(s0);
    const y = yy.redSqrt();

    // Note that the result of x^((p + 1) / 4)
    // is always a quadratic residue, so we
    // assume `y` is a square here. Otherwise,
    // would would have to pass something like
    // `(~y.redLegendre() >>> 31) ^ q`.
    y.cinject(y.redNeg(), q ^ 1);
  ...
  _invert2(p, rng) {
    ...
    //   f3^-1(x) = c(+-(3x^2 + (4b - 2)x + (1 - 4b))^1/2 - 3x + (1 - 2b))/2
    //
    // Alternatives to f3^-1(x):
    //
    //   f3^-1(x) = (b + 1)c/(c +- 3(1 - x)^1/2)
    //   f3^-1(x) = c(+-(4bx - 4b + 3x^2 - 2x + 1)^1/2 - 2b - 3x + 1)/2
    //
    // Loop:
    //
    ...
    //   c = sqrt(-3)
    //   w = (p + 1) / 4
    //   t = (3 * x^2 + (4 * b - 2) * x + (1 - 4 * b))^w
    //   s1 = (b + 1) * (c + 2 * x + 1) / (c - 2 * x - 1)
    //   s2 = (b + 1) * (c - 2 * x - 1) / (c + 2 * x + 1)
    //   s3 = c * (t - 3 * x + (1 - 2 * b)) / 2
    //   s4 = c * (-t - 3 * x + (1 - 2 * b)) / 2
    //   S = [sn if f(sn) != -1 and svdw(sn) = x, ...]
    //   s = none, if r >= [0, 9, 14, 189][n]
    //     = S[r mod n], if f(y) != -1
    //     = -S[r mod n], otherwise
    const e = this.p.subn(2);
    const c = this.three.redNeg().redSqrt();
    const w = this.p.addn(1).iushrn(2);
    const bp1 = this.b.redAdd(this.one);
    const b4 = this.b.redMuln(4);
    const b2 = this.b.redMuln(2);
    const [x, y] = p.getXY();
    const xt2p1 = x.redMuln(2).redIAdd(this.one);
    const xt3 = x.redMuln(3);
    const c0 = c.redAdd(xt2p1);
    const c1 = c.redSub(xt2p1);
    const t0 = x.redSqr().redIMuln(3);
    const t1 = b4.redSub(this.two).redMul(x);
    const t2 = this.one.redSub(b4);
    const t3 = this.one.redSub(b2);
    const t4 = t0.redAdd(t1).redIAdd(t2).redPow(w);
    const s1 = bp1.redMul(c0).redMul(c1.redPow(e));
    const s2 = bp1.redMul(c1).redMul(c0.redPow(e));
    const s3 = t4.redSub(xt3).redIAdd(t3).redMul(c).redMul(this.i2);
    const s4 = t4.redINeg().redISub(xt3).redIAdd(t3).redMul(c).redMul(this.i2);
    const [x1,, q1] = this._svdwf(s1);
    const [x2,, q2] = this._svdwf(s2);
    const [x3,, q3] = this._svdwf(s3);
    const [x4,, q4] = this._svdwf(s4);
    ...
    const max = [0, 9, 14, 189][S.length];

    if (r >= max)
      throw new Error('Invalid point.');

    const s = S[r % S.length];
    const q = ~y.redLegendre() >>> 31;

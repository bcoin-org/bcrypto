...
class Curve {
  ...
  _slideMul(width, p, k) {
    ...
    // Multiply.
    let acc = this.jpoint();
    let left = k.bitLength();

    while (left >= width) {
      const bits = k.bits(left - width, width);

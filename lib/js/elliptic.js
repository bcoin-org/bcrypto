...
class ShortCurve extends Curve {
  ...
  _mont(b0, twisted = true) {
    // Weierstrass to Montgomery conversion:
    //
    //   A = +-sqrt(-3 * B^2 * a + 3)
    //   B = +-sqrt(-1/3 * A^2 / a + 1 / a)
    //
    // B * (A + 2) is usually square for twisted.
    assert(b0 == null || (b0 instanceof BN));
    assert(typeof twisted === 'boolean');
    assert(this.h.cmpn(4) >= 0);
    ...
    const b = b0.clone();
    const b2 = b.redSqr();
    const a2 = this.a.redMul(b2).redIMuln(-3).redIAdd(three);

    if (b0.isZero() || a2.redJacobi() !== 1)
      throw new Error('Invalid `b` coefficient.');

    const a = a2.redSqrt();
    const a2b = a.redAdd(this.two).redMul(b);

    if (a2b.redJacobi() !== Number(twisted))
      a.redINeg();
  ...
  toMont(b0, twisted) {
    const [a, b] = this._mont(b0, twisted);
...
class MontCurve extends Curve {
  ...
  pointFromEdwards(point) {
    ...
    if (point.curve.twisted) {
      ...
      // well: flipping the signs appears to fix it.
      //
      // Birational maps:
      //
      //   u = (y + 1) / (y - 1)

...
class ShortCurve extends Curve {
  ...
  pointFromEdwards(point) {
    ...
    //
    // Unexceptional Cases:
    //   - (+-sqrt(1 / a), 0) -> (a / 3 + 1, +-sqrt(-(a + 2) / b))
    const zpy = point.z.redAdd(point.y);
    const zmy = point.z.redSub(point.y);
    const xx = zpy.redAdd(a3.redMul(zmy));
    const xz = zmy;
    const yy = c.redMul(point.z).redMul(zpy);
    const yz = zmy.redMul(point.x);

    // Should never happen.
    if (xz.isZero() || yz.isZero())
      throw new Error('Invalid point.');

    const nz = xz.redMul(yz).redInvert();
    const nx = xx.redMul(yz).redMul(nz);
    const ny = yy.redMul(xz).redMul(nz);
...
class MontCurve extends Curve {
  ...
  _twisted(a0) {
    ...
    if (a0) {
      if (a0.isZero() || a0.cmp(this.one) === 0)
        throw new Error('Invalid `a` coefficient.');

      const a = a0.clone();
      const d = a.redMul(am2).redMul(ap2.redInvert());
...
class MontPoint extends Point {
  ...
  jmulConst(k, rng) {
    ...
    const p = this.toX();
    const [a, b] = p.ladderConst(k, rng);
    const q = this.recover(b, a);
...
class EdwardsCurve extends Curve {
  constructor(conf) {
    ...
    this.a = BN.fromJSON(conf.a).toRed(this.red);
    this.c = BN.fromJSON(conf.c || '1').toRed(this.red);
  ...
  _untwisted(b0) {
    ...
    const den = amd.redInvert();
    const a = apd.redMuln(2).redMul(den);
    const b = den.redMuln(4);
  ...
  _scale(curve) {
    ...
    let factor = this.cache.get(curve);
  ...
  pointFromShort(point) {
    ...
    const nx = xx.redMul(yz);
    const ny = yy.redMul(xz);
  ...
  pointFromMont(point) {
    ...
    const t = this.twisted ? xx.redMul(yy) : null;
  ...
  pointFromEdwards(point) {
    ...
    if (this.iso4 && point.curve.twisted) {
      ...
      //   y = (y^2 - x^2) / (y^2 + x^2)
      //
      // Undefined for y^2 - x^2 = 2 or y^2 + x^2 = 0.
      const x2 = x.redSqr();
      const y2 = y.redSqr();
...
class EdwardsPoint extends Point {
  constructor(curve, x, y, z, t) {
    ...
    this.y = this.curve.c;
    this.z = this.curve.one;
    this.t = this.curve.twisted ? this.curve.zero : null;
    this.zOne = 1;
  ...
  _init(x, y, z, t) {
    ...
    assert(this.curve.twisted || t == null);

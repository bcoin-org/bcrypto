...
class ShortCurve extends Curve {
  ...
  randomPoint(rng) {
    ...
    for (;;) {
      const x = BN.random(rng, 1, this.p);
      const sign = BN.random(rng, 0, 255);
...
class MontPoint extends Point {
  ...
  jmul(k) {
    ...
    const exp = k.encode('le', bytes);

    // Clamp scalar.
    this.curve.clamp(exp);
...
class EdwardsCurve extends Curve {
  ...
  randomPoint(rng) {
    ...
    for (;;) {
      const y = BN.random(rng, 1, this.p);
      const sign = BN.random(rng, 0, 255);
...
class Precomp {
  ...
  toJSON() {
    return {
      ...
      blinding: this.blinding ? this.blinding.toJSON() : null
    };
  }

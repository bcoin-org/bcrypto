...
class Curve {
  ...
  _init(type, conf) {
    ...
    this.z = conf.z ? BN.fromJSON(conf.z) : new BN(0);
    this.g = null;
  ...
  _isqrt5mod8(u, v) {
    ...
    //
    // For Ed25519, sqrt(a) also seems to be
...
class ShortCurve extends Curve {
  ...
  pointFromHash(bytes) {
    ...
    const r1 = this.decodeUniform(s1);
    const r2 = this.decodeUniform(s2);
...
class MontCurve extends Curve {
  ...
  _elligator2(r) {
    ...
    const rhs = this.one.redAdd(this.z.redMul(r.redSqr()));
  ...
  _invert2(p, sign) {
    ...
    const r = this._isqrt(lhs, rhs);

    r.cinject(r.redNeg(), r.redIsOdd() ^ sign);
...
class EdwardsCurve extends Curve {
  ...
  _elligator1(t) {
    ...
    x.cinject(x.redNeg(), x.redIsOdd() ^ t.redIsOdd());
  ...
  _invert1(p) {
    ...
    x.cinject(x0, n.redMul(r).ceq(two.redNeg()));

    const nr1 = one.redAdd(n.redMul(r));
    const Xl = nr1.redNeg();
    const Xr = nr1.redSqr().redSub(one).redPow(e2);
    const X = Xl.redAdd(Xr);
    const z0 = c.redSub(one);
    const z1 = one.redAdd(X);
    const z2 = X.redSqr().redAdd(c2.redPow(e0));
    ...
    t.cinject(t.redNeg(), t.redIsOdd() ^ p.x.redIsOdd());
  ...
  pointFromUniform(curve, r) {
    assert(curve instanceof MontCurve);
    assert(r instanceof BN);
    assert(r.red === this.red);

    const u = r.fromRed().toRed(curve.red);
    const [m, sign] = curve.pointFromUniform(u);
  ...
  pointToUniform(curve, p) {
    ...
    const u = curve.pointToUniform(m, sign);
  ...
  pointFromHash(curve, bytes) {
    ...
    const r1 = this.decodeUniform(s1);
    const r2 = this.decodeUniform(s2);
    const p1 = this.pointFromUniform(curve, r1);
    const p2 = this.pointFromUniform(curve, r2);

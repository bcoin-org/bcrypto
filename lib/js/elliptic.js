...
class ShortCurve extends Curve {
  ...
  _mont(b0, sign) {
    ...
    const {three} = this;
    const b = (b0 || this.one).clone();
    const b2 = b.redSqr();
    const a2 = this.a.redMul(b2).redIMuln(-3).redIAdd(three);

    if (b.isZero() || a2.redJacobi() !== 1)
...
class MontCurve extends Curve {
  ...
  pointFromEdwards(point) {
    ...
    if (point.curve.twisted) {
      ...
      vz = x.redMul(uz);
    } else if (point.curve.iso4 && this.iso4) {
      // 4-isogeny maps:
      //
      //   u = y^2 / x^2
      //   v = (2 - x^2 - y^2) * y / x^3
...
class EdwardsCurve extends Curve {
  ...
  pointFromMont(point) {
    ...
    } else if (this.iso4 && point.curve.iso4) {
      // 4-isogeny maps:
      //

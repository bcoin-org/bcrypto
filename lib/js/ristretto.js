...
class Ristretto {
  constructor(curve) {
    ...
    // ADM1 = a * d - 1
    this.adm1 = this.ad.redSub(this.curve.one);
    ...
    if (this.curve.h.cmpn(8) === 0) {
      // - AS = sqrt(a)
      // + AS = -sqrt(a)
      this.as = this.curve.a.redSqrt().redNeg();
      ...
      // - IAMDS = 1 / sqrt(a - d)
      // + IAMDS = -1 / sqrt(a - d)
      this.iamds = this.amd.redSqrt().redInvert().redNeg();

      // ADM1S = sqrt(a * d - 1)
      this.adm1s = this.adm1.redSqrt();
    } else {
      // AS = -1
      ...
      // - ADM1S = sqrt(a * d - 1)
      // + ADM1S = -sqrt(a * d - 1)
      // Note: Should we negate for regular Ed448?
      this.adm1s = this.adm1.redSqrt().redNeg();
    }
  }

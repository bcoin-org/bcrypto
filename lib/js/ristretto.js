...
class Ristretto {
  ...
  encodeBatch(points) {
    ...
    for (let i = 0; i < products.length; i++) {
      const inv = acc.clone();
      const prod = products[i].clone();
      const zero = prod.czero();

      inv.cinject(this.curve.zero, zero);
      prod.cinject(this.curve.one, zero);
    ...
    for (let i = products.length - 1; i >= 0; i--) {
      ...
      prod.cinject(this.curve.one, prod.czero());

...
class Ristretto {
  constructor(curve) {
    ...
    // IADM1S = 1 / sqrt(a * d - 1)
    this.iadm1s = this.adm1s.redInvert();
    ...
    // DMADPA = DMA / DPA
    this.dmadpa = this.dma.redMul(this.dpa.redInvert());
    ...
    if (this.curve.h.cmpn(8) === 0) {
      ...
      // IDMAS = 1 / sqrt(d - a)
      this.idmas = this.dma.redSqrt().redInvert();
    } else {
      ...
      // IDMAS = 0 (unused)
      this.idmas = this.curve.zero;
    }

    // ASDS = AS * D
    this.asds = this.curve._mulD(this.as).redSqrt();
  ...
  _fix() {
    ...
    if (this.curve.id === 'ED25519'
        || this.curve.id === 'ED448') {
      this.iadm1s = this.iadm1s.redNeg();
    }
  }
  ...
  pointFromUniform(r0) {
    ...
    // S = sqrt(NS / D)
    // SQR = 1 if S is a root, 0 otherwise
  ...
  _quartic(p) {
    ...
    // H0 = 2 / sqrt(-d - 1) * Z
    const h0 = this.iadm1s.redMuln(2).redMul(z);
    ...
    const d1 = z2my2.redNeg().redMul(this.idmas).redMul(g);

    // IZ = I * Z
    const iz = this.as.redMul(z);
    ...
    // H1 = (2 / sqrt(a * d - 1)) * IZ
    const h1 = this.iadm1s.redMuln(2).redMul(iz);
    ...
    // H2 = I / sqrt(a * d - 1)
    const h2 = this.as.redMul(this.iadm1s);
  ...
  _invert(s, t) {
    ...
    // A = (T + 1) * ((d - a) / (d + a))
    const a = t.redAdd(one).redMul(this.dmadpa);
    ...
    // Y = 1 / sqrt(I * (S4 - A2))
    // SQR = 1 if Y is a root, 0 otherwise
    const [sqr, y] = this._invsqrt(this.as.redMul(s4.redSub(a2)));

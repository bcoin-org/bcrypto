...
class Ristretto {
  constructor(curve) {
    ...
    // DMADDPA = DMA / DPA
    this.dmaddpa = this.dma.redMul(this.dpa.redInvert());
  ...
  _encode4(p) {
    ...
    const t = this.twisted ? p.t : p.x.redMul(p.y).redMul(p.z.redInvert());
  ...
  _quartic(p) {
    ...
    // H2 = qnr / sqrt(a * d - 1)
    const h2 = this.qnr.redMul(this.iadm1s);
  ...
  pointToHash(p, rng) {
    ...
    for (;;) {
      ...
      const hint = BN.randomBits(rng, 3).word(0);

...
class Ristretto {
  constructor(curve) {
    ...
    if (this.curve.h.cmpn(8) === 0) {
      // Note: we get the inverse of sqrt(a) in order
      // to use the same square root as curve25519-dalek.
      // This is necessary for the elligator encoding
      ...
      // Note: get the negation in order for elligator
      // to behave the same as ed448-goldilocks.
  ...
  isqrt(u, v) {
    ...
    if (this.curve.p.andln(3) === 3)
      return this._isqrt43(u, v);

    // p mod 8 == 5 (p25519)
    if (this.curve.p.andln(7) === 5)
      return this._isqrt85(u, v);

 ...
 *   https://github.com/dalek-cryptography/curve25519-dalek/blob/9a62386/src/field.rs
...
class Ristretto {
  ...
  _encode4(p) {
    ...
    // Zinv = I^2 * U * Y0 * T0
    const zinv = i.redSqr().redMul(u).redMul(p.y).redMul(p.t);

    // Y = Y0
    const y = p.y.clone();

    // Y = -Y if Zinv < 0
    y.cinject(y.redNeg(), zinv.redIsOdd() | 0);
  ...
  _encode8(p) {
    ...
    if (this.curve.mOneA) {
      ...
      // X = -Y0 if rotate = 1
      x.cinject(p.y.redNeg(), rotate);

      // Y = X0 if rotate = 1
      y.cinject(p.x, rotate);

      // D = (D1 / sqrt(a - d)) * qnr if rotate = 1
      d.cinject(d1.redMul(this.amdsi).redMul(this.qnr), rotate);
    }
  ...
  decode(bytes) {
    ...
    // Fail if V * U2^2 is not square or T < 0 or Y = 0.
  ...
  encodeBatch(points) {
    ...
    const invs = new Array(products.length);

    let acc = this.curve.one;

    for (let i = 0; i < products.length; i++) {
      if (products[i].isZero()) {
        invs[i] = this.curve.zero;
        continue;
      }

      invs[i] = acc;
      acc = acc.redMul(products[i]);
    }
    ...
    for (let i = 0; i < states.length; i++) {
      ...
      if (this.curve.h.cmpn(8) === 0) {
        const rotate = eg.redMul(zinv).redIsOdd() | 0;
        const minuse = e.redNeg();
        const fqnr = this.curve.mOneA ? f.redMul(this.qnr) : f.redNeg();

        e.cinject(g, rotate);
        g.cinject(minuse, rotate);
        h.cinject(fqnr, rotate);

        magic.cinject(this.qnr, rotate);
      } else {
        magic.inject(this.adm1si);
      }

      g.cinject(g.redNeg(), h.redMul(e).redMul(zinv).redIsOdd() | 0);
  ...
  pointToUniform(p, hint) {
    ...
    const [ok, r] = this._invert(s, t, sign);
  ...
  _invert(s, t, hint) {
    ...
    // R = 0 if S = 0
    r.cinject(zero, szero);

    // R = sqrt(qnr * d) if S = 0, T = 1
    r.cinject(this.qnrds, szero & tone);

    // R = -R if R < 0 (or random)
    r.cinject(r.redNeg(), r.redIsOdd() ^ (hint & 1));

...
/**
 * PKCS1v1.5+ASN1 DigestInfo prefixes.
...
const digestInfo = {
  ...
  // blake2b160:
  //   Buffer.from('3027300f060b2b060104018d3a0c02010505000414', 'hex'),
  // blake2b256:
  //   Buffer.from('3033300f060b2b060104018d3a0c02010805000420', 'hex'),
  // blake2b384:
  //   Buffer.from('3043300f060b2b060104018d3a0c02010c05000430', 'hex'),
  // blake2b512:
  //   Buffer.from('3053300f060b2b060104018d3a0c02011005000440', 'hex'),
  // blake2s128:
  //   Buffer.from('3023300f060b2b060104018d3a0c02020405000410', 'hex'),
  // blake2s160:
  //   Buffer.from('3027300f060b2b060104018d3a0c02020505000414', 'hex'),
  // blake2s224:
  //   Buffer.from('302f300f060b2b060104018d3a0c0202070500041c', 'hex'),
  // blake2s256:
  //   Buffer.from('3033300f060b2b060104018d3a0c02020805000420', 'hex')
};
...
rsa.sign = function sign(hash, msg, key) {
  ...
  if (!digestInfo.hasOwnProperty(hash))
    throw new Error('Unknown PKCS1v1.5 padding.');

  if (msg.length !== hashSizes[hash])
  ...
  const c = decrypt(key, em);

  // https://github.com/openssl/openssl/blob/82eba37/crypto/rsa/rsa_ossl.c#L356
  copyLeftPad(em, c);
...
rsa.verify = function verify(hash, msg, sig, key) {
  ...
  // https://github.com/openssl/openssl/blob/82eba37/crypto/rsa/rsa_sign.c#L207
  if (msg.length !== hashSizes[hash])
    return false;

  // https://github.com/openssl/openssl/blob/82eba37/crypto/rsa/rsa_sign.c#L131
  // https://github.com/openssl/openssl/blob/82eba37/crypto/rsa/rsa_ossl.c#L538
...
// eslint-disable-next-line func-name-matching
rsa.decrypt = function _decrypt(msg, key) {
  assert(Buffer.isBuffer(msg));
  assert(key instanceof RSAPrivateKey);

  // https://github.com/openssl/openssl/blob/82eba37/crypto/rsa/rsa_ossl.c#L397
...
rsa.encryptOAEP = function encryptOAEP(hash, msg, key, label) {
  ...
  if (!digestInfo.hasOwnProperty(hash.id))
...
rsa.signPSS = function signPSS(hash, msg, priv, saltLen = -1) {
  assert(hash && typeof hash.id === 'string');
  assert(Buffer.isBuffer(msg));
  ...
  if (!digestInfo.hasOwnProperty(hash.id))
    throw new Error('Unknown PSS hash function.');
  ...
  if (saltLen === 0) // Auto
    saltLen = priv.size() - 2 - hash.size;
  else if (saltLen === -1) // Equals
    saltLen = hash.size;

  const salt = random.randomBytes(saltLen);
  const nbits = priv.bits();
  const em = pssEncode(hash, msg, nbits - 1, salt);
  const c = decrypt(priv, em);
  const s = Buffer.allocUnsafe((nbits + 7) >>> 3);
...
rsa.verifyPSS = function verifyPSS(hash, msg, sig, pub, saltLen = -1) {
  assert(hash && typeof hash.id === 'string');
  assert(Buffer.isBuffer(msg));
  assert(Buffer.isBuffer(sig));
  assert(pub instanceof RSAKey);
  assert(saltLen === -1 || (saltLen >>> 0) === saltLen);
  ...
  const nbits = pub.bits();
  const m = encrypt(pub, sig);
  const embits = nbits - 1;

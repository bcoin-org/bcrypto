...
function sign(hash, msg, key, decipher = decryptRaw) {
  ...
  // PS = emLen - tLen - 3 octets of 0xff
...
function encrypt(msg, key, encipher = encryptRaw) {
  ...
  // PS = k - mLen - 3 random nonzero octets
...
function encryptOAEP(hash, msg, key, label, encipher = encryptRaw) {
  ...
  xormgf1(hash, db, seed);
  xormgf1(hash, seed, db);
...
function decryptOAEP(hash, msg, key, label, decipher = decryptRaw) {
  ...
  // DB = maskedDB \xor MGF(seed, k - hLen - 1)
  // DB = Hash(L) || PS || 0x01 || M
  const em = decipher(msg, key);
  ...
  if (!(fbiz & lvalid & (invalid ^ 1) & (looking ^ 1)))
...
function generateKey(bits, exponent) {
  ...
  for (;;) {
    ...
    const q = randomPrime(bits - p.bitLength());
...
function pssEncode(hash, msg, embits, salt) {
  ...
  // H = Hash((0x)00 00 00 00 00 00 00 00 || mHash || salt)
  // PS = emLen - sLen - hLen - 2 zero octets
  // DB = PS || 0x01 || salt
  // maskedDB = DB \xor MGF(H, emLen - hLen - 1)
  // EM = maskedDB || H || 0xbc
...
function pssVerify(hash, msg, em, embits, slen) {
  ...
  // DB = maskedDB \xor MGF(H, emLen - hLen - 1)
  // PS = emLen - hLen - sLen - 2 leftmost octets of DB
  // salt = the last sLen octets of DB
  // H' = Hash((0x)00 00 00 00 00 00 00 00 || Hash(M) || salt)
  const db = em.slice(0, emlen - hlen - 1);
  const h = em.slice(emlen - hlen - 1, emlen - 1);
  const mask = 0xff >>> (8 * emlen - embits);
...
function xormgf1(hash, out, seed) {

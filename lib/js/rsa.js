...
rsa.encryptRaw = function encryptRaw(msg, key) {
  ...
  return c.toBuffer('be', n.byteLength());
};
...
rsa.decryptRaw = function decryptRaw(msg, key) {
  ...
  if (needsCompute(key)) {
    ...
    const m1 = c.powm(dp, p);
    const m2 = c.powm(dq, q);
  ...
  return m.toBuffer('be', n.byteLength());
};
...
rsa.veil = function veil(msg, bits, key) {
  ...
  return c1.toBuffer('be', (bits + 7) >>> 3);
};

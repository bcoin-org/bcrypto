...
class RSAPublicKey {
  ...
  getSize() {
    ...
    size += 1 + asn1.sizeSize(size);

    return size;
  }

  encode() {
    const out = Buffer.alloc(this.getSize());
    ...
    out[pos++] = 0x30;
    pos = asn1.writeSize(out, pos, size);
...
class RSAPrivateKey extends RSAPublicKey {
  ...
  verify() {
    ...
    if (this.e.mul(this.d).imod(pm1.lcm(qm1)).cmpn(1) !== 0)
      return false;
  ...
  decode(data) {
    ...
    // Read version first.
    [version, pos] = asn1.readInt(data, pos);

    // Should be zero.
    if (version.sign() !== 0)
      throw new Error('Version must be zero.');

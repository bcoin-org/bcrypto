...
rsa.generate = function generate(bits, exponent) {
  ...
  const [key] = generateMulti(2, bits, exponent);
...
rsa.generateAsync = async function generateAsync(bits, exponent) {
  ...
  try {
    ...
    return rsa.generate(bits, exponent);
  }
};
...
rsa.decryptKey = function decryptKey(msg, out, key) {
  ...
  const [valid, em, index] = rsa._decrypt(msg, key);
...
function generateMulti(total, bits, exponent) {

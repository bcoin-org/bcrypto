...
rsa.publicKeyImportSPKI = function publicKeyImportSPKI(raw) {
  ...
  return rsa.publicKeyImport(spki.subjectPublicKey.rightAlign());
};
...
rsa.sign = function sign(hash, msg, key) {
  ...
  const [prefix, hlen] = getDigestInfo(hash);
  ...
  return this.decryptCheck(key, em);
};
...
rsa._verify = function _verify(hash, msg, sig, key) {
  ...
  const em = this.encryptRaw(key, sig);
...
rsa.decrypt = function decrypt(msg, key) {
  ...
  // EM = 0x00 || 0x02 || PS || 0x00 || M
  const em = this.decryptRaw(key, msg);
...
next:
  for (;;) {
    ...
    for (let i = 0; i < total; i++) {
      ...
      const prime = this.randomPrime(size);
...
rsa.randomPrime = function _randomPrime(bits) {
...
rsa.encryptRaw = function encryptRaw(key, msg) {
  ...
  return toBuffer(c, n.byteLength());
};
...
rsa.decryptRaw = function decryptRaw(key, msg) {
  ...
  return toBuffer(m, n.byteLength());
};

/**
 * Raw decryption with sanity checking.
 * @private
 * @param {RSAPrivateKey} key
 * @param {Buffer} msg
 * @returns {Buffer}
...
rsa.decryptCheck = function decryptCheck(key, msg) {
  const m = this.decryptRaw(key, msg);
  const c = this.encryptRaw(key, m);
...
function hasDigestInfo(name) {
  return getDigestInfo(name)[0] !== null;
}

...
class BasePoint {
  ...
  mulBlind(rng, k) {
    ...
    // Try to multiply with a blinding factor.
    for (;;) {
      const b = BN.random(rng, 1, this.curve.n);
      const bn = this.curve.n.sub(b);
      const kb = k.add(b).umod(this.curve.n);

      if (kb.isZero())
        continue;

      // Could in theory optimize with:
      // this.mulAdd(kb, this, bn);

      const kbp = this.mul(kb);

      if (kbp.isInfinity())
        continue;

      const bnp = this.mul(bn);
...
class MontPoint extends BasePoint {
  ...
  mulBlind(rng, k) {
    ...
    for (; !t.isZero(); t.iushrn(1))
      bits.push(t.andln(1));

    for (let i = bits.length - 1; i >= 0; i--) {
      [a, b] = this._swap(bits[i], a, b);

...
class Curve {
  ...
  _wnafMulAdd(defW, points, coeffs) {
    assert((defW >>> 0) === defW);
    assert(Array.isArray(points));
    assert(Array.isArray(coeffs));
    assert(points.length === coeffs.length);
    ...
    for (let i = len - 1; i >= 1; i -= 2) {
      ...
      // Compute width.
      const awidth = points[a]._getWidth(defW);
      const bwidth = points[b]._getWidth(defW);

      // Compute regular NAF.
      if (awidth !== 1 || bwidth !== 1) {
        wnd[a] = points[a]._getPoints(defW);
        wnd[b] = points[b]._getPoints(defW);
...
class Point {
  ...
  _getWidth(wnd) {
    ...
    if (this.precomputed && this.precomputed.naf)
      return this.precomputed.naf.wnd;

    return wnd;
  }

  _getPoints(wnd) {
    return this._getNAF(wnd).points;
  }

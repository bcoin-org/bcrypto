  ...
  decodePoint(bytes) {
    ...
    switch (first) {
      ...
      case 0x03: {
        ...
        return this.pointFromX(x, first === 0x03);
      }
...
class Mask {
  ...
  clamp(bytes) {
    if (this.curve.endian === 'be')
      return this.clampBE(bytes);
    return this.clampLE(bytes);
  }

  verify(bytes) {
    if (this.curve.endian === 'be')
      return this.verifyBE(bytes);
    return this.verifyLE(bytes);
  }
  ...
  clampBE(bytes) {
    ...
    // Slice if we need to.
    if (bytes.length !== scalarLength)
      return bytes.slice(i, i + scalarLength);
  ...
  verifyBE(bytes) {
    ...
    // Must be a multiple of the cofactor.
    if (bytes[i + scalarLength - 1] & ~this.h)
      return false;

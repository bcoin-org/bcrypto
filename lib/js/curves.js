...
class ShortCurve extends Curve {
  ...
  pointFromX(x, odd) {
    ...
    if (y.redSqr().redISub(y2).cmp(this.zero) !== 0)
      throw new Error('Invalid point.');
  ...
  _endoWnafMulAdd(points, coeffs, initial, jacobian) {
    ...
    return this._wnafMulAdd(1, npoints, ncoeffs, i * 2, initial, jacobian);
  }
  ...
  _mulAll(points, coeffs, initial, jacobian) {
    ...
    if (this.endo && len > 0 && points[0].type === 'affine')
      return this._endoWnafMulAdd(points, coeffs, initial, jacobian);

    return this._wnafMulAdd(1, points, coeffs, len, initial, jacobian);
  }
...
class ShortPoint extends Point {
  ...
  _mulAdd(k1, p2, k2, initial, jacobian) {
    ...
    return this.curve._wnafMulAdd(1, points, coeffs, 2, initial, jacobian);
  }
...
class JPoint extends Point {
  ...
  _mulAdd(k1, p, k2, initial, jacobian) {
    ...
    return this.curve._wnafMulAdd(1, [this, p], [k1, k2], 2, initial, jacobian);
  }
...
class MontCurve extends Curve {
  ...
  validate(point) {
    ...
    const x2 = x.redSqr();
    const rhs = x2.redMul(x).redIAdd(x2.redMul(this.a)).redIAdd(x);
    const y = rhs.redSqrt();
...
class EdwardsCurve extends Curve {
  ...
  pointFromY(y, odd) {
    ...
    if (x.redSqr().redISub(x2).cmp(this.zero) !== 0)
      throw new Error('Invalid point.');

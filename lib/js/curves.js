...
class Curve {
  ...
  _wnafMulAdd(defW, points, coeffs, initial, jacobian) {
    ...
    for (let i = 0; i < len; i++) {
      ...
      size = Math.max(coeff.bitLength() + 1, size);
    }
...
class MontPoint extends Point {
  ...
  diffAddDbl(p, diff) {
    ...
    // A = X2+Z2
    const a = x2.redAdd(z2);

    // AA = A^2
    const aa = a.redSqr();

    // B = X2-Z2
    const b = x2.redSub(z2);
    ...
    // C = X3+Z3
    const c = x3.redAdd(z3);

    // D = X3-Z3
    const d = x3.redSub(z3);
  ...
  _mul(k, initial, jacobian) {
    ...
    for (let i = bits - 1; i >= 0; i--) {
      ...
      if (bit === 0) {
        // N * Q + Q = ((N / 2) * Q + Q)) + (N / 2) * Q
        // N * Q = 2 * ((N / 2) * Q + Q))
        [a, b] = a.diffAddDbl(b, c);
      } else {
        // N * Q = ((N / 2) * Q + Q) + ((N / 2) * Q)
        // N * Q + Q = 2 * ((N / 2) * Q + Q)
        [b, a] = b.diffAddDbl(a, c);
      }
    }

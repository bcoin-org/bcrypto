...
class Curve {
  ...
  _simpleMulAdd(points, coeffs, initial, jacobian) {
    ...
    for (let i = 0; i < points.length; i += 2) {
      ...
      const p3 = p1.toJ().add(p2);
      const ps = [p, p1, p2, p3];

      for (let i = bits - 1; i >= 0; i--) {
        const b1 = k1.utestn(i);
        const b2 = k2.utestn(i);
  ...
  _constMul(p, k, initial, jacobian) {
    // Use Co-Z arithmetic for Weierstrass.
    if (this.type === 'short')
      return this._coZLadderMul(p, k, initial, jacobian);

    // Otherwise, a regular ladder.
    return this._ladderMul(p, k, initial, jacobian);
  }

  _ladderMul(p, k, initial, jacobian) {
    assert(p instanceof Point);
    assert(k instanceof BN);
    assert(!k.red);
    assert(initial == null || (initial instanceof Point));
    ...
    const bits = Math.max(k.bitLength(), this.scalarBits);
    const bytes = (bits + 7) >>> 3;

    // Recode scalar to base256.
    const b256 = k.toArray('le', bytes);
    ...
    // Unblind (this branch is not secret).
    if (initial)
      b = b.uadd(initial);
    ...
    return p.type === types.AFFINE ? b.toP() : b;
  }

  _coZLadderMul(p, k, initial, jacobian) {
    assert(p instanceof Point);
    assert(k instanceof BN);
    assert(!k.red);
    assert(initial == null || (initial instanceof Point));
    ...
    //
    // Multiply using Co-Z arithmetic. Note
    // that while this method is twice as
    // fast as our regular ladder with unified
    // addition, it leaks the length of the
    // key (probably 1 bit of information on
    // average, assuming the hi bit has a 50%
    // chance of being set when we're dealing
    // with uniformally random scalars).
    //
    // A possibility for a perf improvement
    // involves using the ZACAU method. This
    // is faster assuming a cost of 1S < 1M,
    // but our squaring implementation is
    // identical to our multiplication
    // implementation, so we wouldn't get
    // any real benefit.
    //
    // Current cost:
    //
  ...
  _wnafMulAdd(defW, points, coeffs, initial, jacobian) {
    ...
    // Multiple point multiplication, also known
    // as "Shamir's trick" (interleaved with NAFs).
    //
    // See: Guide to Elliptic Curve Cryptography.
    // Algorithm 3.48, page 109, section 3.3.3.
    ...
    for (let i = len - 1; i >= 1; i -= 2) {
      ...
      if (type === types.AFFINE) {
        if ((points[a].inf | points[b].inf) === 0) {
          if (points[a].y.cmp(points[b].y) === 0) {
            ...
            comb[2] = points[a].toJ().add(points[b].neg());
          } else if (points[a].y.cmp(points[b].y.redNeg()) === 0) {
            comb[1] = points[a].toJ().add(points[b]);
      ...
      // Add comb to window a, ignore b.
      wnd[a] = comb;

      // Get JSF in NAF form (naf[b] always equals 0).
...
class Point {
  ...
  _mulConst(k, jacobian) {
    const blinding = this._getBlinding(null);

    if (!blinding)
      return this.curve._constMul(this, k, null, jacobian);

    // Blind if we have it.
    const {blind, unblind} = blinding;
    const t = k.sub(blind);

    return this.curve._constMul(this, t, unblind, jacobian);
  }
  ...
  mulConst(k) {
    return this._mulConst(k, false);
  }

  jmulConst(k) {
    return this._mulConst(k, true);
  }
...
class JPoint extends Point {
  ...
  _uadd(p) {
    ...
    // Generally only happens on endomorphic curves.
    const degenerate = (m.sign() | r.sign()) ^ 1;

    // M = U1 - U2 (if degenerate = 1)
    m.condInject(u1.redSub(u2), degenerate);

    // R = S1 - S2 (if degenerate = 1)
    r.condInject(s1.redSub(s2), degenerate);
    ...
    // Check for infinity.
    const ai = this.z.sign() ^ 1;
    const bi = p.z.sign() ^ 1;
    const fi = f.sign() ^ 1;
    const ni = fi & (ai ^ 1) & (bi ^ 1);

    // X3 = X1 (if Z2 = 0)
    nx.condInject(this.x, bi);

    // Y3 = Y1 (if Z2 = 0)
    ny.condInject(this.y, bi);

    // Z3 = Z1 (if Z2 = 0)
    nz.condInject(this.z, bi);

    // X3 = X2 (if Z1 = 0)
    nx.condInject(p.x, ai);

    // Y3 = Y2 (if Z1 = 0)
    ny.condInject(p.y, ai);

    // Z3 = Z2 (if Z1 = 0)
    nz.condInject(p.z, ai);

    // X3 = 1 (if Z3 = 0, Z1 != 0, Z2 != 0)
    nx.condInject(one, ni);

    // Y3 = 1 (if Z3 = 0, Z1 != 0, Z2 != 0)
    ny.condInject(one, ni);

    // Z3 = 0 (if Z3 = 0, Z1 != 0, Z2 != 0)
    nz.condInject(zero, ni);

    assert(((ai & bi) ^ 1) | (nz.sign() ^ 1));
  ...
  zaddu(p) {
    ...
    // A = X1 - X2
    const a = this.x.redSub(p.x);

    // B = A^2
    const b = a.redSqr();

    // C = B * X2
    const c = b.redMul(p.x);

    // D = Y1 - Y2
    const d = this.y.redSub(p.y);

    // X4 = X1 * B
    const x4 = this.x.redMul(b);

    // X3 = D^2 - X4 - C
    const x3 = d.redSqr().redISub(x4).redISub(c);

    // Y4 = Y1 * (X4 - C)
    const y4 = this.y.redMul(x4.redSub(c));

    // Y3 = D * (X4 - X3) - Y4
    const y3 = d.redMul(x4.redSub(x3)).redISub(y4);

    // Z4 = Z1 * A
    const z4 = this.z.redMul(a);
  ...
  zaddc(p) {
    ...
    // B = Y1 - Y2
    const b = this.y.redSub(p.y);

    // C = Y1 + Y2
    const c = this.y.redAdd(p.y);

    // D = A^2
    const d = a.redSqr();

    // E = X1 * D
    const e = this.x.redMul(d);

    // F = D * X2
    const f = d.redMul(p.x);

    // X4 = (Y1 + Y2)^2 - E - F
    const x4 = c.redSqr().redISub(e).redISub(f);

    // X3 = B^2 - E - F
    const x3 = b.redSqr().redISub(e).redISub(f);

    // G = (F - E) * Y1
    const g = f.redISub(e).redMul(this.y);

    // H = B + 2 * Y2
    const h = p.y.redMuln(2).redIAdd(b);

    // I = E - X4
    const i = e.redISub(x4);

    // J = X4 + I - X3
    const j = x4.redAdd(i).redISub(x3);

    // Y4 = H * I + G
    const y4 = h.redMul(i).redIAdd(g);

    // Y3 = B * J + G
    const y3 = b.redMul(j).redIAdd(g);
  ...
  zdblu() {
    ...
    // A = Y1^2
    const a = this.y.redSqr();

    // B = (X1 + A)^2
    const b = this.x.redAdd(a).redSqr();
    ...
    // X4 = 2 * (B - C - D)
    const x4 = b.redISub(c).redISub(d).redIMuln(2);

    // E = C + a + (2 * C)
    const e = c.redAdd(this.curve.a).redIAdd(c.redIMuln(2));

    // X3 = E^2 - (2 * X4)
    const x3 = e.redSqr().redISub(x4.redMuln(2));

    // Y4 = 8 * D
    const y4 = d.redIMuln(8);

    // Y3 = (X4 - X3) * E - Y4
    const y3 = x4.redSub(x3).redMul(e).redISub(y4);

    // Z4 = 2 * Y1
    const z4 = this.y.redMuln(2);
  ...
  swap(point, flag) {
    ...
    this.x.condSwap(point.x, flag);
    this.y.condSwap(point.y, flag);
    this.z.condSwap(point.z, flag);
...
class MontPoint extends Point {
  ...
  neg() {
    // Note: Montgomery curves are single coordinate.
    // They have no concept of a sign (in other words,
    // the curve has only one "side"). Conceptually,
    // a point's negation would be identical to the
    // original point.
    return this;
  }
...
function getFixedNAF(k, w, size, step) {
  assert((step >>> 0) === step);

  // Recode to NAF.
  const naf = getNAF(k, w, size);

...
class ShortPoint extends Point {
  ...
  dbl() {
    ...
    // C = (XX * 3 + A) / YS
    const c = xx.redIMuln(3).redIAdd(this.curve.a).redMul(ys.redInvert());
...
class JPoint extends Point {
  ...
  dblp(pow) {
    ...
    let jyd = this.y.redMuln(2);

    // Not sure where @indutny got this from, but it seems to work.
    // I'd like to document it more clearly, but it seems slightly
    // different from all of the doubling formulas on EFD.
    for (let i = 0; i < pow; i++) {
      const jx2 = jx.redSqr();
  ...
  _zeroDbl() {
    ...
    if (this.zOne) {
      ...
      // Y3 = M * (S - T) - 8 * YYYY
      ny = m.redMul(s.redISub(t)).redISub(yyyy.redIMuln(8));
      ...
      // Y3 = E * (D - X3) - 8 * C
      ny = e.redMul(d.redISub(nx)).redISub(c.redIMuln(8));
  ...
  _threeDbl() {
    ...
    if (this.zOne) {
      ...
      ny = alpha.redMul(beta4.redISub(nx)).redISub(gamma.redSqr().redIMuln(8));
    }

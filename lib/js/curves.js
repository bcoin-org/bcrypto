...
class Curve {
  ...
  _simpleMulAdd(points, coeffs) {
    ...
    const npoints = new Array(points.length);
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      max = Math.max(coeff.bitLength(), max);
    }
  ...
  _wnafMulAdd(width, points, coeffs) {
    ...
    for (let i = len - 1; i >= 1; i -= 2) {
      ...
      // Compute regular NAF.
      const nafA = points[a]._getNAF(width);
      const nafB = points[b]._getNAF(width);

      // Use precomputation if available.
      if (nafA.width !== 1 || nafB.width !== 1) {
        wnd[a] = nafA.points;
        wnd[b] = nafB.points;
        naf[a] = getNAF(coeffs[a], nafA.width, max);
        naf[b] = getNAF(coeffs[b], nafB.width, max);
        continue;
      }
    ...
    // Compute final NAF (if odd).
    if (len & 1) {
      const naf0 = points[0]._getNAF(width);

      wnd[0] = naf0.points;
      naf[0] = getNAF(coeffs[0], naf0.width, max);
    }

...
class BaseCurve {
  constructor() {
    ...
    this.n = new BN(0);
    this.nh = new BN(0);
    this.g = new BasePoint(this, null);
    this._wnafT1 = [];
    this._wnafT2 = [];
    this._wnafT3 = [];
    this._wnafT4 = [];
    ...
    this.scalarLength = 0;
    this.zeroRaw = this.prefix;
    this.orderRaw = this.prefix;
    this.halfRaw = this.prefix;
    this.schnorr = null;
    this.native = 0;
  }
  ...
  init(type, conf) {
    ...
    this.orderRaw = this.encodeInt(this.n);
    this.halfRaw = this.encodeInt(this.nh);
  }
...
class MontPoint extends BasePoint {
  ...
  encode() {
    return this.getX().toBuffer('le', this.curve.p.byteLength());
  }
...
class X25519 extends MontCurve {
  ...
  decodePoint(bytes) {
    ...
    if (bytes.length === this.p.byteLength()) {
...
class ED25519 extends EdwardsCurve {
  ...
  clamp(data) {
    ...
    assert(data.length >= this.scalarLength);

    data[0] &= 248;
    data[this.scalarLength - 1] &= 127;
    data[this.scalarLength - 1] |= 64;

    return data.slice(0, this.scalarLength);
  }
...
class ED448 extends EdwardsCurve {
  ...
  clamp(data) {
    ...
    data[0] &= ~3;
    data[this.scalarLength - 1] |= 0x80;

    if (data.length > this.scalarLength)
      data[this.scalarLength] = 0;

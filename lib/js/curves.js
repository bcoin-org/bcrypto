...
class ShortPoint extends Point {
  ...
  dbl() {
    ...
    // C = (XX * 3 + A) / Y2
    const c = xx.redIMuln(3).redIAdd(this.curve.a).redMul(y2.redInvert());
...
class JPoint extends Point {
  ...
  mixedAddOld(p) {
    ...
    // 8M + 3S + 6A + 5*2 (implemented as: 8M + 3S + 7A)
    const z2 = this.z.redSqr();
    const u1 = this.x;
    const u2 = p.x.redMul(z2);
    const s1 = this.y;
  ...
  _zeroDbl() {
    ...
    if (this.zOne) {
      ...
      // (implemented as: 1M + 5S + 7A + 2*2 + 1*3 + 1*8)
      ...
      // D = 2 * ((X1 + B)^2 - A - C)
      const d = this.x.redAdd(b).redSqr()
                      .redISub(a).redISub(c)
                      .redIMuln(2);
  ...
  _threeDbl() {
    ...
    // Z = 1
    if (this.zOne) {
      // https://hyperelliptic.org/EFD/g1p/auto-shortw-jacobian-3.html#doubling-mdbl-2007-bl
      // 1M + 5S + 7A + 3*2 + 1*3 + 1*8
  ...
  _dblOld() {
    ...
    // (implemented as: 4M + 6S + 5A + 1*a + 1*2 + 1*3 + 1*4 + 1*8)
    const jx = this.x;
    const jy = this.y;
    const jz = this.z;
  ...
  trpl() {
    ...
    // (implemented as: 5M + 10S + 14A + 1*3 + 2*4 + 1*6 + 1*8 + 1*16)

...
class Curve {
  ...
  _coZLadderMul(p, k) {
    ...
    // Negate scalar.
    const n = this.n.sub(c);

    // Check bits < order bits and bits < neg bits.
    const small = ((c.bitLength() - this.scalarBits) >>> 31)
                & ((c.bitLength() - n.bitLength()) >>> 31);

    // Track new sign.
    const sign = k.isNeg() ^ small;

    // Possibly negate.
    c.cinject(n, small);

    // Calculate the new scalar's length.
    const bits = c.bitLength();
    const bytes = (bits + 7) >>> 3;

    // Recode scalar to base256.
    const b256 = c.toArray('le', bytes);
  ...
  _wnafMul(p, k) {
    ...
    // Precompute window.
    const {width, points} = p._getNAF(4);
  ...
  _wnafMulAdd(width, points, coeffs) {
    ...
    for (let i = len - 1; i >= 1; i -= 2) {
      ...
      // Compute width.
      const awidth = points[a]._getWidth(width);
      const bwidth = points[b]._getWidth(width);

      // Compute regular NAF.
      if (awidth !== 1 || bwidth !== 1) {
        wnd[a] = points[a]._getPoints(width);
        wnd[b] = points[b]._getPoints(width);
        naf[a] = getNAF(coeffs[a], awidth, size);
        naf[b] = getNAF(coeffs[b], bwidth, size);
        continue;
      }

      // Comb small window NAFs.
      const comb = points[a]._getComb(points[b]);

      // Get JSF in NAF form (naf[b] always equals 0).
      const [j1, j2] = getJSF(coeffs[a], coeffs[b], size);

      // Note: we ignore window b.
      wnd[a] = comb;
      wnd[b] = comb;
      naf[a] = j1;
      naf[b] = j2;
...
class Point {
  ...
  _getWidth(width) {
    ...
    if (this.precomputed && this.precomputed.naf)
      return this.precomputed.naf.width;

    return width;
  }

  _getPoints(width) {
    return this._getNAF(width).points;
  }
  ...
  mulAll(k, points, coeffs) {
    return this.jmulAll(k, points, coeffs);
  }

  mulAllSimple(k, points, coeffs) {
    return this.jmulAllSimple(k, points, coeffs);
  }
  ...
  jmulAll(k, points, coeffs) {
    const p1 = this.jmul(k);
    const p2 = this.curve.jmulAll(points, coeffs);
    return p1.add(p2);
  }

  jmulAllSimple(k, points, coeffs) {
    const p1 = this.jmulSimple(k);
    const p2 = this.curve.jmulAllSimple(points, coeffs);
    return p1.add(p2);
  }
}
...
class ShortPoint extends Point {
  ...
  _getComb(point) {
    ...
    if (this.inf | point.inf)
      return this.toJ()._getComb(point.toJ());
  ...
  mulAll(k, points, coeffs) {
    return super.mulAll(k, points, coeffs).toP();
  }

  mulAllSimple(k, points, coeffs) {
    return super.mulAllSimple(k, points, coeffs).toP();
  }
...
class JPoint extends Point {
  ...
  zdblu() {
    ...
    // 1M + 5S + 8A + 4*2 + 1*8
    //
    // Note that the original function assumed Z1=1.
    // We have modified it to allow for scaled points.
    //
    // New Cost: 2M + 5S + 8A + 1*a + 4*2 + 1*8
    ...
    // - E = C + a + (2 * C)
    // + E = C + a * Z1^4 + (2 * C)
    const zzzz = this.z.redSqr().redSqr();
    const e = c.redAdd(this.curve.a.redMul(zzzz)).redIAdd(c.redIMuln(2));
...
function getNAF(c, width, size) {
  ...
  while (k.cmpn(1) >= 0) {
    ...
    const shift = (k.sign() !== 0 && k.andln(ws - 1) === 0) ? (width + 1) : 1;
...
function getFixedNAF(k, width, size, step) {
  assert((step >>> 0) === step);

  // Recode to NAF.
  const naf = getNAF(k, width, size);

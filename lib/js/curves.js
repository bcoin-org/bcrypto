...
class Point {
  ...
  mulBlind(k) {
    ...
    const t = k.add(blind).umod(this.curve.n);

    return this.mul(t).add(unblind);
  }
...
class MontPoint extends Point {
  ...
  mul(k) {
    ...
    for (let i = bits - 1; i >= 0; i--) {
      ...
      [a, b] = swapConst(swap, a, b);

      swap = bit;

      // 0: N * Q + Q = ((N / 2) * Q + Q)) + (N / 2) * Q
      // 1: N * Q = ((N / 2) * Q + Q) + ((N / 2) * Q)
      a = a.diffAdd(b, c);
...
function swapConst(bit, a, b) {
  // This is probably the best we can do in JS.
  // Might be better to clone the points for
  // each iteration.

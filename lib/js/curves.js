 ...
 *     by D. Hankerson, A. Menezes, and S. Vanstone.
 *
...
class Point {
  ...
  _getBlinding(rng) {
    ...
    // from scalars before multiplication.
    for (;;) {
...
class ShortCurve extends Curve {
  ...
  _getEndoRoots(num) {
    ...
    // See: Guide to Elliptic Curve Cryptography,
    // Example 3.76, page 128, section 3.
    //
    ...
    //
    // For the record, I'm not sure where or how
    // @indutny found the method below, but it works
    // well. As it happens, this method combined
    // with above code derives the exact same roots
    // derived by Hal Finney.
    //
    ...
    // R1 = -(1 / 2) + S
    const r1 = ntinv.redAdd(s).fromRed();

    // R2 = -(1 / 2) - S
    const r2 = ntinv.redISub(s).fromRed();
  ...
  _getEndoBasis(lambda) {
    ...
    const [rl, tl, rl1, tl1, rl2, tl2] = this._endoEGCD(lambda);
  ...
  _endoEGCD(lambda) {
    ...
    // See: Guide to Elliptic Curve Cryptography
    // Algorithm 2.19, page 40, section 2.
    //
  ...
  _endoSplit(k) {
    ...
    // 4. Compute c1 = b2 * k / n and c2 = -b1 * k / n.
  ...
  _endoWnafMulAdd(points, coeffs, initial, jacobian) {
    ...
    // See: Guide to Elliptic Curve Cryptography,
    // Algorithm 3.77, page 129, section 3.
    const len = points.length;
...
class ShortPoint extends Point {
  ...
  add(p) {
    ...
    // L = (Y1 - Y2) / (X1 - X2)
    let l = this.y.redSub(p.y);

    if (l.sign() !== 0)
      l = l.redMul(this.x.redSub(p.x).redInvert());
  ...
  dbl() {
    ...
    // L = (3 * XX + a) / (2 * Y)
    const l = xx.redIMuln(3).redIAdd(this.curve.a)
                .redMul(this.y.redMuln(2).redInvert());
...
class JPoint extends Point {
  ...
  _dblp(pow) {
    ...
    //   M = 3 * (XX - ZZZZ)
    //
    // For unknown reasons (at least to me).
  ...
  eqX(x) {
    ...
    // Verify that scalar `x` is equal to field
  ...
  eqXToP(x) {
    ...
    // compares a scalar to an X coordinate
    // by scaling it.
    //

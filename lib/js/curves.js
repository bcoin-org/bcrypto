...
class Curve {
  ...
  init(type, conf) {
    ...
    this.fieldLength = this.p.byteLength();
    this.signBit = this.fieldLength * 8 - 1;
    ...
    this.scalarLength = conf.n ? this.n.byteLength() : this.fieldLength;
    ...
    // Used for the low S check.
    this.half = this.nh ? this.encodeScalar(this.nh) : null;
  ...
  encodeInt(num) {
    ...
    assert(!num.red);
    return num.encode(this.endian, this.fieldLength);
  }

  decodeInt(bytes) {
    assert(Buffer.isBuffer(bytes));

    if (bytes.length !== this.fieldLength)
  ...
  _wnafMulAdd(defW, points, coeffs, initial, jacobian) {
    ...
    for (let i = len - 1; i >= 1; i -= 2) {
      ...
      if (wndWidth[a] !== 1 || wndWidth[b] !== 1) {
        naf[a] = getNAF(coeffs[a], wndWidth[a], this.n);
        naf[b] = getNAF(coeffs[b], wndWidth[b], this.n);
...
class Mask {
  ...
  splitHash(bytes) {
    const {fieldLength, scalarLength} = this.curve;

    assert(Buffer.isBuffer(bytes));
    assert(bytes.length === fieldLength * 2);

    const prefix = bytes.slice(fieldLength);

    let scalar = bytes.slice(0, fieldLength);
...
class Point {
  ...
  encode(compact) {
    ...
    const {fieldLength} = this.curve;

    if (compact) {
      const p = Buffer.allocUnsafe(1 + fieldLength);
    ...
    const p = Buffer.allocUnsafe(1 + fieldLength * 2);
    const x = this.curve.encodeInt(this.getX());
    const y = this.curve.encodeInt(this.getY());

    p[0] = 0x04;
    x.copy(p, 1);
    y.copy(p, 1 + fieldLength);
  ...
  _getBlinding(rng) {
    ...
    for (;;) {
      ...
      const unblind = this.jmul(blind.neg().iumod(this.curve.n));

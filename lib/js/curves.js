...
class Curve {
  ...
  init(type, conf) {
    ...
    this.g = conf.g ? this.pointFromJSON(conf.g) : this.point(null, null, null);
  ...
  _wnafMulAdd(defW, points, coeffs, initial, jacobian) {
    ...
    if (points.length & 1) {
      if (affine)
        points.push(this.point(null, null));
      else
        points.push(this.jpoint(null, null, null));

      coeffs.push(new BN(0));
    }

    const len = points.length;
    const wndWidth = new Array(len);
...
class ShortCurve extends Curve {
  constructor(conf) {
    ...
    this.zeroA = this.a.fromRed().sign() === 0;
  ...
  _getEndoRoots(num) {
    ...
    const s = new BN(3).toRed(red).redINeg().redSqrtp().redMul(tinv);
...
class EdwardsPoint extends Point {
  constructor(curve, x, y, z, t) {
    ...
    if (x === null && y === null && z === null) {
      ...
      this.x = BN.cast(x, 16);
      this.y = BN.cast(y, 16);
      this.z = z ? BN.cast(z, 16) : this.curve.one;
      this.t = t ? BN.cast(t, 16) : null;

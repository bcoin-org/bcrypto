...
class Curve {
  ...
  init(type, conf) {
    ...
    // Clamping constants.
    const bits = (this.p.byteLength() - 1) * 8;
    this.hMask = -this.h.toNumber() & 0xff;
    this.nMask = this.p.ushrn(bits).toNumber();
    this.bMask = (this.nMask + 1) >>> 1;
    this.andMask = BN.shift(this.nMask + 1, bits).isubn(1);
    this.andMask.iuandn(this.hMask);
    this.orMask = BN.shift(this.bMask, bits);
    ...
    if (this.id === 'ed25519' || this.id === 'x25519') {
      assert(this.hMask === (-8 & 0xff));
      assert(this.nMask === 0x7f);
      assert(this.bMask === 0x40);
    } else if (this.id === 'ed448' || this.id === 'x448') {
      assert(this.hMask === (-4 & 0xff));
      assert(this.nMask === 0xff);
      assert(this.bMask === 0x80);
    }
...
class MontCurve extends Curve {
  constructor(conf) {
    ...
    this.mask = conf.mask
      ? [new BN(conf.mask[0], 16), new BN(conf.mask[1], 16)]
      : null;
  }

  decodeScalar(bytes) {
    const k = super.decodeScalar(bytes);
    ...
    super({
      ...
      mask: [
        ['7fffffff ffffffff ffffffff ffffffff',
         'ffffffff ffffffff ffffffff fffffff8'].join(''),
        ['40000000 00000000 00000000 00000000',
         '00000000 00000000 00000000 00000000'].join('')
      ]
    });
  }
...
class ED25519 extends EdwardsCurve {
  ...
  clamp(bytes) {
    ...
    bytes[0] &= -8;
    bytes[this.scalarLength - 1] &= 0x7f;
    bytes[this.scalarLength - 1] |= 0x40;
  ...
  isClamped(bytes) {
    ...
    if (bytes[this.scalarLength - 1] & ~0x7f)
      return false;

    if (!(bytes[this.scalarLength - 1] & 0x40))
      return false;
...
class ED448 extends EdwardsCurve {
  ...
  clamp(bytes) {
    ...
    bytes[0] &= -4;
    bytes[this.scalarLength - 1] &= 0xff;
    bytes[this.scalarLength - 1] |= 0x80;

    if (bytes.length > this.scalarLength)
      bytes[this.scalarLength] = 0x00;
  ...
  isClamped(bytes) {
    ...
    if (bytes[this.scalarLength - 1] & ~0xff)
      return false;

    if (!(bytes[this.scalarLength - 1] & 0x80))
      return false;

    if (bytes.length > this.scalarLength) {
      if (bytes[this.scalarLength] !== 0x00)
        return false;
    }

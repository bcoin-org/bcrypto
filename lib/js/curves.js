...
class EdwardsCurve extends Curve {
  ...
  pointFromMont(point, odd) {
    ...
    if (this.twisted) {
      ...
      const four = two.redSqr();
      const u2 = u.redSqr();
      const u3 = u.redPow(new BN(3));
      const u5 = u.redPow(new BN(5));
      const v2 = v.redSqr();

      const a = u5;
      const b = two.redMul(u3);
      const c = four.redMul(u).redMul(v2);
      const d = u;

      const e = u5;
      const f = two.redMul(u2).redMul(v2);
      const g = two.redMul(u3);
      const h = two.redMul(v2);
      const i = u;

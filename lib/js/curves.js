...
class ShortPoint extends Point {
  ...
  uni(p) {
    ...
    if (m.sign() === 0 && r.sign() === 0) {
      // M = X1 - X2
      m.redIAdd(this.x.redSub(p.x));

      // R = Y1 - Y2
      r.redIAdd(this.y.redSub(p.y));
...
class JPoint extends Point {
  ...
  _uni(p) {
    ...
    // Generally only happens on endomorphic curves.
    const d = (m.sign() | r.sign()) ^ 1;

    // M = U1 - U2 (if degenerate)
    m.redIAdd(u1.redSub(u2).redIMuln(d));

    // R = S1 - S2 (if degenerate)
    r.redIAdd(s1.redSub(s2).redIMuln(d));
    ...
    // X3 = X1 (if Z2 = 0)
    nx.redIMuln(bi ^ 1).redIAdd(this.x.redMuln(bi));

    // Y3 = Y1 (if Z2 = 0)
    ny.redIMuln(bi ^ 1).redIAdd(this.y.redMuln(bi));

    // Z3 = Z1 (if Z2 = 0)
    nz.redIMuln(bi ^ 1).redIAdd(this.z.redMuln(bi));

    // X3 = X2 (if Z1 = 0)
    nx.redIMuln(ai ^ 1).redIAdd(p.x.redMuln(ai));

    // Y3 = Y2 (if Z1 = 0)
    ny.redIMuln(ai ^ 1).redIAdd(p.y.redMuln(ai));

    // Z3 = Z2 (if Z1 = 0)
    nz.redIMuln(ai ^ 1).redIAdd(p.z.redMuln(ai));

    // X3 = 1 (if Z3 = 0 & Z1 != 0 & Z2 != 0)
    nx.redIMuln(ni ^ 1).redIAdd(one.redMuln(ni));

    // Y3 = 1 (if Z3 = 0 & Z1 != 0 & Z2 != 0)
    ny.redIMuln(ni ^ 1).redIAdd(one.redMuln(ni));

...
class EdwardsCurve extends Curve {
  ...
  pointFromMont(point, odd) {
    ...
    if (this.twisted) {
      ...
      const r2 = point.curve.two;
      const r4 = new BN(4).toRed(point.curve.red);
      const n3 = new BN(3);
      const n5 = new BN(5);

      const a = u.redPow(n5);
      const b = r2.redMul(u.redPow(n3));
      const c = r4.redMul(u).redMul(v.redSqr());
      const d = u;

      const e = u.redPow(n5);
      const f = r2.redMul(u.redSqr()).redMul(v.redSqr());
      const g = r2.redMul(u.redPow(n3));
      const h = r2.redMul(v.redSqr());
      const i = u;

      const lhs = a.redSub(b).redSub(c).redAdd(d).redNeg();
      const rhs = e.redSub(f).redSub(g).redSub(h).redAdd(i);

      y = lhs.redMul(rhs.redInvm());

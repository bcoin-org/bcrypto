...
class Curve {
  ...
  _wnafMul(p, k, initial) {
    ...
    let acc = initial || this.jpoint(null, null, null);
...
class Point {
  ...
  precompute(power, rng) {
    ...
    if (!precomputed.cached) {
      precomputed.cached = true;

      if (!precomputed.naf)
        precomputed.naf = this._getNAFPoints(8);

      if (!precomputed.doubles)
    ...
    if (!precomputed.blinding)
      precomputed.blinding = this._getBlinding(rng);
...
class ShortPoint extends Point {
  ...
  static fromJSON(curve, json) {
    ...
    point.precomputed = {
      ...
      naf: pre.naf && {
        wnd: pre.naf.wnd,
        points: [point].concat(pre.naf.points.map(convert))
      },
      doubles: pre.doubles && {
        step: pre.doubles.step,
        points: [point].concat(pre.doubles.points.map(convert))
      },
      beta: null,
      blinding: null
    };
...
class MontPoint extends Point {
  ...
  toJSON() {
    return [this.x.fromRed(), this.z.fromRed()];
  }
...
class EdwardsCurve extends Curve {
  ...
  decodePoint(bytes) {
    ...
    const bit = (this.encodingLength * 8) - 1;
    const xIsOdd = y.testn(bit);
...
class EdwardsPoint extends Point {
  ...
  toJSON() {
    const x = this.x.fromRed();
    const y = this.y.fromRed();
    const z = this.z.fromRed();
    const t = this.t && this.t.fromRed();
  ...
  static fromJSON(curve, json) {
    ...
    const [x, y, z, t, pre] = json;
    const point = curve.point(x, y, z, t);

    if (!pre)
      return point;

    const convert = ([x, y, z, t]) => curve.point(x, y, z, t);

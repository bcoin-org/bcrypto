...
class Curve {
  ...
  _simpleMulAdd(points, coeffs, initial, jacobian) {
    ...
    for (let i = 0; i < points.length; i++) {
      const point = this._simpleMul(points[i], coeffs[i], null, true);
  ...
  _wnafMulAdd(defW, points, coeffs, initial, jacobian) {
    ...
    for (let i = len - 1; i >= 1; i -= 2) {
      ...
      if (width[a] !== 1 || width[b] !== 1) {
        naf[a] = getNAF(coeffs[a], width[a]);
        naf[b] = getNAF(coeffs[b], width[b]);
...
class ShortCurve extends Curve {
  ...
  pointFromX(x, odd) {
    ...
    const y2 = x.redPown(3).redIAdd(ax).redIAdd(this.b);
    const y = y2.redSqrt();
...
class ShortPoint extends Point {
  ...
  dbl() {
    ...
    const c = x2.redMuln(3).redIAdd(a).redMul(dyinv);
...
class JPoint extends Point {
  ...
  dblp(pow) {
    ...
    const jy = this.y;
    let jz = this.z;
    let jz4 = jz.redPown(4);

    // Reuse results.
    let jyd = jy.redMuln(2);

    for (let i = 0; i < pow; i++) {
      const jx2 = jx.redSqr();
      const jyd2 = jyd.redSqr();
      const jyd4 = jyd2.redSqr();
      const c = jx2.redMuln(3).redIAdd(a.redMul(jz4));

      const t1 = jx.redMul(jyd2);
      const nx = c.redSqr().redISub(t1.redMuln(2));
      const t2 = t1.redISub(nx);

      const dny = c.redMul(t2).redIMuln(2).redISub(jyd4);
  ...
  _zeroDbl() {
    ...
    if (this.zOne) {
      ...
      // S = 2 * ((X1 + YY)^2 - XX - YYYY)
      const s = this.x.redAdd(yy).redSqr().redISub(xx).redISub(yyyy);
      ...
      // D = 2 * ((X1 + B)^2 - A - C)
      const d = this.x.redAdd(b).redSqr().redISub(a).redISub(c);
      ...
      // Z3 = 2 * Y1 * Z1
      nz = this.y.redMul(this.z).redIMuln(2);
    }
  ...
  _threeDbl() {
    ...
    if (this.zOne) {
      ...
      // M = 3 * XX + a
      const m = xx.redMuln(3).redIAdd(this.curve.a);
      ...
      const alpha = this.x.redSub(delta).redMul(this.x.redAdd(delta));
      ...
      const ggamma8 = gamma.redSqr().redIMuln(8);
  ...
  trpl() {
    ...
    const e = this.x.redAdd(yy).redSqr().redISub(xx).redISub(yyyy);
    ...
    // Y3 = 8 * Y1 * (U * (T - U) - E * EE)
    const ny = this.y.redMul(u.redMul(t.redISub(u)).redISub(e.redMul(ee)));
...
class EdwardsPoint extends Point {
  ...
  _extDbl() {
    ...
    // C = 2 * Z1^2
    const c = this.z.redSqr().redIMuln(2);
  ...
  _extAdd(p) {
    ...
    // D = Z1 * 2 * Z2
    const d = this.z.redMul(p.z.redMuln(2));

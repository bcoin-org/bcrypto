...
class Point {
  ...
  _mulConst(k, jacobian) {
    ...
    if (!blinding)
      return this.curve._ladderMul(this, k, null, jacobian);

    // Blind if we have it.
    const {blind, unblind} = blinding;
    const t = k.sub(blind);

    return this.curve._ladderMul(this, t, unblind, jacobian);
  }
...
class JPoint extends Point {
  ...
  _uni(p) {
    ...
    const {zeroA, zero, one} = this.curve;
    ...
    // ZZZZ = Z^4
    const zzzz = zeroA ? zero : z.redPown(4);

    // AZ = a * ZZZZ
    const az = this.curve.a.redMul(zzzz);

    // R = T^2 - U1 * U2 + AZ
    const r = t.redSqr().redISub(u1.redMul(u2)).redIAdd(az);

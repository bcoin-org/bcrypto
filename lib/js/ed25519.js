...
class Ed25519 {
  ...
  hashInt(...items) {
    const hash = this.hash.multi(...items);
    return new BN(hash, 'le');
  }
  ...
  privateKeyConvert(secret) {
    ...
    return this.clamp(this.hash.digest(secret));
  }
  ...
  scalarTweakAdd(key, tweak) {
    ...
    const t = this.decodeInt(tweak).umod(n);
    const k = this.decodeInt(key).iadd(t).umod(n);
  ...
  scalarTweakMul(key, tweak) {
    ...
    const n = this.curve.n;
    const t = this.decodeInt(tweak).umod(n);
    const k = this.decodeInt(key).imul(t).umod(n);
  ...
  publicKeyTweakAdd(key, tweak) {
    ...
    const point = this.ec.g.mul(t).add(k);
  ...
  sign(msg, secret) {
    ...
    const hash = this.hash.digest(secret);
    const key = this.clamp(hash);
    const prefix = hash.slice(this.size);

    return this._sign(msg, key, prefix);
  }
  ...
  _sign(msg, key, prefix) {
    ...
    const a = this.decodeInt(key);
    const k = this.encodePoint(this.ec.g.mul(a));
    const r = this.hashInt(prefix, msg);
    const R = this.encodePoint(this.ec.g.mul(r));
    const h = this.hashInt(R, k, msg).mul(a);
    const S = r.add(h).umod(this.curve.n);
  ...
  derive(pub, secret) {
    ...
    const a = this.decodeInt(priv).umod(n);

...
function convert(input, i, output, j, frombits, tobits, pad) {
  ...
  for (; i < input.length; i++) {
    acc = ((acc << frombits) | input[i]) & maxacc;
    bits += frombits;
  ...
  if (pad) {
    ...
    if (bits >= frombits || ((acc << left) & mask))
...
function encode(prefix, type, hash) {
  ...
  const conv = convert(data, 0, output, 0, 8, 5, true);
...
function decode(str, prefix = 'bitcoincash') {
  const data = deserialize(str, prefix);

  if (data.length === 0 || data.length > 104)
    throw new Error('Invalid cash32 data.');

  const output = data;
  const conv = convert(data, 0, output, 0, 5, 8, false);

  if (conv.length === 0 || conv.length > 1 + 64)
    throw new Error('Invalid cash32 data.');

  const type = (conv[0] >>> 3) & 31;
  const hash = conv.slice(1);

  let size = 20 + 4 * (conv[0] & 3);

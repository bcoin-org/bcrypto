...
class AESKey {
  ...
  decrypt(input, ipos, output, opos) {
    ...
    // Map byte array block to cipher
    // state and add initial round key.
    let s0 = readU32(input, ipos + 0) ^ key[kp + 0];
    let s1 = readU32(input, ipos + 4) ^ key[kp + 1];
    let s2 = readU32(input, ipos + 8) ^ key[kp + 2];
    let s3 = readU32(input, ipos + 12) ^ key[kp + 3];
...
class AESCipher {
  ...
  encrypt(input, ipos, output, opos) {
    if (this.chain) {
      ...
      this.key.encrypt(output, opos, output, opos);
      this.prev = output;
      this.ppos = opos;
    } else {
      this.key.encrypt(input, ipos, output, opos);
    }
  }
  ...
  final() {
    ...
    this.block.fill(left, this.bpos, 16);
    this.encrypt(this.block, 0, this.block, 0);
    this.key.destroy();
    return this.block;
  }
}
...
class AESDecipher {
  ...
  decrypt(input, ipos, output, opos) {
    if (this.chain) {
      this.key.decrypt(input, ipos, output, opos);
      for (let i = 0; i < 16; i++)
  ...
  final() {
    ...
    assert(this.bpos === 0, 'Bad decrypt (trailing bytes).');
    assert(this.last, 'Bad decrypt (no data).');
    ...
    return block.slice(this.lpos, b);
  }
}
...
function encrypt(data, key, iv, bits, mode) {
...
function decrypt(data, key, iv, bits, mode) {
...
AES.encipher = function encipher(data, key, iv) {
  ...
  assert(iv.length === 16);
  return encrypt(data, key, iv, 256, 'cbc');
};
...
AES.decipher = function decipher(data, key, iv) {
  ...
  assert(key.length === 32);
  assert(iv.length === 16);
  return decrypt(data, key, iv, 256, 'cbc');
};

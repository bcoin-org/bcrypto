...
class Schnorr {
  constructor(curve, hash) {
    ...
    this.hash = hash;
    this.n3 = new BN(3);
    this.r7 = new BN(7).toRed(curve.red);
    this.p14 = curve.p.addn(1).divn(4);
  }
  ...
  _batchVerify(batch) {
    ...
    for (const [msg, sig, key] of batch) {
      ...
      // Let c = (r^3 + 7) mod p.
      const c = Rred.redPow(this.n3).redIAdd(this.r7);

      // Let y = c^((p+1)/4) mod p.
      const y = c.redPow(this.p14);
    ...
    const [A, S, e, R] = items[0];

    let lhs = S;
    let rhs = R.add(A.mul(e));

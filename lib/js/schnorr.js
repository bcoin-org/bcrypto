...
class Schnorr {
  constructor(curve, hash) {
    this.curve = curve.precompute(rng);
    this.hash = hash;
    this.id = this.curve.id;
    this.type = this.curve.type;
    ...
    this.deriveTag = this.createTag('BIPSchnorrDerive');
    this.hashTag = this.createTag('BIPSchnorr');
    this.rng = new RNG(this);
  }

  createTag(tag) {
    const raw = Buffer.from(tag, 'binary');
    const hash = this.hash.digest(raw);

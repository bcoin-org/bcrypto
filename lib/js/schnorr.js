...
class Schnorr {
  ...
  publicKeyToUniform(key, hint = rng.randomInt()) {
    const A = this.curve.decodeSquare(key);
  ...
  publicKeyExport(key) {
    const {x, y} = this.curve.decodeSquare(key);
  ...
  publicKeyTweakTest(key, tweak, sign, test) {
    ...
    const P = this.curve.decodeSquare(key);
    const Q = this.curve.decodeSquare(test);
    const t = this.curve.decodeScalar(tweak);
  ...
  derive(pub, priv) {
    const A = this.curve.decodeSquare(pub);

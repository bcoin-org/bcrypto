...
class Schnorr {
  ...
  sign(msg, key) {
    ...
    // Return the signature bytes(R) || bytes((k + e * d) mod n).
  ...
  _verify(msg, sig, key) {
    ...
    // In concept, a Schnorr signature can be validated with:
    //
    //   s * G == R + e * P
    //
    ...
    // Optimized as `is_square(y * z mod p)`.
    if (!R.hasQuadY())
      return false;

    // Check `x(R) == r` in the Jacobian space.
    // Optimized as `x != z^2 * r mod p`.
    if (!R.eqX(Rx))
      return false;

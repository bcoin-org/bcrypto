...
class Schnorr {
  ...
  hashNonce(a, A, m, d) {
    ...
    const h = this.hash.multi(this._auxTag, d);
    const t = this.curve.encodeScalar(a);

    for (let i = 0; i < this.curve.scalarSize; i++)
      t[i] ^= h[i % this.hash.size];

    return this.hashInt(this._nonceTag, t, A, m);
  }
  ...
  publicKeyTweakTest(key, tweak, sign, test) {
    ...
    const P = this.curve.decodeEven(key);
  ...
  _sign(msg, key, aux) {
    ...
    const k = this.hashNonce(a, Araw, msg, aux);

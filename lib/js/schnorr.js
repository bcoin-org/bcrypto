...
class Schnorr {
  ...
  _verify(msg, sig, key) {
    ...
    // We stay in the jacobian space here. This
    // avoids any divisions by the z coordinate.
    if (this.curve.redN) {
      // Let R = s*G - e*P.
      const R = G.jmulAdd(S, A, N.sub(e));
    ...
    // Stick with the spec otherwise (note that
    // secp256k1 will get the above optimizations).
    // Let R = s*G - e*P.
    const R = G.mulAdd(S, A, N.sub(e));
  ...
  _batchVerify(batch) {
    ...
    for (const [msg, sig, key] of batch) {
      ...
      // Let c = (r^3 + 7) mod p.
      // Note: generalized for other curves.
      ...
      // Fail if c != y^2 mod p.
      // Avoid simple equals due to reduction engine.
      if (!y.redSqr().redISub(c).eq(this.curve.zero))
        return false;
      ...
      lhs = lhs.iadd(S.imul(a)).iumod(N);
      rhs = rhs.add(R.jmulAdd(a, A, ea));
    }

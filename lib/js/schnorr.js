...
class Schnorr {
  ...
  _batchVerify(batch) {
    ...
    for (const {message, signature, key} of batch) {
      ...
      // Let y = c^(p+1)/4 mod p.
      // y = (c ** ((P + 1) / 4)) % P
      const y = c.redPow(P.addn(1).divn(4));

      // z = (y ** 2) % P
      const z = y.redPow(two);
    ...
    for (let i = 1; i < items.length; i++) {
      ...
      const a_ = random.randomBytes(this.curve.size);
      const a = this.curve.decodeInt(a_).umod(N);

      lhs = lhs.iadd(a.mul(S)).umod(N);
      rhs = rhs.add(R.mulAdd(a, A, a.mul(e)));
    }

    // Fail if (s1 + a2*s2 + ... + au*su)G != R1 + a2*R2 + ... + au*Ru + e1*P1
    //                                      + (a2*e2)P2 + ... + (au*eu)Pu.

...
class Curve {
  constructor(params) {
    ...
    this.red = redPrime('p448', params.p);
    this.d = this.field(params.d, 16);
    this.n = new BN(params.n, 16);
    this.x = this.field(params.x, 16);
    this.y = this.field(params.y, 16);
    this.bits = params.bits;
    this.size = params.size;
    this.hash = params.hash;
    this.prefix = Buffer.from(params.prefix, 'binary');
    this.zero = this.field(0);
    this.one = this.field(1);
  ...
  field(...args) {
    return new BN(...args).toRed(this.red);
  }
  ...
  solveX2(y) {
    ...
    const {d, one} = this;
    const ys = y.redSqr();
    const a = ys.redSub(one);
    const b = d.redMul(ys).redSub(one);
...
function redPrime(name, p) {
  // Hack to add a custom prime to bn.js.
  const MPrime = BN._prime('p192').constructor.super_;
  const prime = new MPrime(name, p);
  const red = BN.red(prime.p);
  red.prime = prime;
  return red;
}

...
class Red {
  ...
  pow(a, num, size) {
    ...
    return this.powNum(a, num, size);
  }

  powNum(a, num, size) {
    const bits = Math.max(num.bitLength(), size);
    ...
    for (let i = steps - 1; i >= 0; i--) {
      const bits = num.bits(i * WND_WIDTH, WND_WIDTH);
...
function powInt(x, e, m, size) {
  const bits = Math.max(e.bitLength(), size);
  const steps = ((bits + WND_WIDTH - 1) / WND_WIDTH) >>> 0;
  const wnd = new Array(WND_SIZE);
  ...
  for (let i = steps - 1; i >= 0; i--) {
    const bits = e.bits(i * WND_WIDTH, WND_WIDTH);

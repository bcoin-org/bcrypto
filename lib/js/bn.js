...
class Red {
  ...
  sqrt5mod8p(x) {
    if (this.sm1 === null) {
      const two = new BN(2).toRed(this);
      const e = this.m.subn(1).iushrn(2);

      this.sm1 = this.pow(two, e);
    }
  ...
  legendre(num) {
    ...
    // Euler's criterion.
    const symbol = this.pow(num, this.m.subn(1).iushrn(1));
    const one = new BN(1).toRed(this);
...
class Mont extends Red {
  ...
  isPos(a) {
    this._verify1(a);
    return this.convertFrom(a).cmp(this.m.ushrn(1)) <= 0;
  }

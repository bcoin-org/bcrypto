...
const WND_WIDTH = 4;
const WND_SIZE = 1 << WND_WIDTH;
const WND_MASK = WND_SIZE - 1;
...
class Red {
  ...
  fixed(a, num, size) {
    ...
    const steps = ((bits + WND_WIDTH - 1) / WND_WIDTH) >>> 0;
    const exp = num.toArray('le', (bits + 7) >>> 3);
    const wnd = new Array(WND_SIZE);

    wnd[0] = new BN(1).toRed(this);
    wnd[1] = a.clone();

    for (let i = 2; i < WND_SIZE; i += 2) {
      wnd[i] = this.sqr(wnd[i >>> 1]);
      wnd[i + 1] = this.mul(wnd[i], a);
    }
    ...
    for (let i = steps - 1; i >= 0; i--) {
      ...
      const bits = (exp[pos >>> 3] >> (pos & 7)) & WND_MASK;

      if (i === steps - 1) {
        res = wnd[bits];
      } else {
        for (let j = 0; j < WND_WIDTH; j++)
          res = this.sqr(res);

        res = this.mul(res, wnd[bits]);
      }
    }
...
function powInt(x, e, m, size) {
  ...
  const steps = ((bits + WND_WIDTH - 1) / WND_WIDTH) >>> 0;
  const exp = e.toArray('le', (bits + 7) >>> 3);
  const wnd = new Array(WND_SIZE);

...
class Mont extends Red {
  constructor(m) {
    ...
    this.ri = this.r._invmp(this.m);

    assert(this.ri.sign() > 0, '_invmp bug!');

    this.mi = this.ri.mul(this.r).isubn(1).div(this.m);
    this.mi = this.mi.imod(this.r);
    this.mi = this.r.sub(this.mi);
  }

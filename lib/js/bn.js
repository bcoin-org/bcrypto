...
next:
    for (let i = 0; i < reps; i++) {
      ...
      if (y.ucmp(rone) === 0 || y.ucmp(rnm1) === 0)
        continue;
  ...
  ceq(num) {
    ...
    for (let i = 0; i < this.length; i++)
      z |= this.words[i] ^ num.words[i % num.length];
  ...
  cset(num, flag) {
    ...
    this.words[0] = (this.words[0] & mask0) | (num & mask1);
...
class Prime {
  ...
  _tmp() {
    const len = Math.ceil(this.n / 13);
    const tmp = new BN();

    tmp.words = new Array(len);
  ...
  ireduce(num) {
    ...
    assert(num.negative === 0);
    assert(num.length <= this.p.length * 2);

    if (num.length < this.p.length)
      return num;
...
class Red {
  ...
  climb(a, num, size) {
    ...
    const b256 = num.toArray('le', bytes);

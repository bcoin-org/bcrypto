...
class BN {
  ...
  imul(num) {
    return this.clone()._mul(num, this);
  }
  ...
  _wordDiv(num, flags) {
    ...
    for (let j = m - 1; j >= 0; j--) {
      const kj = (a.words[b.length + j] | 0) * 0x4000000
               + (a.words[b.length + j - 1] | 0);

      let qj = Math.min((kj / bhi) | 0, 0x3ffffff);
  ...
  powmn(y, m, mont) {
    const p = mont ? BN.mont(m) : BN.red(m);
    const n = this.toRed(p);
    return n.redPowmn(y).fromRed();
  }

  isqrtp(p) {
    return this.sqrtp(p)._move(this);
  }

  sqrtp(p) {
    const n = this.toRed(BN.mont(p));
    return n.redSqrtp().fromRed();
  }
...
next:
  ...
  redISqrtp() {
    red(this.red, 'redISqrtp');
    return this.red.isqrtp(this);
  }

  redSqrtp() {
    red(this.red, 'redSqrtp');
    return this.red.sqrtp(this);
  }
  ...
  redIPowm(num) {
    enforce(BN.isBN(num), 'num', 'bignum');
    red(this.red, 'redIPowm');
    nonred(!num.red, 'redIPowm');
    return this.red.ipowm(this, num);
  }

  redPowm(num) {
    enforce(BN.isBN(num), 'num', 'bignum');
    red(this.red, 'redPowm');
    nonred(!num.red, 'redPowm');
    return this.red.powm(this, num);
  }

  redIPowmn(num) {
    enforce(isSMI(num), 'num', 'smi');
    red(this.red, 'redIPowmn');
    return this.red.ipowmn(this, num);
  }

  redPowmn(num) {
    enforce(isSMI(num), 'num', 'smi');
    red(this.red, 'redPowmn');
    return this.red.powmn(this, num);
  }
  ...
  _fromHex(str, start) {
    ...
    this.length = Math.ceil((str.length - start) / 6) + 1;
  ...
  fromArrayLike(data, endian) {
    ...
    this.length = Math.ceil(data.length / 3) + 1;
    this.words = new Array(this.length);
    this.negative = 0;
...
class K256 extends Prime {
  ...
  imulK(num) {
    ...
    if (num.length > 1)
      assert(num.words[num.length - 1] !== 0);
...
class Red {
  ...
  ishln(a, num) {
    return this.shln(a, num)._move(a);
  }
  ...
  sqrtp(x) {
    ...
    let b = this.powm(x, s);
    let g = this.powm(n, s);
    let y = this.powm(x, s.iaddn(1).iushrn(1));
    let k = e;
  ...
  fermat(a) {
    ...
    return this.powm(a, this.m.subn(2));
  }
  ...
  legendre(a) {
    ...
    // Euler's criterion.
    const s = this.powm(a, this.m.subn(1).iushrn(1));
  ...
  ipowm(a, num) {
    return this.powm(a, num)._move(a);
  }
  ...
  powm(a, num) {
    ...
    if (num.length === 1)
      return this.powmn(a, num.words[0]);
  ...
  ipowmn(a, num) {
    return this.powmn(a, num)._move(a);
  }

...
class Keccak {
  ...
  update(data) {
    ...
    this.rest = (this.rest + size) % this.bs;

    if (index) {
      const left = this.bs - index;
  ...
  transform(block, off) {
    ...
    for (let i = 0; i < count; i++) {
      this.state[i] ^= block.readUInt32LE(off, true);
      off += 4;
    }
  ...
  final(std = false) {
    ...
    this.block.fill(0, this.rest, this.bs);
    this.block[this.rest] |= std ? 0x06 : 0x01;

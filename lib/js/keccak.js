...
class Keccak {
  ...
  static hmac(bits = 256, pad = 0x01) {
    const bs = (1600 - bits * 2) / 8;
    return new HMAC(Keccak, bs, bits, pad);
  }

  static digest(data, bits = 256, pad = 0x01) {
    return Keccak.ctx.init(bits).update(data).final(pad);
  }

  static root(left, right, bits = 256, pad = 0x01) {
    assert(Buffer.isBuffer(left) && left.length === bits / 8);
    assert(Buffer.isBuffer(right) && right.length === bits / 8);
    return Keccak.ctx.init(bits).update(left).update(right).final(pad);
  }

  static multi(one, two, three, bits = 256, pad = 0x01) {
    ...
    if (three)
      ctx.update(three);
    return ctx.final(pad);
  }

  static mac(data, key, bits = 256, pad = 0x01) {
    return Keccak.hmac(bits, pad).init(key).update(data).final();
  }
}

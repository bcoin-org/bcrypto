...
class EDDSA {
  ...
  get ka() {
    if (!this._ka)
      this._ka = elliptic.ec('curve' + this.xid.substring(1).toLowerCase());
    return this._ka;
  }
  ...
  _privateKeyTweakMul(key, tweak) {
    ...
    const n = this.curve.n;
    const t = this.ec.decodeInt(tweak).umod(n);
    const k = this.ec.decodeInt(key).imul(t).umod(n);
  ...
  signTweakMul(msg, secret, tweak) {
    ...
    const cpriv = this._privateKeyTweakMul(priv, tweak);
    const cpub = this.publicKeyTweakMul(pub, tweak);
  ...
  derive(pub, secret) {
    ...
    const n = this.curve.n;
    const priv = this.privateKeyConvert(secret);
    const a = this.ec.decodeInt(priv).umod(n);
    const A = this.ec.decodePoint(toArray(pub));
    const point = A.mul(a);

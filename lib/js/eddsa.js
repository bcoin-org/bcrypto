...
class EDDSA {
  ...
  privateKeyExpand(secret) {
    ...
    const hash = this.hash.digest(secret, this.size * 2);
  ...
  scalarReduce(scalar) {
    ...
    if (scalar.length > this.curve.scalarLength)
      scalar = scalar.slice(0, this.curve.scalarLength);

    const s = BN.decode(scalar, this.curve.endian).iumod(this.curve.n);
  ...
  scalarNegate(scalar) {
    ...
    const s = a.ineg().iumod(this.curve.n);
  ...
  _verify(msg, sig, key, ph, ctx) {
    ...
    //   R == G*S - A*e
    const Rp = G.mulAdd(S, A, e.ineg().iumod(N));

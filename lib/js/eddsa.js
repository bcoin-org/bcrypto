...
//   disallow points at infinity (in all functions).
//
...
class EDDSA {
  ...
  publicKeyFromUniform(bytes) {
    ...
    const p = this.curve.pointFromUniform(this.x, u);

    return p.encode();
  }

  pointFromUniform(bytes) {
    const u = this.x.decodeUniform(bytes);
    const [p] = this.x.pointFromUniform(u);
  ...
  publicKeyToUniform(pub) {
    ...
    const u = this.curve.pointToUniform(this.x, p);
  ...
  pointToUniform(pub, sign) {
    ...
    const u = this.x.pointToUniform(p, sign);

    return this.x.encodeUniform(u, rng);
  }

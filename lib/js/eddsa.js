...
class EDDSA {
  ...
  publicKeyFromUniform(bytes) {
    ...
    const p = this.curve.pointFromUniform(this.mont, u);
  ...
  publicKeyToUniform(pub, hint = rng.randomInt()) {
    ...
    const u = this.curve.pointToUniform(this.mont, p, hint);

    return this.curve.encodeUniform(u, rng);
  }

  publicKeyFromHash(bytes, pake = false) {
    const p = this.curve.pointFromHash(this.mont, bytes, pake);
  ...
  publicKeyToHash(pub) {
    const p = this.curve.decodePoint(pub);
    return this.curve.pointToHash(this.mont, p, rng);
  }

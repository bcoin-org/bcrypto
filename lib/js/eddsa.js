...
class EDDSA {
  ...
  _verifyBatch(batch, ph, ctx) {
    const N = this.curve.n;
    const G = this.curve.g;
    const points = new Array(batch.length * 2);
    const coeffs = new Array(batch.length * 2);
    const sum = new BN(0);
    ...
    for (let i = 0; i < batch.length; i++) {
      ...
      // e = H(R, A, m).
      const e = this.hashInt(ph, ctx, Rraw, key, msg);
      const a = i === 0 ? new BN(1) : BN.random(rng, 1, N);
    ...
    return G.mulAll(sum, points, coeffs).isInfinity();
  }

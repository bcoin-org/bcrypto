...
class ECDSA {
  ...
  get curve() {
    if (!this._curve) {
      ...
      this._curve.hash = hashes[this._curve.hash];
      this._curve.schnorr = new Schnorr(this._curve);
    }
    return this._curve;
  }
  ...
  privateKeyTweakAdd(key, tweak) {
    ...
    const T = a.iadd(t).umod(this.curve.n);
  ...
  privateKeyTweakMul(key, tweak) {
    ...
    const a = this.curve.decodeInt(key);
    const T = a.imul(t).umod(this.curve.n);
  ...
  _truncate(msg, truncOnly) {
    const m = new BN(msg);
    const d = m.byteLength() * 8 - this.curve.n.bitLength();

    if (d > 0)
      m.iushrn(d);

    if (!truncOnly && m.cmp(this.curve.n) >= 0)
      return m.isub(this.curve.n);
  ...
  _sign(msg, key) {
    ...
    const bytes = this.curve.n.byteLength();

    // Zero-extend nonce to have the same byte size as N
    const nonce = m.toBuffer('be', bytes);

    // Instantiate Hmac_DRBG
    const drbg = new DRBG(this.curve.hash, key, nonce);

    // Number of bytes to generate
    const ns1 = this.curve.n.subn(1);

    for (;;) {
      const k = this._truncate(drbg.generate(bytes), true);

      if (k.cmpn(1) <= 0 || k.cmp(ns1) >= 0)
        continue;

      const kp = this.curve.g.mul(k);

      if (kp.isInfinity())
        continue;

      const kpX = kp.getX();
      const r = kpX.umod(this.curve.n);

      if (r.cmpn(0) === 0)
        continue;

      let s = k.invm(this.curve.n);

      s = s.mul(r.mul(a).iadd(m));
  ...
  _verify(msg, sig, key) {
    ...
    if (r.cmpn(1) < 0 || r.cmp(this.curve.n) >= 0)
      return false;

    if (s.cmpn(1) < 0 || s.cmp(this.curve.n) >= 0)
      return false;

    // Validate signature
    const sinv = s.invm(this.curve.n);
    const u1 = sinv.mul(m).umod(this.curve.n);
    const u2 = sinv.mul(r).umod(this.curve.n);
    ...
    if (!this.curve._maxwellTrick) {
      ...
      return p.getX().umod(this.curve.n).cmp(r) === 0;
    }
  ...
  _recover(msg, sig, j) {
    const m = this._truncate(new BN(msg));
    const r = this.curve.decodeInt(sig.r);
    const s = this.curve.decodeInt(sig.s);
    ...
    // 1.1. Let x = r + jn.
    if (isSecondKey)
      x = x.add(this.curve.n);

    const r2 = this.curve.pointFromX(x, isYOdd);
    const rInv = r.invm(this.curve.n);
    const s1 = this.curve.n.sub(m).mul(rInv).umod(this.curve.n);
    const s2 = s.mul(rInv).umod(this.curve.n);

    // 1.6.1 Compute Q = r^-1 (sR -  eG)
    //               Q = r^-1 (sR + -eG)
    return this.curve.g.mulAdd(s1, r2, s2);
  }

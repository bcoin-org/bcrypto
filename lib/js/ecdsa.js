 ...
 *   http://www.secg.org/sec1-v2.pdf (section 4.1.3 & 4.1.4)
 *   https://tinyurl.com/guide-to-ecc (section 4.4.1)
 */
...
class ECDSA {
  ...
  publicKeyToUniform(key, hint = rng.randomInt()) {
    const p = this.curve.decodePoint(key);
  ...
  publicKeyExport(key, xOnly = false) {
    return this.publicKeyConvert(key, xOnly).slice(1);
  }
  ...
  _sign(msg, key) {
    ...
    for (;;) {
      ...
      // Recovery param.
      let param = kp.sign() | (!x.eq(r) << 1);

      // Use complement of `s` if > `n / 2`.
      if (s.cmp(Nh) > 0) {
        s.ineg().imod(N);
        param ^= 1;
      }

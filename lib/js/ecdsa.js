...
class ECDSA {
  ...
  publicKeyImport(json, compress) {
    ...
    const A = this.curve.pointFromX(x, Boolean(json.sign));
  ...
  _encodeDER(r, s) {
    ...
    const size = 1 + asn1.sizeSize(len) + len;
    const out = Buffer.alloc(size);

    let pos = 0;

    out[pos++] = 0x30;
    pos = asn1.writeSize(out, pos, len);
  ...
  _decodeDER(sig, strict) {
    ...
    try {
      ...
      if (e.message !== 'Integers must be positive.')
        throw e;
      overflow = true;
    }
    ...
    if (overflow || r.cmp(n) >= 0 || s.cmp(n) >= 0)

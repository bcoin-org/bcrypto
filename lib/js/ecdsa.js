...
class ECDSA {
  ...
  _verify(msg, sig, key) {
    ...
    const u1 = sinv.mul(m).iumod(this.curve.n);
    const u2 = sinv.mul(r).iumod(this.curve.n);
  ...
  _recover(msg, sig, j) {
    ...
    const s1 = this.curve.n.sub(m).imul(rInv).iumod(this.curve.n);
    const s2 = s.mul(rInv).iumod(this.curve.n);

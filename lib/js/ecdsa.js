...
class ECDSA {
  constructor(name) {
    ...
    this.id = name;
    this.curve = new curves[this.id]();
    this.hash = hashes[this.curve.hash];
    this.p = this.curve.p;
    this.n = this.curve.n;
    this.nh = this.curve.nh;
    this.g = this.curve.g;
    this.size = this.curve.size;
    this.bits = this.curve.bits;
    this.zero = this.curve.encodeInt(new BN(0));
    this.order = this.curve.encodeInt(this.n);
    this.half = this.curve.encodeInt(this.nh);
    this.schnorr = new Schnorr(this.curve, this.hash);
  }
  ...
  _truncate(msg, truncOnly) {
    const m = new BN(msg);
    const d = m.byteLength() * 8 - this.n.bitLength();

    if (d > 0)
      m.iushrn(d);

    if (!truncOnly && m.cmp(this.n) >= 0)
      return m.isub(this.n);
  ...
  _verify(msg, sig, key) {
    ...
    const u1 = sinv.mul(m).umod(this.n);
    const u2 = sinv.mul(r).umod(this.n);

    if (!this.curve._maxwellTrick) {
      const p = this.g.mulAdd(u1, A, u2);
    ...
    // https://git.io/vad3K
    const p = this.g.jmulAdd(u1, A, u2);
  ...
  _recover(msg, sig, j) {
    ...
    if (r.cmp(this.p.umod(this.n)) >= 0 && isSecondKey)
      return null;
    ...
    const rInv = r.invm(this.n);
    const s1 = this.n.sub(m).mul(rInv).umod(this.n);
    const s2 = s.mul(rInv).umod(this.n);

...
class ECDSA {
  ...
  signatureNormalize(raw) {
    ...
    if (s.cmp(this.curve.nh) > 0)
      sig.s = this.curve.encodeScalar(this.curve.n.sub(s));
  ...
  isLowS(sig) {
    return Signature.isLowS(sig, this.size, this.curve.nhRaw);
  }

  isLowDER(sig) {
    return Signature.isLowDER(sig, this.size, this.curve.nhRaw);
  }

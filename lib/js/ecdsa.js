...
class ECDSA {
  ...
  publicKeyCreate(key, compress) {
    ...
    const A = this.curve.g.mulBlind(rng, a);
  ...
  _sign(msg, key) {
    ...
    for (;;) {
      ...
      // t := (b * a * r) mod n
      const t = b.mul(a).umod(N)
                 .imul(r).umod(N);

      // bm := (b * m) mod n
      const bm = b.imul(m).umod(N);

      // s := ((b * a * r) + (b * m)) mod n
      // s := (s * k^-1) mod n
      // s := (s * b^-1) mod n
      let s = t.add(bm).umod(N)
               .imul(ki).umod(N)
               .imul(bi).umod(N);

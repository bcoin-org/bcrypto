...
class ECDH {
  ...
  get ed() {
    ...
    if (!this._ed) {
      this._ed = elliptic.curve(this.eid, this._pre);
      this._ed.precompute(rng);
      this._pre = null;
    }
  ...
  publicKeyConvert(pub, sign = false) {
    const point = this.curve.decodePoint(pub);

    if (!point.validate())
      throw new Error('Invalid point.');

    return this.ed.pointFromMont(point, sign).encode();
  }
  ...
  publicKeyFromHash(bytes) {
    const p = this.curve.pointFromHash(this.ed, bytes);

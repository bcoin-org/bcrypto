...
class BlockCipher extends Cipher {
  ...
  _final() {
    const bs = this.blockSize;
    const left = bs - this.blockPos;
    const block = Buffer.from(this.block);

    block.fill(left, this.blockPos, bs);
...
class CTR extends Cipher {
  ...
  _final() {
    ...
    const out = Buffer.allocUnsafe(this.blockPos);
...
class GCM {
  ...
  final() {
    const size = this.tag ? this.tag.length : 16;

    this.mac = this.aead.final(size);

    if (!this.encrypt) {
      if (!this.tag || !gcm.GCM.verify(this.mac, this.tag))

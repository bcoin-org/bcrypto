...
class Block extends Mode {
  ...
  init(key, iv, options) {
    ...
    assert(options == null || (options && typeof options === 'object'));
...
class CBCMAC {
  constructor(ctx) {
    ...
    this.block = Buffer.alloc(this.size);
    this.pos = -1;
  }
  ...
  update(data) {
    ...
    this.pos = (this.pos + len) % this.size;

    if (pos > 0) {
      let want = this.size - pos;
      ...
      this.absorb(this.block, 0);
    }

    while (len >= this.size) {
      this.absorb(data, off);
      off += this.size;
      len -= this.size;
    }
  ...
  pad() {
    if (this.pos > 0) {
      const left = this.size - this.pos;

      this.update(Buffer.alloc(left));
    }
...
class CCM extends Stream {
  ...
  _init(key, iv, options) {
    ...
    const tagLen = options.authTagLength;
    const ptLen = options.plaintextLength;

    assert(aad == null || Buffer.isBuffer(aad));
    assert((tagLen >>> 0) === tagLen);
    assert((ptLen >>> 0) === ptLen);

    assert(iv.length >= 7 && iv.length <= 13);
    assert(tagLen >= 4 && tagLen <= 16);
    assert((tagLen & 1) === 0);
    ...
    flags |= (((tagLen - 2) >>> 1) & 7) << 3;
    flags |= (15 - iv.length - 1) & 7;
    ...
    iv.copy(block, 1, 0, iv.length);

    let len = ptLen;

    for (let i = 15; i >= 1 + iv.length; i--) {
      block[i] = len & 0xff;
      len >>>= 8;
    }
  ...
  _final() {
    const mac = this.hash.final(this.tagLen);
  ...
  _setAAD(data) {
    throw new Error('Must set AAD on initialization.');
  }

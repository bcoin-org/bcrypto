...
dsa.paramsVerify = function paramsVerify(params, strict) {
...
dsa._sign = function _sign(msg, key) {
  ...
  for (;;) {
    ...
    // Blind.
    const bx = x.mul(blind).iumod(q);
    const bm = m.mul(blind).iumod(q);

    // s := ((r * x + m) * k^-1) mod q
    const s = r.mul(bx).iumod(q)
               .iadd(bm).iumod(q)
               .imul(ki).iumod(q);

    // Unblind.
    s.imul(unblind).iumod(q);
...
function isSaneParams(params) {
  ...
  if (compare(params.g, params.p) >= 0)
    return false;

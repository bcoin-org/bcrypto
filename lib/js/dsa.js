...
dsa.privateKeyCreate = function privateKeyCreate(params) {
  ...
  const xn = BN.random(rng, 1, qn);
  const yn = gn.powm(xn, pn);
...
dsa.privateKeyTweakAdd = function privateKeyTweakAdd(key, tweak) {
  ...
  // xx := (x + (t mod q)) mod q
  const xx = new BN(key.x).iadd(t).iumod(q);

  if (xx.isZero())
    throw new Error('Invalid DSA tweak integer.');

  const yy = g.powm(xx, p);
...
/**
 * Multiply private key by tweak integer.
 * @param {DSAPrivateKey} key
 * @param {Buffer} tweak
 * @returns {DSAPrivateKey}
 */

dsa.privateKeyTweakMul = function privateKeyTweakMul(key, tweak) {
  assert(key instanceof DSAPrivateKey);
  assert(Buffer.isBuffer(tweak));
  ...
  // xx := (x * (t mod q)) mod q
  const xx = new BN(key.x).imul(t).iumod(q);
...
dsa.publicKeyTweakAdd = function publicKeyTweakAdd(key, tweak) {
  ...
  // yy := (g^(t mod q) * y) mod p
  const yy = g.powm(t, p).imul(y).iumod(p);
  const pub = new DSAPublicKey();
...
/**
 * Multiply public key by tweak integer.
 * @param {DSAKey} key
 * @param {Buffer} tweak
 * @returns {DSAPublicKey}
 */

dsa.publicKeyTweakMul = function publicKeyTweakMul(key, tweak) {
  assert(key instanceof DSAKey);
  assert(Buffer.isBuffer(tweak));
...
dsa._sign = function _sign(msg, key) {
  ...
  for (; attempts > 0; attempts--) {
    ...
    // Blinding factor.
    const b = BN.random(rng, 1, qn);
    const bi = fermatInverse(b, qn);

    // t := (b * x * r) mod q
    const t = b.mul(xn).iumod(qn)
               .imul(r).iumod(qn);

    // bm := (b * m) mod q
    const bm = b.imul(m).iumod(qn);

    // s := ((b * x * r) + (b * m)) mod q
    // s := (s * k^-1) mod q
    // s := (s * b^-1) mod q
    s = t.iadd(bm).iumod(qn)
         .imul(ki).iumod(qn)
         .imul(bi).iumod(qn);
  ...
  sig.r = r.toBuffer('be', n);
  sig.s = s.toBuffer('be', n);
...
dsa._verify = function _verify(msg, sig, key) {
  ...
  const m = new BN(msg);
  const si = sn.invm(qn);
  const msi = m.imul(si).iumod(qn);
  const rsi = rn.mul(si).iumod(qn);
  const yrsi = yn.powm(rsi, pn);
  const re = gn.powm(msi, pn)
               .imul(yrsi)
               .iumod(pn)
               .iumod(qn);
...
dsa.derive = function derive(pub, priv) {
  ...
  const pp = new BN(pub.p);
  const pq = new BN(pub.q);
  const pg = new BN(pub.g);
  ...
  const y = new BN(pub.y);

  // Multiply with blinding factor.
  for (;;) {
    const b = BN.random(rng, 1, q);
    const bn = q.sub(b);
    const xb = x.add(b).iumod(q);

    if (xb.isZero())
      continue;

    const xbp = y.powm(xb, p);

    if (xbp.isZero())
      continue;

    const bnp = y.powm(bn, p);

    if (bnp.isZero())
      continue;

    // Unblind.
    const s = xbp.imul(bnp).iumod(p);
...
generate:
  ...
  for (;;) {
    ...
    params.p = pn.toBuffer();
    params.q = qn.toBuffer();
    params.g = gn.toBuffer();

...
dsa._sign = function _sign(msg, key) {
  ...
  const entropy = leftPad(key.x, Math.max(24, bytes));
  ...
  for (let i = 0; i < 10; i++) {
    ...
    // an EGCD.
    const ki = fermatInverse(k, q);

    // Without blinding factor.
    // s := ((x * r + m) * k^-1) mod q
    // const s = x.mul(r).iumod(q)
    //            .iadd(m).iumod(q)
    //            .imul(ki).iumod(q);

    // Blinding factor.
    const b = BN.random(rng, 1, q);
    const bi = fermatInverse(b, q);

    // t := (b * x * r) mod q
    const t = b.mul(x).iumod(q)
               .imul(r).iumod(q);

    // bm := (b * m) mod q
    const bm = b.imul(m).iumod(q);

    // s := ((b * x * r) + (b * m)) mod q
    // s := (s * k^-1) mod q
    // s := (s * b^-1) mod q
    const s = t.iadd(bm).iumod(q)
               .imul(ki).iumod(q)
               .imul(bi).iumod(q);
    ...
    sig.r = r.toBuffer('be', bytes);
    sig.s = s.toBuffer('be', bytes);
...
dsa.derive = function derive(pub, priv) {
  ...
  for (;;) {
    ...
    const secret = blinded.imul(unblind).iumod(p);

    return secret.toBuffer();
  }
};

...
function _verify(msg, sig, key) {
  ...
  const P = G.powm(u1, p);
  const Q = Y.powm(u2, p);
  const R = P.imul(Q).imod(p);

  return R.imod(q).eq(r);
}
...
function derive(pub, priv) {
  ...
  const P = powConst(Y, x, p, q);

  return P.encode('be', p.byteLength());
}
...
function truncate(msg, q) {
  ...
  // differs OpenSSL's behavior. We
  // replicate the OpenSSL behavior
  // which takes the left-most
  // ceil(log2(n)) bits modulo `q`.
  assert(Buffer.isBuffer(msg));
...
function powBlind(Y, x, p, q, size = null) {
  ...
  // the calculation of `P` with a multi
  ...
  const u = b.neg().imod(q);
  const k = x.add(b).imod(q);
  const Yk = Y.powm(k, p, false, size);
  const Yu = Y.powm(u, p, false, size);

  return Yk.imul(Yu).imod(p);
}

function powConst(Y, x, p, q) {
  assert(q instanceof BN);
  return powBlind(Y, x, p, q, q.bitLength());
}

...
dsa._sign = function _sign(msg, key) {
  ...
  // https://github.com/openssl/openssl/blob/7f9822a/crypto/dsa/dsa_ossl.c#L95
  if (msg.length > n)
    msg = msg.slice(0, n);
  ...
  sig.setR(toBuffer(r), n);
  sig.setS(toBuffer(s), n);
...
dsa._verify = function _verify(msg, sig, key) {
  ...
  if (sig.r.length < 1 || sig.r.length > 32)
    return false;

  if (sig.s.length < 1 || sig.s.length > 32)
    return false;
  ...
  // https://github.com/openssl/openssl/blob/7f9822a/crypto/dsa/dsa_ossl.c#L330
  if (msg.length > (n >>> 3))
    msg = msg.slice(0, n >>> 3);

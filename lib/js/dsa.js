...
dsa.privateKeyTweakAdd = function privateKeyTweakAdd(key, tweak) {
  ...
  // xx = (x + (t % q)) % q
  const xx = new BN(key.x).iadd(t).umod(q);
...
dsa.privateKeyTweakMul = function privateKeyTweakMul(key, tweak) {
  ...
  // xx = (x * (t % q)) % q
  const xx = new BN(key.x).imul(t).umod(q);
...
dsa.publicKeyTweakAdd = function publicKeyTweakAdd(key, tweak) {
  ...
  // yy = (g^(t % q) * y) % p
  const yy = g.powm(t, p).imul(y).umod(p);
  const pub = new DSAPublicKey();
...
dsa._sign = function _sign(msg, key) {
  ...
  for (; attempts > 0; attempts--) {
    ...
    // t := (b * x * r) mod q
    const t = b.mul(xn).umod(qn)
               .imul(r).umod(qn);

    // bm := (b * m) mod q
    const bm = b.imul(m).umod(qn);

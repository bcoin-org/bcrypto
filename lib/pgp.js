...
class PGPPublicKey extends PGPBody {
  ...
  toKey() {
    switch (this.algorithm) {
      ...
      case keyTypes.RSA_ENCRYPT_ONLY:
      case keyTypes.RSA_SIGN_ONLY: {
        return new rsa.RSAPublicKey(this.n.data, this.e.data);
      }
      ...
      case keyTypes.EDDSA: {
        switch (this.curve) {
          case curveTypes.P256:
            return p256.PublicKey.fromPoint(this.point.data);
          case curveTypes.P384:
            return p384.PublicKey.fromPoint(this.point.data);
          case curveTypes.P521:
            return p521.PublicKey.fromPoint(this.point.data);
          case curveTypes.SECP256K1:
            return secp256k1.PublicKey.fromPoint(this.point.data);
          case curveTypes.ED25519:
            return ed25519.PublicKey.fromPoint(this.point.data);
          default:
  ...
  fromKey(key) {
    assert(key && typeof key.type === 'string');

    this.timestamp = Math.floor(Date.now() / 1000);

    switch (key.type) {
      case 'rsa': {
        this.algorithm = keyTypes.RSA;
        this.n.set(key.n);
        this.e.set(key.e);
        break;
      }
      ...
      case 'ecdsa':
      case 'eddsa': {
        this.algorithm = key.type === 'ecdsa'
          ? keyTypes.ECDSA
          : keyTypes.EDDSA;

        switch (key.curve) {
          case 'p256':
            this.curve = curveTypes.P256;
            break;
          case 'p384':
            this.curve = curveTypes.P384;
            break;
          case 'p521':
            this.curve = curveTypes.P521;
            break;
          case 'secp256k1':
            this.curve = curveTypes.SECP256K1;
            break;
          case 'ed25519':
            this.curve = curveTypes.ED25519;
            break;
          default:
            throw new Error('Unknown curve.');
        }

        this.point.set(key.toPoint());
...
class PGPPrivateKey extends PGPBody {
  ...
  toKey(passphrase) {
    const sk = this.secret(passphrase);
    return sk.toKey(this.key);
  }

  fromKey(key) {
    this.key.fromKey(key);
    this.data = SecretKey.fromKey(key).encode();
    return this;
  }
...
class CipherParams extends bio.Struct {
  ...
  encipher(pt, key) {
    ...
    const ctx = new Cipher(name).init(key, this.iv);

    return bio.concat(ctx.update(padded), ctx.final());
  }

  decipher(ct, key) {
    const name = this.algName();
    const ctx = new Decipher(name).init(key, this.iv);
    const padded = bio.concat(ctx.update(ct), ctx.final());
  ...
  toKey(pk) {
    ...
    switch (pk.algorithm) {
      ...
      case keyTypes.EDDSA: {
        switch (pk.curve) {
          case curveTypes.P256:
            return new p256.PrivateKey(sk.d.data);
          case curveTypes.P384:
            return new p384.PrivateKey(sk.d.data);
          case curveTypes.P521:
            return new p521.PrivateKey(sk.d.data);
          case curveTypes.SECP256K1:
            return new secp256k1.PrivateKey(sk.d.data);
          case curveTypes.ED25519:
            return new ed25519.PrivateKey(sk.d.data);
          default:
            throw new Error('Unsupported curve.');
        }
        break;

...
ec.privateKeyGenerate = function privateKeyGenerate() {
  let key;
...
ec.generatePrivateKey = ec.privateKeyGenerate;
...
ec.verify = function verify(msg, sig, key) {
  ...
  try {
    const s = secp256k1.signatureNormalize(sig);
...
ec.verifyDER = function verifyDER(msg, sig, key) {
  ...
  let s;
  try {
    s = secp256k1.signatureImportLax(sig);
  } catch (e) {
    return false;
  }

  return ec.verify(msg, s, key);
};
...
ec.recoverDER = function recoverDER(msg, sig, param, compress) {
  ...
  let s;
  try {
    s = secp256k1.signatureImport(sig);
  } catch (e) {
    return null;
  }

  return ec.recover(msg, s, param, compress);
};
...
ec.isLowDER = function isLowDER(raw) {
  assert(Buffer.isBuffer(raw));

  let sig;

  try {
    sig = secp256k1.signatureImport(raw);
  } catch (e) {
    return false;
  }

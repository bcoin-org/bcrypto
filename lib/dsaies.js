...
class DSAIES {
  ...
  _generate(priv, pub) {
    if (priv == null)
      return this.dsa.privateKeyCreate(pub);

    assert(priv instanceof this.dsa.DSAPrivateKey);
  ...
  encrypt(msg, pub, priv = null) {
    ...
    assert(pub instanceof this.dsa.DSAKey);
    assert(priv == null || (priv instanceof this.dsa.DSAPrivateKey));

    const klen = (pub.bits() + 7) >>> 3;

    const ourPriv = this._generate(priv, pub);
    const ourPub = this.dsa.publicKeyCreate(ourPriv);
    const secret = this.dsa.dh(pub, ourPriv);
  ...
  decrypt(msg, priv) {
    ...
    const theirPub = new this.dsa.DSAPublicKey(
      priv.p,
      priv.q,
      priv.g,
      theirY
    ...
    const secret = this.dsa.dh(theirPub, priv);
    const key = this.hash.digest(secret);

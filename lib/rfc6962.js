...
const slab = Buffer.alloc(8, 0x00);
...
exports.createTree = function createTree(alg, leaves) {
  ...
  for (let i = 0; i < leaves.length; i++) {
    const data = leaves[i];
    const leaf = hashLeaf(alg, data, i);
    nodes.push(leaf);
  }
...
exports.deriveRoot = function deriveRoot(alg, leaf, branch, index) {
  ...
  let root = hashLeaf(alg, leaf, index);
...
function hashLeaf(alg, data, index) {
  assert((index >>> 0) === index);
  ...
  return alg.multi(LEAF, data, slab);
}

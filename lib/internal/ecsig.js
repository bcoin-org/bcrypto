...
class ECSignature extends bio.Struct {
  ...
  static fromDERLax(data, size) {
    return new this().fromDERLax(data, size);
  }
}
...
function fromDERLax(raw, size) {
  const sig = ECSignature.fromDERLax(raw, size);
  return sig.encode(size);
}
...
function isLowDER(raw, size, half) {
  ...
  try {
    sig = ECSignature.fromStrict(raw, size);
  } catch (e) {
    return false;
  }

...
function isValidPoint(point, size) {
  ...
  const odd = point[point.length - 1] & 1;

  switch (point[0]) {
    case 0x02:
      return point.length === 1 + size && odd === 0;
    case 0x03:
      return point.length === 1 + size && odd === 1;
    case 0x04:
      return point.length === 1 + size * 2;
    case 0x06:
      return point.length === 1 + size * 2 && odd === 0;
    case 0x07:
      return point.length === 1 + size * 2 && odd === 1;
    default:
      return false;
  }
}

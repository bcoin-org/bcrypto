...
const {leftPad, rightPad} = require('./util');
...
function privateKeyImportJWK(curve, json) {
  ...
  if (curve.type === 'short')
    key = leftPad(key, curve.size);
  else
    key = rightPad(key, curve.size);
...
function publicKeyImportJWK(curve, json, compress) {
  ...
  if (curve.type !== 'short') {
    const x = rightPad(base64.decodeURL(json.x), curve.size);
    return curve.publicKeyImport(x);
  }

  const x = leftPad(base64.decodeURL(json.x), curve.size);
  const y = leftPad(base64.decodeURL(json.y), curve.size);

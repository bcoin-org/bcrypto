...
class RSAPrivateKey extends RSAKey {
  ...
  E() {
    if (this.e.equals(DUMMY))
      throw new Error('Invalid exponent E.');

    return new BN(this.e);
  }

  P() {
    if (this.p.equals(DUMMY))
      throw new Error('Invalid prime P.');

    return new BN(this.p);
  }

  Q() {
    if (this.q.equals(DUMMY))
      throw new Error('Invalid prime Q.');

    return new BN(this.q);
  }

  N(p, q) {
    if (this.n.equals(DUMMY))
      return p.mul(q);
    return new BN(this.n);
  }

  D(e, p, q) {
    if (this.d.equals(DUMMY)) {
      const t = p.subn(1).imul(q.subn(1));
      return e.invm(t);
    }
    return new BN(this.d);
  }

  DP(d, p) {
    if (this.dp.equals(DUMMY))
      return d.mod(p.subn(1));
    return new BN(this.dp);
  }

  DQ(d, q) {
    if (this.dq.equals(DUMMY))
      return d.mod(q.subn(1));
    return new BN(this.dq);
  }

  QI(q, p) {
    if (this.qi.equals(DUMMY))
      return q.invm(p);
    return new BN(this.qi);
  }
  ...
  compute() {
    ...
    const eb = e.bitLength();
    const nb = p.bitLength() + q.bitLength();

    if (e.cmpn(3) < 0 || e.isEven() || eb > MAX_EXP_BITS)
    ...
    const n = this.N(p, q);
    const d = this.D(e, p, q);
    const dp = this.DP(d, p);
    const dq = this.DQ(d, q);
    const qi = this.QI(q, p);

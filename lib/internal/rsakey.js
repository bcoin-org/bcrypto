...
class RSAPublicKey extends RSAKey {
  ...
  fromSSH(data) {
    ...
    const type = br.readString(7, 'binary');
...
function parsePEM(pem) {
  ...
  while (pem.length) {
    ...
    if (m) {
      ...
      if (!tag || tag !== m[1])
        throw new Error('PEM parse error (tag mismatch).');

      const data = fromBase64(chunk);
  ...
  if (chunks.length === 0)
    throw new Error('PEM parse error (no data).');

  if (tag)
    throw new Error('PEM parse error (un-ended tag).');

  ...
  read(br) {
    switch (this._mode) {
      case EXPLICIT: {
        ...
        if (this._optional && br.left() === 0)
          return this;

        const hdr = peekHeader(br);
...
class Integer extends Node {
  ...
  writeBody(bw) {
    ...
    if (val[i] & 0x80) {
      ...
      bw.copy(val, i - 1, val.length);
      return bw;
    }

    bw.copy(val, i, val.length);
    return bw;
  }

  readBody(br) {
    const val = br.readBytes(br.left());
  ...
  toNumber() {
    return bio.readUBE(this.value, 0, this.value.length);
  }

  fromNumber(num) {
    assert(Number.isSafeInteger(num) && num >= 0);
  ...
  format() {
    return `<${this.constructor.name}: ${this.value.toString('hex')}>`;
  }
}

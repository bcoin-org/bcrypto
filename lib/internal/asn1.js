...
function readScalar(data, pos, order, strict = true) {
  // Replicates libsecp256k1 behavior.
  assert(Buffer.isBuffer(data));
  assert((pos >>> 0) === pos);
  assert(order instanceof BN);
  assert(typeof strict === 'boolean');
  ...
  // Invalid size.
  if (size > order.byteLength())
    overflow = true;

  let num;

  if (!overflow) {
    num = BN.decode(data.slice(pos, pos + size));

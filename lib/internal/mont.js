...
class Mont {
  ...
  publicKeyConvert(pub, sign = false) {
    return this.ed.publicKeyDeconvert(pub, sign);
  }

  publicKeyDeconvert(pub) {
    return this.ed.publicKeyConvert(pub);
  }
  ...
  _scalarBaseMul(key) {
    ...
    if (this.ed.x) {
      const s = this.ed.x.decodeScalar(key);
      const k = this.ed.x.reduce(s);
      const p = this.ed.x.g.mul(k);
      return p.encode();
    }
  ...
  publicKeyFromUniform(bytes) {
    return this.ed.pointFromUniform(bytes);
  }

  publicKeyToUniform(pub, sign = false) {
    return this.ed.pointToUniform(pub, sign);
  }

  publicKeyFromHash(bytes) {
    return this.ed.pointFromHash(bytes);
  }

  publicKeyVerify(key) {
    return this.ed.pointVerify(key);
  }

  publicKeyIsSmall(key) {
    return this.ed.pointIsSmall(key);
  }

  publicKeyHasTorsion(key) {
    return this.ed.pointHasTorsion(key);
  }

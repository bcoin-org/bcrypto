...
class Node extends bio.Struct {
  ...
  get _optional() {
    return (this._flags & OPTIONAL) !== 0;
  }

  set _optional(value) {
    if (value)
      this._flags |= OPTIONAL;
    else
      this._flags &= ~OPTIONAL;
  }

  get _target() {
    return this._flags & TARGET;
  }
...
class Any extends Node {
  ...
  read(br) {
    assert(br);
    assert(this._mode === NORMAL);
    this.node = readAny(br, this._flags);
    return this;
  }
...
class OID extends Node {
  ...
  fromString(str) {
    ...
    else if (objects.hashOid.hasOwnProperty(str))
      str = objects.hashOid[str];
...
function readAny(br, flags) {
  ...
  const optional = (flags & OPTIONAL) !== 0;
  const hdr = peekHeader(br, optional);
...
function decodeAny(data, flags) {
  const br = bio.read(data);
  return readAny(br, flags);
}
...
function readSizeLax(br, primitive) {
  const size = br.readU8();
  ...
  if (br.offset + bytes > br.data.length)
    throw new bio.EncodingError(br.offset, 'Out of bounds read');

  while (bytes > 0 && br.data[br.offset] === 0x00) {
    bytes -= 1;
    br.offset += 1;
  }

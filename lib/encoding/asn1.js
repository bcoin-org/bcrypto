...
class UTCTime extends Node {
  ...
  readBody(br) {
    ...
    const str = br.readString(12, 'binary');
  ...
  format() {
    const date = new Date(this.value * 1000).toISOString();
    return `<${this.constructor.name}: ${this.value} (${date})>`;
  }
}
...
class GenTime extends Node {
  ...
  readBody(br) {
    ...
    const str = br.readString(14, 'binary');

...
class PEMBlock {
  ...
  encrypt(cipher, passwd) {
    ...
    const {name, keySize, blockSize} = ciphers.get(cipher);
    const iv = random.randomBytes(blockSize);
    const [key] = eb2k.derive(passwd, iv, keySize, blockSize);

    const ctx = new Cipher(cipher).init(key, iv);
  ...
  decrypt(passwd) {
    ...
    const {keySize, blockSize} = ciphers.get(info.cipher);
    const [key] = eb2k.derive(passwd, info.iv, keySize, blockSize);

    const ctx = new Decipher(info.cipher).init(key, info.iv);

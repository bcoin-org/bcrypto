...
class PEMBlock {
  ...
  encrypt(name, passwd) {
    ...
    const {keySize, blockSize} = cipher.get(name);
    ...
    this.setProcType(4, 'ENCRYPTED');
    this.setDEKInfo(name, iv);
  ...
  decrypt(passwd) {
    ...
    const {keySize, blockSize} = cipher.get(info.name);
    const [key] = eb2k.derive(passwd, info.iv, keySize, blockSize);

    this.data = cipher.decrypt(info.name, key, info.iv, this.data);

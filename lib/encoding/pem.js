...
class PEMBlock {
  ...
  encrypt(cipher, passwd) {
    ...
    const {name, keySize, blockSize} = crypto.getCipher(cipher);
    const iv = crypto.randomBytes(blockSize);
    const [key] = eb2k.derive(passwd, iv, keySize, blockSize);

    const ctx = crypto.encipher(cipher, key, iv);

    this.data = bio.concat(
      ctx.update(this.data),
      ctx.final()
    );
  ...
  decrypt(passwd) {
    ...
    const {keySize, blockSize} = crypto.getCipher(info.cipher);
    const [key] = eb2k.derive(passwd, info.iv, keySize, blockSize);

    const ctx = crypto.decipher(info.cipher, key, info.iv);

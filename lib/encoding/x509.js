...
const extensions = require('./x509-extensions');
const {types} = asn1;
...
class SubjectPublicKeyInfo extends asn1.Sequence {
  ...
  getJSON() {
    ...
    const OBJ =
      extensions.identifierToClass(this.algorithm.algorithm.getJSON());
    if (OBJ) {
      publicKey = new OBJ();
...
class Extension extends asn1.Sequence {
  ...
  getJSON() {
    ...
    const OBJ = extensions.identifierToClass(this.extnID.getJSON());
    if (OBJ) {
      val = new OBJ();

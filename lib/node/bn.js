...
class BN {
  ...
  ceq(num) {
    enforce(BN.isBN(num), 'num', 'bignum');
    return ((this.n ^ num.n) === 0n) | 0;
  }

  ceqn(num) {
    return ((this.n ^ BigInt(num | 0)) === 0n) | 0;
  }
  ...
  cswap(num, flag) {
    ...
    const bit = BigInt(cond);
    const a = (this.n * (bit ^ 1n)) + num.n * bit;
    const b = (num.n * (bit ^ 1n)) + this.n * bit;
  ...
  cinject(num, flag) {
    ...
    this.n = (this.n * (bit ^ 1n)) + num.n * bit;
  ...
  cset(num, flag) {
    const cond = ((flag >> 31) | (-flag >> 31)) & 1;
    const bit = BigInt(cond);
    const tmp = BN.fromNumber(num);

    this.n = (this.n * (bit ^ 1n)) + tmp.n * bit;
...
function climb(x, e, m, size) {
  ...
  const b256 = toBuffer(e, 'le', bytes);

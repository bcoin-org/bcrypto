...
class BN {
  ...
  forceRed(ctx) {
    ...
    if (this.red) {
      if (ctx.m !== this.red.m || ctx.mont !== this.red.mont)
        throw new Error('Already in reduction context.');
    } else {
      range(this.n >= 0n, 'red');
      range(this.n < ctx.m, 'red');
    }
  ...
  redIsNeg() {
    red(this.red, 'redIsNeg');
    return this.red.isNeg(this);
  }

  redIsPos() {
    red(this.red, 'redIsPos');
    return this.red.isPos(this);
  }
...
class Red {
  ...
  precompute() {
    ...
    if (this.sm1 === null && (this.m & 7n) === 5n) {
      // sqrt(-1) = 2^((p - 1) / 4) mod p
      this.sm1 = powm(2n, (this.m - 1n) >> 2n, this.m);
    }
  ...
  isqrt(a) {
    ...
    if (this.sm1 != null)
      a.n = sqrt5mod8sm1(a.n, this.m, this.sm1);
    else
  ...
  isSquare(a) {
    ...
    if (this.prime || this.coprime)
      symbol = legendre(a.n, this.m);
    else if (this.m & 1n)
      symbol = jacobi(a.n, this.m);
    else
      symbol = kronecker(a.n, this.m);

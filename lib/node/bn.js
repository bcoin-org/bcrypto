...
function divRound(x, y) {
  ...
  // Divide by two. Note that shifting
  // right by one will do floor division
  // if y is negative. We want truncated
  // division, so we add one if y is odd
  // and negative. This avoids an actual
  // division call even for negative
  // divisors.
  const t = (y & 1n) & (ys ? 1n : 0n);
  const h = (y >> 1n) + t;

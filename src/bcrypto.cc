...
bcrypto_bcrypt_verify(napi_env env, napi_callback_info info) {
  ...
  CHECK(napi_get_buffer_info(env, argv[0], (void **)&pass,
                             &pass_len) == napi_ok);
  CHECK(napi_get_value_string_utf8(env, argv[1], record, sizeof(record),
                                   &record_len) == napi_ok);
...
bcrypto_eb2k_derive(napi_env env, napi_callback_info info) {
  ...
  CHECK(create_external_buffer(env, key_len, key, &keyval) == napi_ok);
  CHECK(create_external_buffer(env, iv_len, iv, &ivval) == napi_ok);
...
bcrypto_ecdh_create(napi_env env, napi_callback_info info) {
  ...
  ec = (bcrypto_ecdh_t *)torsion_alloc(sizeof(bcrypto_ecdh_t));
  ec->ctx = ctx;
...
bcrypto_ecdsa_create(napi_env env, napi_callback_info info) {
  ...
  ec = (bcrypto_ecdsa_t *)torsion_alloc(sizeof(bcrypto_ecdsa_t));
  ec->ctx = ctx;
...
bcrypto_eddsa_create(napi_env env, napi_callback_info info) {
  ...
  ec = (bcrypto_eddsa_t *)torsion_alloc(sizeof(bcrypto_eddsa_t));
  ec->ctx = ctx;
...
bcrypto_schnorr_create(napi_env env, napi_callback_info info) {
  ...
  ec = (bcrypto_schnorr_t *)torsion_alloc(sizeof(bcrypto_schnorr_t));
  ec->ctx = ctx;
...
fail:
  ...
  torsion_free(msgs);
  torsion_free(msg_lens);
  torsion_free(sigs);
  torsion_free(sig_data);
  torsion_free(pubkeys);
  torsion_free(pubkey_data);

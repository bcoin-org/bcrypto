...
static void bcrypto_blake2s_set_lastnode( bcrypto_blake2s_ctx *S )
{
  S->f[1] = (uint32_t)-1;
}

/* Some helper functions, not necessarily useful */
static int bcrypto_blake2s_is_lastblock( const bcrypto_blake2s_ctx *S )
{
  return S->f[0] != 0;
}

static void bcrypto_blake2s_set_lastblock( bcrypto_blake2s_ctx *S )
{
  if( S->last_node ) bcrypto_blake2s_set_lastnode( S );

  S->f[0] = (uint32_t)-1;
}

static void bcrypto_blake2s_increment_counter( bcrypto_blake2s_ctx *S, const uint32_t inc )
{
  S->t[0] += inc;
  S->t[1] += ( S->t[0] < inc );
}

static void bcrypto_blake2s_init0( bcrypto_blake2s_ctx *S )
{
  size_t i;
  memset( S, 0, sizeof( bcrypto_blake2s_ctx ) );

  for( i = 0; i < 8; ++i ) S->h[i] = bcrypto_blake2s_IV[i];
}
...
int bcrypto_blake2s_init( bcrypto_blake2s_ctx *S, size_t outlen )
{
  ...
  if ( ( !outlen ) || ( outlen > BCRYPTO_BLAKE2S_OUTBYTES ) ) return -1;
...
int bcrypto_blake2s_init_key( bcrypto_blake2s_ctx *S, size_t outlen, const void *key, size_t keylen )
{
  ...
  if ( !key || !keylen || keylen > BCRYPTO_BLAKE2S_KEYBYTES ) return -1;
  ...
  if( bcrypto_blake2s_init_param( S, P ) < 0 ) return -1;

  {
    ...
    secure_zero_memory( block, BCRYPTO_BLAKE2S_BLOCKBYTES ); /* Burn the key from stack */
  }
  return 0;
}
...
static void bcrypto_blake2s_compress( bcrypto_blake2s_ctx *S, const uint8_t in[BCRYPTO_BLAKE2S_BLOCKBYTES] )
{
  uint32_t m[16];
  uint32_t v[16];
  size_t i;
...
int bcrypto_blake2s_update( bcrypto_blake2s_ctx *S, const void *pin, size_t inlen )
{
  ...
  if( inlen > 0 )
  {
    ...
    if( inlen > fill )
    {
      ...
      bcrypto_blake2s_compress( S, S->buf ); /* Compress */
      in += fill; inlen -= fill;
...
int bcrypto_blake2s_final( bcrypto_blake2s_ctx *S, void *out, size_t outlen )
{
  ...
  memset( S->buf + S->buflen, 0, BCRYPTO_BLAKE2S_BLOCKBYTES - S->buflen ); /* Padding */
...
int bcrypto_blake2s( void *out, size_t outlen, const void *in, size_t inlen, const void *key, size_t keylen )
{
  ...
  if( keylen > BCRYPTO_BLAKE2S_KEYBYTES ) return -1;

  if( keylen > 0 )
  {
    if( bcrypto_blake2s_init_key( S, outlen, key, keylen ) < 0 ) return -1;
  }
  else
  {
    if( bcrypto_blake2s_init( S, outlen ) < 0 ) return -1;
  }

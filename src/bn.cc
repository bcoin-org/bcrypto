...
BBN::Init(v8::Local<v8::Object> &target) {
  ...
  Nan::SetPrototypeMethod(tpl, "inot", BBN::Inot);
...
NAN_METHOD(BBN::Quorem) {
  ...
  BBN *q = ObjectWrap::Unwrap<BBN>(info[0].As<v8::Object>());
...
NAN_METHOD(BBN::Divmod) {
  ...
  if (mpz_sgn(b->n) < 0)
    mpz_cdiv_qr(q->n, r->n, a->n, b->n);
  else
    mpz_fdiv_qr(q->n, r->n, a->n, b->n);
...
NAN_METHOD(BBN::Idiv) {
  ...
  if (mpz_sgn(b->n) < 0)
    mpz_cdiv_q(a->n, a->n, b->n);
  else
    mpz_fdiv_q(a->n, a->n, b->n);
...
NAN_METHOD(BBN::Idivn) {
  ...
  // No good way to do this while
  // still avoiding an allocation.
  int asign = mpz_sgn(a->n) < 0;
  int bsign = num < 0;
...
NAN_METHOD(BBN::Imodn) {
  ...
  if (neg) {
    mpz_cdiv_r_ui(a->n, a->n, num);
    mpz_neg(a->n, a->n);
  } else {
    mpz_fdiv_r_ui(a->n, a->n, num);
  }
...
NAN_METHOD(BBN::Modrn) {
  ...
  if (neg)
    r = mpz_cdiv_ui(a->n, num);
  else
    r = mpz_fdiv_ui(a->n, num);
...
NAN_METHOD(BBN::Iuand) {
  ...
  int aneg = mpz_sgn(a->n) < 0;
  int bneg = mpz_sgn(b->n) < 0;
...
NAN_METHOD(BBN::IsPrimeMR) {
  ...
  if (!info[0]->IsFunction()) {
    ...
    data.context = info[0].As<v8::Object>();
    data.is_bytes = 0;
  }
...
bmpz_rng_custom(mpz_t ret, unsigned long bits, void *data) {
  ...
  v8::MaybeLocal<v8::Value> result_ =
    Nan::Call(rng_data->callback, rng_data->context, 1, argv);

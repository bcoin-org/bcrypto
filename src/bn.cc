...
IsInt(v8::Local<v8::Value> obj) {
  ...
  int64_t n = Nan::To<int64_t>(obj).FromJust();

  if (Nan::To<double>(obj).FromJust() != (double)n)
    return false;
...
IsSMI(v8::Local<v8::Value> obj) {
  ...
  return n >= -0x3ffffff && n <= 0x3ffffff;
}
...
BBN::Init(v8::Local<v8::Object> &target) {
  ...
  target->Set(Nan::New("BN").ToLocalChecked(), ctor->GetFunction());
}
...
NAN_METHOD(BBN::Idiv) {
  ...
  if (mpz_sgn(b->n) == 0)
    return Nan::ThrowRangeError("Cannot divide by zero.");
...
NAN_METHOD(BBN::IsSquare) {
  ...
  bool ret = mpz_perfect_square_p(a->n) != 0;
...
NAN_METHOD(BBN::Bincn) {
  ...
  mpz_init(tmp);
  mpz_set_ui(tmp, 1);
  mpz_mul_2exp(tmp, tmp, bit);
  mpz_sub_ui(tmp, tmp, 1);
  mpz_and(tmp, a->n, tmp);
  mpz_fdiv_q_2exp(a->n, a->n, bit);
  mpz_add_ui(a->n, a->n, 1);
  mpz_mul_2exp(a->n, a->n, bit);
  mpz_ior(a->n, a->n, tmp);
  mpz_clear(tmp);
...
NAN_METHOD(BBN::Ipowm) {
  ...
  if (mpz_sgn(b->n) < 0) {
    ...
    if (mpz_invert(y, y, c->n) == 0)
      return Nan::ThrowError("Not invertible.");

    mpz_powm(a->n, a->n, y, c->n);
    mpz_clear(y);
  } else {
    mpz_powm(a->n, a->n, b->n, c->n);
  }
...
NAN_METHOD(BBN::IsPrimeMR) {
  ...
  bool r = bmpz_prime_mr(a->n, nreps, force2);
...
NAN_METHOD(BBN::ToNumber) {
  ...
  if (mpz_sizeinbase(a->n, 2) > 53)
    return Nan::ThrowRangeError("Number exceeds 53 bits.");
...
NAN_METHOD(BBN::FromDouble) {
  ...
  double n = Nan::To<double>(info[0]).FromJust();

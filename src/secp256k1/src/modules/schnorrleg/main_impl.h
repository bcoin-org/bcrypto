...
static int secp256k1_nonce_function_schnorrleg(unsigned char *nonce32, const unsigned char *msg32, const unsigned char *key32, const unsigned char *algo16, void *data, unsigned int counter) {
...
int secp256k1_schnorrleg_serialize(const secp256k1_context* ctx, unsigned char *out64, const secp256k1_schnorrleg* sig) {
    (void) ctx;
...
int secp256k1_schnorrleg_parse(const secp256k1_context* ctx, secp256k1_schnorrleg* sig, const unsigned char *in64) {
    (void) ctx;
    VERIFY_CHECK(ctx != NULL);
    ARG_CHECK(sig != NULL);
    ARG_CHECK(in64 != NULL);
...
int secp256k1_schnorrleg_sign(const secp256k1_context* ctx, secp256k1_schnorrleg *sig, int *nonce_is_negated, const unsigned char *msg32, const unsigned char *seckey, secp256k1_nonce_function noncefp, void *ndata) {
    secp256k1_scalar x;
    secp256k1_scalar e;
    secp256k1_scalar k;
    secp256k1_gej pkj;
    ...
    if (noncefp == NULL) {
        noncefp = secp256k1_nonce_function_schnorrleg;
    }
 ...
 * Computes R = sG - eP. */
static int secp256k1_schnorrleg_real_verify(const secp256k1_context* ctx, secp256k1_gej *rj, const secp256k1_scalar *s, const secp256k1_scalar *e, const secp256k1_pubkey *pk) {
...
int secp256k1_schnorrleg_verify(const secp256k1_context* ctx, const secp256k1_schnorrleg *sig, const unsigned char *msg32, const secp256k1_pubkey *pk) {
    secp256k1_scalar s;
    secp256k1_scalar e;
    secp256k1_gej rj;
    secp256k1_fe rx;
...
typedef struct {
     ...
     * the PRNG is called at every odd indexed schnorrleg to fill the cache. */
...
static int secp256k1_schnorrleg_verify_batch_ecmult_callback(secp256k1_scalar *sc, secp256k1_ge *pt, size_t idx, void *data) {
    secp256k1_schnorrleg_verify_ecmult_context *ecmult_context = (secp256k1_schnorrleg_verify_ecmult_context *) data;
 ...
 *  In/Out   sha: an initialized sha256 object which hashes the schnorrleg input in order to get a
 ...
 */
int secp256k1_schnorrleg_verify_batch_init_randomizer(const secp256k1_context *ctx, secp256k1_schnorrleg_verify_ecmult_context *ecmult_context, secp256k1_sha256 *sha, const secp256k1_schnorrleg *const *sig, const unsigned char *const *msg32, const secp256k1_pubkey *const *pk, size_t n_sigs) {
    size_t i;
 ...
 */
int secp256k1_schnorrleg_verify_batch_sum_s(secp256k1_scalar *s, unsigned char *chacha_seed, const secp256k1_schnorrleg *const *sig, size_t n_sigs) {
    secp256k1_scalar randomizer_cache[2];
    size_t i;
...
int secp256k1_schnorrleg_verify_batch(const secp256k1_context *ctx, secp256k1_scratch *scratch, const secp256k1_schnorrleg *const *sig, const unsigned char *const *msg32, const secp256k1_pubkey *const *pk, size_t n_sigs) {
    ...
    secp256k1_sha256_initialize(&sha);
    if (!secp256k1_schnorrleg_verify_batch_init_randomizer(ctx, &ecmult_context, &sha, sig, msg32, pk, n_sigs)) {
        return 0;
    }
    ...
    if (!secp256k1_schnorrleg_verify_batch_sum_s(&s, ecmult_context.chacha_seed, sig, n_sigs)) {
        return 0;
    }
    secp256k1_scalar_negate(&s, &s);

    return secp256k1_ecmult_multi_var(&ctx->ecmult_ctx, scratch, &rj, &s, secp256k1_schnorrleg_verify_batch_ecmult_callback, (void *) &ecmult_context, 2 * n_sigs)

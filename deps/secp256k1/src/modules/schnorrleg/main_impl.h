...
secp256k1_schnorrleg_sign(const secp256k1_context *ctx,
  ...
  secp256k1_schnorrleg_hash_nonce(buf, msg, msg_len, seckey);
  ...
  secp256k1_fe_get_b32(sig, &R.x);

  secp256k1_eckey_pubkey_serialize(&A, buf, &buflen, 1);
  secp256k1_schnorrleg_hash_challenge(buf, sig, buf, msg, msg_len);
...
secp256k1_schnorrleg_verify(const secp256k1_context *ctx,
  ...
  if (!secp256k1_fe_set_b32(&rx, &sig[0]))
    return 0;
  ...
  if (!secp256k1_gej_has_quad_y_var(&R)) /* Fails if R is infinity. */
    return 0;

  if (!secp256k1_gej_eq_x_var(&rx, &R))
    return 0;
...
static int
secp256k1_schnorrleg_batch_callback(secp256k1_scalar *sc,
                                    secp256k1_ge *pt,
                                    size_t idx,
                                    void *data) {
  ...
  if (idx % 2 == 0) {
    ...
    *sc = batch->randomizer_cache[(idx / 2) % 2];

    if (!secp256k1_fe_set_b32(&rx, batch->sig[idx / 2]))
      return 0;
    ...
    secp256k1_ec_pubkey_serialize(batch->ctx, buf, &buflen,
                                  batch->pk[idx / 2],
                                  SECP256K1_EC_COMPRESSED);

    secp256k1_schnorrleg_hash_challenge(buf,
                                        batch->sig[idx / 2],
                                        buf,
                                        batch->msg[idx / 2],
                                        batch->msg_len[idx / 2]);
    ...
    secp256k1_scalar_mul(sc, sc, &batch->randomizer_cache[(idx / 2) % 2]);

    if (!secp256k1_pubkey_load(batch->ctx, pt, batch->pk[idx / 2]))
      return 0;
  }
...
secp256k1_schnorrleg_batch_sum_s(secp256k1_scalar *sum,
  ...
  for (i = 0; i < n_sigs; i++) {
    ...
    secp256k1_scalar_set_b32(&s, sig[i] + 32, &overflow);
...
secp256k1_schnorrleg_verify_batch(const secp256k1_context *ctx,
  ...
  if (!secp256k1_schnorrleg_batch_sum_s(&sum, batch.chacha_seed, sig, n_sigs))
    return 0;
